...
class KeyBaseTable(BaseTable):
    ...
    def get_unique_key(self) -> int:
        ...
        while True:
            ...
            wrapdisp3212 = RefOutArgWrapper(0)
            wraplen3213 = RefOutArgWrapper(0)
            inoutres3214 = self.__read_index_info(self.__m_unique_key_position, wrapdisp3212, wraplen3213)
            disp = wrapdisp3212.value
            len0_ = wraplen3213.value
            if (not inoutres3214): 
                continue
            if (disp == (0) and len0_ == 0): 
                retVal3696 = self.__m_unique_key_position
                self.__m_unique_key_position += 1
                return retVal3696
        self.__m_unique_key_position = -1
        return max0_ + 1
    ...
    def open0_(self, read_only : bool, index_in_memory_max_length : int=0) -> bool:
        ...
        info = self._read_index0()
        if (info is not None and len(info) == KeyBaseTable.__index_record_size): 
            if ((((info[KeyBaseTable.__index_record_size - 1]) & 1)) != 0): 
                self.auto_zip_data = True
        if (not read_only and self.flex_auto_zip_data): 
            if (info is None): 
                info = Utils.newArrayOfBytes(KeyBaseTable.__index_record_size, 0)
            info[KeyBaseTable.__index_record_size - 1] |= (1)
            self.auto_zip_data = True
            self._write_index0(info)
        if (((index_in_memory_max_length > 0 and self._m_index.length <= index_in_memory_max_length)) or self.load_all_in_memory): 
        ...
        if (self.load_all_in_memory): 
            self._m_data_buf = Utils.newArrayOfBytes(self._m_data.length, 0)
            self._m_data.position = 0
            self._m_data.read(self._m_data_buf, 0, len(self._m_data_buf))
        return True
    ...
    def read_key_data_len(self, key : int) -> int:
        ...
        disp = 0
        len0_ = 0
        wrapdisp3215 = RefOutArgWrapper(0)
        wraplen3216 = RefOutArgWrapper(0)
        inoutres3217 = self.__read_index_info(key, wrapdisp3215, wraplen3216)
        disp = wrapdisp3215.value
        len0_ = wraplen3216.value
        if (not inoutres3217): 
            return -1
        else: 
            return len0_
    ...
    def read_key_data(self, key : int, max_len : int=0) -> bytearray:
        """ Получить записи для 1-го терма
        ...
        disp = 0
        len0_ = 0
        wrapdisp3218 = RefOutArgWrapper(0)
        wraplen3219 = RefOutArgWrapper(0)
        inoutres3220 = self.__read_index_info(key, wrapdisp3218, wraplen3219)
        disp = wrapdisp3218.value
        len0_ = wraplen3219.value
        if (not inoutres3220): 
            if (log0_): 
        ...
        if (self._m_data_buf is not None): 
            i = 0
            while i < len0_: 
                res[i] = self._m_data_buf[disp + (i)]
                i += 1
        else: 
    ...
    def read_keys_data(self, key_min : int, max_count : int, max_data_size : int=10000000) -> typing.List[tuple]:
        ...
        if (self._m_data_buf is not None): 
            i = 0
            while i < len(dats): 
                dats[i] = self._m_data_buf[disp0 + (i)]
                i += 1
        else: 
    ...
    def remove_key_data(self, key : int) -> None:
        ...
        disp = 0
        len0_ = 0
        wrapdisp3221 = RefOutArgWrapper(0)
        wraplen3222 = RefOutArgWrapper(0)
        inoutres3223 = self.__read_index_info(key, wrapdisp3221, wraplen3222)
        disp = wrapdisp3221.value
        len0_ = wraplen3222.value
        if (not inoutres3223): 
            return
        if (disp == (0)): 
            return
    ...
    def update_part_of_data(self, key : int, data : bytearray, pos : int) -> None:
        disp = 0
        len0_ = 0
        wrapdisp3224 = RefOutArgWrapper(0)
        wraplen3225 = RefOutArgWrapper(0)
        inoutres3226 = self.__read_index_info(key, wrapdisp3224, wraplen3225)
        disp = wrapdisp3224.value
        len0_ = wraplen3225.value
        if (not inoutres3226): 
            return
    ...
    def update_start_of_data(self, key : int, data : bytearray) -> None:
        disp = 0
        len0_ = 0
        wrapdisp3227 = RefOutArgWrapper(0)
        wraplen3228 = RefOutArgWrapper(0)
        inoutres3229 = self.__read_index_info(key, wrapdisp3227, wraplen3228)
        disp = wrapdisp3227.value
        len0_ = wraplen3228.value
        if (not inoutres3229): 
            return
        self._m_data.position = disp
        self._m_data.write(data, 0, len(data))
    ...
    def __add_data(self, key : int, data : bytearray, dst : Stream) -> None:
        ...
        if (dst == self._m_data): 
            disp = 0
            len0_ = 0
            wrapdisp3230 = RefOutArgWrapper(0)
            wraplen3231 = RefOutArgWrapper(0)
            inoutres3232 = self.__read_index_info(key, wrapdisp3230, wraplen3231)
            disp = wrapdisp3230.value
            len0_ = wraplen3231.value
            if (inoutres3232): 
    ...
    def optimize(self, min_percent : int=10) -> bool:
        ...
        tmp_table.flex_auto_zip_data = self.flex_auto_zip_data
        tmp_table.open0_(False, 0)
        tmp_table.flex_auto_zip_data = False
        sw = Stopwatch()
        max0_ = self.get_max_key()
        id0_ = 1
        sw.start()
        ...
        self.auto_zip_data = False
        flex_auto_zip = self.flex_auto_zip_data
        self.flex_auto_zip_data = False
        while id0_ <= max0_:
            if (max0_ > 10000): 
        ...
        self.flex_auto_zip_data = flex_auto_zip
        pathlib.Path(self._m_data_file_name).unlink()
    ...
    @staticmethod
    def _new29(_arg1 : 'IRepository', _arg2 : str, _arg3 : bool) -> 'KeyBaseTable':
        res = KeyBaseTable(_arg1, _arg2)

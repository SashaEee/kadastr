...
class StringDictionaryTable(BaseTable):
    """ Таблица репозитория типа "ключ - строковое значение" """
    ...
    def __restore(self, data : bytearray) -> None:
        ind = 0
        while ind < len(data):
            wrapind3233 = RefOutArgWrapper(ind)
            s = BaseTable.get_string_for_bytes(data, wrapind3233, False, None)
            ind = wrapind3233.value
            if (s is None): 
                break
    ...
    def get_code_by_string(self, val : str, add_if_not_exist : bool) -> int:
        if (Utils.isNullOrEmpty(val)): 
            return 0
        id0_ = 0
        wrapid3234 = RefOutArgWrapper(0)
        inoutres3235 = Utils.tryGetValue(self.__m_hash, val, wrapid3234)
        id0_ = wrapid3234.value
        if (inoutres3235): 
            return id0_

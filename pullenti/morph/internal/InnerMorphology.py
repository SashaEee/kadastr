...
class InnerMorphology:
    ...
    def run(self, text : str, only_tokenizing : bool, dlang : 'MorphLang', good_text : bool, progress : EventHandler) -> typing.List['MorphToken']:
        ...
        while True:
            ...
            if (ty == 1 and not only_tokenizing): 
                wraplemmas275 = RefOutArgWrapper(None)
                inoutres276 = Utils.tryGetValue(uni_lex, term, wraplemmas275)
                lemmas = wraplemmas275.value
                if (not inoutres276): 
        ...
        if (pure_rus_words > pure_ukr_words and pure_rus_words > pure_by_words and pure_rus_words > pure_kz_words): 
            ...
            if ((pure_by_words < 10) and dlang is not None and dlang.is_ru): 
                pass
            elif (pure_by_words > 5): 
        ...
        if (((def_lang.is_undefined or def_lang.is_ua)) and tot_rus_words > 0): 
            if (((tot_ukr_words > tot_rus_words and self.__m_engine_ua.language.is_ua)) or ((tot_by_words > tot_rus_words and self.__m_engine_by.language.is_by)) or ((tot_kz_words > tot_rus_words and self.__m_engine_kz.language.is_kz))): 
                ...
                for kp in uni_lex.items(): 
                    lang = MorphLang()
                    wraplang277 = RefOutArgWrapper(lang)
                    kp[1].word_forms = self.__process_one_word(kp[0], wraplang277)
                    lang = wraplang277.value
        ...
        for kp in uni_lex.items(): 
            ...
            if (lang.is_undefined): 
                if (tot_rus_words > tot_by_words and tot_rus_words > tot_ukr_words and tot_rus_words > tot_kz_words): 
                    ...
                    lang = MorphLang.KZ
            wraplang278 = RefOutArgWrapper(lang)
            kp[1].word_forms = self.__process_one_word(kp[0], wraplang278)
            lang = wraplang278.value
            kp[1].lang = lang
            if (lang.is_ru): 
                tot_rus_words += 1
            if (lang.is_ua): 
        ...
        if (not good_text): 
            ...
            while True:
                ...
                elif (((ui1.is_hiphen and ui0.is_letter and ui2.is_letter) and res[i].end_char > res[i].begin_char and res[i + 2].end_char > res[i + 2].begin_char) and (((res[i + 2].end_char + 1) >= len(twrch) or not twrch[res[i + 2].end_char + 1].is_hiphen))): 
                    newline = False
                    sps = 0
            ...
            if (mt.char_info.is_last_lower and mt.length > 2 and mt.char_info.is_cyrillic_letter): 
                ...
                for wf in mt.word_forms: 
                    if (wf.normal_case == pref or wf.normal_full == pref): 
                        ...
                        break
                if (not ok): 
                    wf0 = MorphWordForm._new279(pref, MorphClass.NOUN, 1)
    ...
    def get_all_words_by_class(self, cla : 'MorphClass', lang : 'MorphLang') -> typing.List['MorphWordForm']:
        tmp = io.StringIO()
        res = list()
        if (self.__m_engine_ru.language.is_ru and ((lang is None or lang.is_ru or lang.is_undefined))): 
            self.__m_engine_ru.get_all_words_by_class(cla, res, tmp)
        if (self.__m_engine_en.language.is_en and ((lang is None or lang.is_en or lang.is_undefined))): 
            self.__m_engine_en.get_all_words_by_class(cla, res, tmp)
        if (self.__m_engine_ua.language.is_ru and ((lang is None or lang.is_ua or lang.is_undefined))): 
            self.__m_engine_ua.get_all_words_by_class(cla, res, tmp)
        return res
    ...
    def __process_one_word0(self, wstr : str) -> typing.List['MorphWordForm']:
        dl = MorphLang()
        wrapdl280 = RefOutArgWrapper(dl)
        inoutres281 = self.__process_one_word(wstr, wrapdl280)
        dl = wrapdl280.value
        return inoutres281

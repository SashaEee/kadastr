...
class MorphEngine:
    ...
    def __load_tree_node(self, tn : 'MorphTreeNode') -> None:
        with self._m_lock: 
            pos = tn.lazy_pos
            if (pos > 0): 
                wrappos282 = RefOutArgWrapper(pos)
                tn._deserialize_lazy(self.__m_lazy_buf, self, wrappos282)
                pos = wrappos282.value
            tn.lazy_pos = 0
    ...
    def process(self, word : str, ignore_no_dict : bool=False) -> typing.List['MorphWordForm']:
        ...
        while i <= len(word): 
            ...
            if (tn.nodes is None or i >= len(word)): 
                break
            ch = ord(word[i])
            wraptn283 = RefOutArgWrapper(None)
            inoutres284 = Utils.tryGetValue(tn.nodes, ch, wraptn283)
            tn = wraptn283.value
            if (not inoutres284): 
                break
            i += 1
            ...
            for r in res: 
                if ((r.class0_.is_pronoun or r.class0_.is_noun or r.class0_.is_adjective) or (r.class0_.is_misc and r.class0_.is_conjunction) or r.class0_.is_preposition): 
                    ...
                    if (ok and not LanguageHelper.ends_with(word, "ИМ")): 
                        need_test_unknown_vars = False
                if (len(res) > 1 and to_first is None): 
                    if (((((r.normal_full == "КОПИЯ" or r.normal_full == "ПОЛК" or r.normal_full == "СУД") or r.normal_case == "МОРЕ" or r.normal_full == "ПАРК") or r.normal_full == "БАНК" or r.normal_case == "МАРИЯ") or r.normal_case == "ВЕТЕР" or r.normal_case == "КИЕВ") or r.normal_full == "ПОЛОЖЕНИЕ"): 
                        to_first = r
        if (to_first is not None and res[0] != to_first): 
            res.remove(to_first)
            res.insert(0, to_first)
    ...
    def get_all_words_by_class(self, cla : 'MorphClass', res : typing.List['MorphWordForm'], tmp : io.StringIO) -> None:
        self.__get_all_words_by_class(cla, self.m_root, res, tmp)
    
    def __get_all_words_by_class(self, cla : 'MorphClass', tn : 'MorphTreeNode', res : typing.List['MorphWordForm'], tmp : io.StringIO) -> None:
        if (tn.lazy_pos > 0): 
            self.__load_tree_node(tn)
        if (tn.rule_ids is not None): 
            for rid in tn.rule_ids: 
                r = self.get_rule(rid)
                i = 0
                while i < len(r.morph_vars): 
                    v = r.morph_vars[i]
                    wf = None
                    for vv in v: 
                        if (not ((vv.class0_) & cla).is_undefined): 
                            if (not vv.case_.is_undefined and not vv.case_.is_nominative): 
                                continue
                            if (((vv.number) & (MorphNumber.SINGULAR)) != (MorphNumber.SINGULAR) and vv.number != MorphNumber.UNDEFINED): 
                                continue
                            wf = MorphWordForm()
                            wf.copy_from(vv)
                            wf.normal_case = Utils.toStringStringIO(tmp)
                            if (not Utils.isNullOrEmpty(r.tails[i])): 
                                wf.normal_case += r.tails[i]
                            res.append(wf)
                            break
                    i += 1
        if (tn.nodes is not None): 
            for kp in tn.nodes.items(): 
                print(chr(kp[0]), end="", file=tmp)
                self.__get_all_words_by_class(cla, kp[1], res, tmp)
    ...
    def get_all_wordforms(self, word : str) -> typing.List['MorphWordForm']:
        ...
        while i <= len(word): 
            ...
            if (tn.nodes is None or i >= len(word)): 
                break
            ch = ord(word[i])
            wraptn285 = RefOutArgWrapper(None)
            inoutres286 = Utils.tryGetValue(tn.nodes, ch, wraptn285)
            tn = wraptn285.value
            if (not inoutres286): 
                break
            i += 1
    ...
    def get_wordform(self, word : str, cla : 'MorphClass', gender : 'MorphGender', cas : 'MorphCase', num : 'MorphNumber', add_info : 'MorphWordForm') -> str:
        ...
        while i <= len(word): 
            ...
            if (tn.nodes is None or i >= len(word)): 
                break
            ch = ord(word[i])
            wraptn287 = RefOutArgWrapper(None)
            inoutres288 = Utils.tryGetValue(tn.nodes, ch, wraptn287)
            tn = wraptn287.value
            if (not inoutres288): 
                break
            i += 1
    ...
    def __calc_eq_coef(self, v : 'MorphRuleVariant', wf : 'MorphWordForm') -> int:
        if (wf.class0_.value != (0)): 
            if ((((v.class0_.value) & (wf.class0_.value))) == 0): 
                return -1
        if (wf.misc is not None and v.misc_info_id != wf.misc.id0_): 
    ...
    def deserialize(self, str0 : Stream, ignore_rev_tree : bool, lazy_load : bool) -> None:
        ...
        pos = 0
        wrappos297 = RefOutArgWrapper(pos)
        cou = buf.deserialize_int(wrappos297)
        pos = wrappos297.value
        while cou > 0: 
            mi = MorphMiscInfo()
            wrappos289 = RefOutArgWrapper(pos)
            mi._deserialize(buf, wrappos289)
            pos = wrappos289.value
            self.add_misc_info(mi)
            cou -= 1
        wrappos296 = RefOutArgWrapper(pos)
        cou = buf.deserialize_int(wrappos296)
        pos = wrappos296.value
        while cou > 0: 
            wrappos291 = RefOutArgWrapper(pos)
            p1 = buf.deserialize_int(wrappos291)
            pos = wrappos291.value
            r = MorphRule()
            if (lazy_load): 
                r.lazy_pos = pos
                pos = p1
            else: 
                wrappos290 = RefOutArgWrapper(pos)
                r._deserialize(buf, wrappos290)
                pos = wrappos290.value
            self.add_rule(r)
            cou -= 1
        root = MorphTreeNode()
        if (lazy_load): 
            wrappos292 = RefOutArgWrapper(pos)
            root._deserialize_lazy(buf, self, wrappos292)
            pos = wrappos292.value
        else: 
            wrappos293 = RefOutArgWrapper(pos)
            root._deserialize(buf, wrappos293)
            pos = wrappos293.value
        self.m_root = root
        if (not ignore_rev_tree): 
            root_rev = MorphTreeNode()
            if (lazy_load): 
                wrappos294 = RefOutArgWrapper(pos)
                root_rev._deserialize_lazy(buf, self, wrappos294)
                pos = wrappos294.value
            else: 
                wrappos295 = RefOutArgWrapper(pos)
                root_rev._deserialize(buf, wrappos295)
                pos = wrappos295.value

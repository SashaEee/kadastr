...
class LanguageHelper:
    ...
    def transliteral_correction(value : str, prev_value : str, always : bool=False) -> str:
        ...
        first_pass3322 = True
        while True:
            if first_pass3322: first_pass3322 = False
            else: i += 1
    ...
    def get_case_after_preposition(prep : str) -> 'MorphCase':
        mc = None
        wrapmc299 = RefOutArgWrapper(None)
        inoutres300 = Utils.tryGetValue(LanguageHelper.__m_prep_cases, prep, wrapmc299)
        mc = wrapmc299.value
        if (inoutres300): 
            return mc
        else: 
    ...
    def normalize_preposition(prep : str) -> str:
        res = None
        wrapres301 = RefOutArgWrapper(None)
        inoutres302 = Utils.tryGetValue(LanguageHelper.__m_prep_norms, prep, wrapres301)
        res = wrapres301.value
        if (inoutres302): 
            return res
        else: 
            return prep

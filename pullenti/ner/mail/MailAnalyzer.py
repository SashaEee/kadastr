        ...
        while True:
            ...
            if (lines[0].typ == MailLine.Types.FROM): 
                ...
                while i < len(lines): 
                    if (lines[i].typ == MailLine.Types.FROM): 
                        ...
                        break
                    i += 1
                mail_ = MailReferent._new1772(MailKind.HEAD)
            ...
            while ii < len(lines): 
                if (lines[ii].typ == MailLine.Types.HELLO): 
                    mail_ = MailReferent._new1772(MailKind.HELLO)
            ...
            if (i0 < len(lines)): 
                if (t2 is not None and t2.previous is None): 
                    ...
                    tt00 = lines[i0].begin_token
                    tt11 = (t2.previous if t2 is not None and t2.previous is not None else lines[len(lines) - 1].end_token)
                    if (tt00.begin_char > tt11.end_char): 
                        pass
                    else: 
                        mail_ = MailReferent._new1772(MailKind.BODY)
                        mt = ReferentToken(mail_, tt00, tt11)
                        if (mt.length_char > 0): 
                            mail_.text = MiscHelper.get_text_value_of_meta_token(mt, GetTextAttr.KEEPREGISTER)
                            ad.register_referent(mail_)
                            mail_.add_occurence_of_ref_tok(mt)
                if (t2 is not None): 
                    mail_ = MailReferent._new1772(MailKind.TAIL)

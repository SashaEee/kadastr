...
class UnitsHelper:
    ...
    def initialize() -> None:
        ...
        uu = None
        t = None
        u = Unit._new1815("раз", "times", "раз", "times", MeasureKind.COUNT)
        UnitsHelper.UNITS.append(u)
        t = Termin._new124("РАЗ", u)
        UnitsHelper.TERMINS.add(t)
        u = Unit._new1815("м", "m", "метр", "meter", MeasureKind.LENGTH)
        ...
        uu = Unit._new1815("миль", "mile", "морская миля", "mile", MeasureKind.LENGTH)
        uu.base_unit = u
        uu.base_multiplier = (1852)
        UnitsHelper.UNITS.append(uu)
        t = Termin._new124("МИЛЯ", uu)
        t.add_variant("МОРСКАЯ МИЛЯ", False)
        t.add_abridge("NMI")
        t.add_variant("MILE", False)
        t.add_variant("NAUTICAL MILE", False)
        UnitsHelper.TERMINS.add(t)
        uu = Unit._new1821("фут", "ft", "фут", "foot", u, 0.304799472, MeasureKind.LENGTH)
        UnitsHelper.UNITS.append(uu)
        t = Termin._new124("ФУТ", uu)
        t.add_abridge("FT.")
        t.add_variant("FOOT", False)
        UnitsHelper.TERMINS.add(t)
        uu = Unit._new1821("дюйм", "in", "дюйм", "inch", u, 0.0254, MeasureKind.LENGTH)
        UnitsHelper.UNITS.append(uu)
        t = Termin._new124("ДЮЙМ", uu)
        t.add_abridge("IN")
        t.add_variant("INCH", False)
        UnitsHelper.TERMINS.add(t)
        u = Unit._new1815("ар", "are", "ар", "are", MeasureKind.AREA)
        u.keywords.extend(["ПЛОЩАДЬ", "ПРОЩИНА", "AREA", "SQWARE", "SPACE"])
        UnitsHelper.UNITS.append(u)
        t = Termin._new124("АР", u)
        t.add_variant("ARE", False)
        t.add_variant("СОТКА", False)
        UnitsHelper.TERMINS.add(t)
        uu = Unit._new1815("га", "ga", "гектар", "hectare", MeasureKind.AREA)
        uu.base_unit = u
        uu.base_multiplier = (100)
        UnitsHelper.UNITS.append(uu)
        t = Termin._new124("ГЕКТАР", uu)
        t.add_variant("HECTARE", False)
        t.add_abridge("ГА")
        t.add_abridge("GA")
        UnitsHelper.TERMINS.add(t)
        u = Unit._new1815("г", "g", "грамм", "gram", MeasureKind.WEIGHT)
        u.keywords.extend(["ВЕС", "ТЯЖЕСТЬ", "НЕТТО", "БРУТТО", "МАССА", "НАГРУЗКА", "ЗАГРУЗКА", "УПАКОВКА", "WEIGHT", "NET", "GROSS", "MASS", "ВАГА", "ТЯЖКІСТЬ", "МАСА"])
        UnitsHelper.UNITS.append(u)
        t = Termin._new124("ГРАММ", u)
        t.add_abridge("Г.")
        t.add_abridge("ГР.")
        t.add_abridge("G.")
        t.add_abridge("GR.")
        ...
        uu = Unit._new1821("ц", "centner", "центнер", "centner", u, 100000, MeasureKind.WEIGHT)
        UnitsHelper.UNITS.append(uu)
        t = Termin._new124("ЦЕНТНЕР", uu)
        t.add_variant("CENTNER", False)
        t.add_variant("QUINTAL", False)
        t.add_abridge("Ц.")
        UnitsHelper.TERMINS.add(t)
        uu = Unit._new1821("т", "t", "тонна", "tonne", u, 1000000, MeasureKind.WEIGHT)
        UnitsHelper.UNITS.append(uu)
        t = Termin._new124("ТОННА", uu)
        t.add_variant("TONNE", False)
        t.add_variant("TON", False)
        t.add_abridge("Т.")
        t.add_abridge("T.")
        UnitsHelper.TERMINS.add(t)
        for f in [UnitsFactors.KILO, UnitsFactors.MEGA]: 
            UnitsHelper.__add_factor(f, uu, "Т", "T", "ТОННА;ТОННЫЙ", "ТОННА;ТОННИЙ", "TONNE;TON")
        u = Unit._new1815("л", "l", "литр", "liter", MeasureKind.VOLUME)
        u.keywords.extend(["ОБЪЕМ", "ЕМКОСТЬ", "ВМЕСТИМОСЬ", "ОБСЯГ", "ЄМНІСТЬ", "МІСТКІСТЬ", "VOLUME", "CAPACITY"])
        UnitsHelper.UNITS.append(u)
        t = Termin._new124("ЛИТР", u)
        t.add_abridge("Л.")
        t.add_abridge("L.")
        ...
        uu = Unit._new1821("галлон", "gallon", "галлон", "gallon", u, 4.5461, MeasureKind.VOLUME)
        UnitsHelper.UNITS.append(uu)
        t = Termin._new124("ГАЛЛОН", u)
        t.add_variant("ГАЛОН", False)
        t.add_variant("GALLON", False)
        t.add_abridge("ГАЛ")
        UnitsHelper.TERMINS.add(t)
        uu = Unit._new1821("баррель", "bbls", "баррель нефти", "barrel", u, 158.987, MeasureKind.VOLUME)
        UnitsHelper.UNITS.append(uu)
        t = Termin._new124("БАРРЕЛЬ", uu)
        t.add_abridge("BBLS")
        t.add_variant("БАРРЕЛЬ НЕФТИ", False)
        t.add_variant("BARRREL", False)
        UnitsHelper.TERMINS.add(t)
        u = Unit._new1815("сек", "sec", "секунда", "second", MeasureKind.TIME)
        UnitsHelper.USEC = u
        u.keywords.extend(["ВРЕМЯ", "ПРОДОЛЖИТЕЛЬНОСТЬ", "ЗАДЕРЖКА", "ДЛИТЕЛЬНОСТЬ", "ДОЛГОТА", "TIME", "DURATION", "DELAY", "ЧАС", "ТРИВАЛІСТЬ", "ЗАТРИМКА"])
        UnitsHelper.UNITS.append(u)
        t = Termin._new124("СЕКУНДА", u)
        t.add_abridge("С.")
        t.add_abridge("C.")
        t.add_abridge("СЕК")
        t.add_abridge("СЕК")
        ...
        uu = Unit._new1815("мин", "min", "минута", "minute", MeasureKind.TIME)
        UnitsHelper.UMINUTE = uu
        uu.base_unit = u
        uu.base_multiplier = (60)
        UnitsHelper.UNITS.append(uu)
        t = Termin._new124("МИНУТА", uu)
        t.add_abridge("МИН.")
        t.add_abridge("MIN.")
        ...
        UnitsHelper.TERMINS.add(t)
        u = uu
        uu = Unit._new1821("ч", "h", "час", "hour", u, 60, MeasureKind.TIME)
        UnitsHelper.UHOUR = uu
        UnitsHelper.UNITS.append(uu)
        t = Termin._new124("ЧАС", uu)
        t.add_abridge("Ч.")
        t.add_abridge("H.")
        ...
        UnitsHelper.TERMINS.add(t)
        uu = Unit._new1821("нед", "week", "неделя", "week", u, 7, MeasureKind.TIME)
        UnitsHelper.UNITS.append(uu)
        t = Termin._new124("НЕДЕЛЯ", uu)
        t.add_abridge("НЕД")
        t.add_variant("WEEK", False)
        t.add_variant("ТИЖДЕНЬ", False)
        UnitsHelper.TERMINS.add(t)
        uu = Unit._new1821("мес", "mon", "месяц", "month", u, 30, MeasureKind.TIME)
        UnitsHelper.UNITS.append(uu)
        t = Termin._new124("МЕСЯЦ", uu)
        t.add_abridge("МЕС")
        t.add_abridge("MON")
        t.add_variant("MONTH", False)
        t.add_variant("МІСЯЦЬ", False)
        UnitsHelper.TERMINS.add(t)
        uu = Unit._new1821("квартал", "quarter", "квартал", "quarter", u, 91, MeasureKind.TIME)
        UnitsHelper.UNITS.append(uu)
        t = Termin._new124("КВАРТАЛ", uu)
        t.add_abridge("QUARTER")
        UnitsHelper.TERMINS.add(t)
        uu = Unit._new1821("г", "year", "год", "year", u, 365, MeasureKind.TIME)
        UnitsHelper.UNITS.append(uu)
        t = Termin._new124("ГОД", uu)
        t.add_abridge("Г.")
        t.add_abridge("ГД")
        ...
        UnitsHelper.TERMINS.add(t)
        uu = Unit._new1821("декада", "decade", "декада", "decade", u, 365 * 10, MeasureKind.TIME)
        UnitsHelper.UNITS.append(uu)
        t = Termin._new124("ДЕКАДА", uu)
        t.add_variant("DECADE", False)
        t.add_variant("ДЕСЯТИЛЕТИЕ", False)
        t.add_variant("ДЕСЯТИЛІТТЯ", False)
        UnitsHelper.TERMINS.add(t)
        u = Unit("рад", "rad", "рад", "rad")
        UnitsHelper.UNITS.append(u)
        u.keywords.extend(["ДОЗА", "ИЗЛУЧЕНИЕ", "ОБЛУЧЕНИЕ", "ИЗЛУЧАТЬ"])
        t = Termin._new124("РАД", u)
        t.add_variant("RAD", False)
        UnitsHelper.TERMINS.add(t)
        for f in [UnitsFactors.KILO, UnitsFactors.MEGA]: 
            UnitsHelper.__add_factor(f, u, "рад", "rad", "рад", "рад", "rad")
        u = Unit("рад", "rad", "радиан", "radian")
        UnitsHelper.UNITS.append(u)
        u.keywords.extend(["УГОЛ", "ВРАЩЕНИЕ"])
        t = Termin._new124("РАДИАН", u)
        t.add_variant("РАД", False)
        t.add_variant("RADIAN", False)
        t.add_variant("RAD", False)
        UnitsHelper.TERMINS.add(t)
        UnitsHelper.UGRADUS = Unit("°", "°", "градус", "degree")
        UnitsHelper.UGRADUS.keywords.extend(["ТЕМПЕРАТУРА", "ШИРОТА", "ДОЛГОТА", "АЗИМУТ", "НАКЛОН", "ДИФФЕРЕНТ", "TEMPERATURE", "LATITUDE", "LONGITUDE", "AZIMUTH"])
        UnitsHelper.UNITS.append(UnitsHelper.UGRADUS)
        t = Termin._new124("ГРАДУС", UnitsHelper.UGRADUS)
        t.add_variant("DEGREE", False)
        UnitsHelper.TERMINS.add(t)
        UnitsHelper.UGRADUSC = Unit._new1815("°C", "°C", "градус Цельсия", "celsius degree", MeasureKind.TEMPERATURE)
        ...
        t = Termin._new124("ГРАДУС ЦЕЛЬСИЯ", UnitsHelper.UGRADUSC)
        t.add_variant("ГРАДУС ПО ЦЕЛЬСИЮ", False)
        t.add_variant("CELSIUS DEGREE", False)
        UnitsHelper.TERMINS.add(t)
        UnitsHelper.UGRADUSF = Unit._new1815("°F", "°F", "градус Фаренгейта", "Fahrenheit degree", MeasureKind.TEMPERATURE)
        ...
        t = Termin._new124("ГРАДУС ФАРЕНГЕЙТА", UnitsHelper.UGRADUSF)
        t.add_variant("ГРАДУС ПО ФАРЕНГЕЙТУ", False)
        t.add_variant("FAHRENHEIT DEGREE", False)
        UnitsHelper.TERMINS.add(t)
        UnitsHelper.UGRADUSK = Unit._new1815("°К", "°K", "градус Кельвина", "Kelvin degree", MeasureKind.TEMPERATURE)
        UnitsHelper.UGRADUSK.keywords = UnitsHelper.UGRADUSC.keywords
        UnitsHelper.UGRADUSK.psevdo.append(UnitsHelper.UGRADUSF)
        UnitsHelper.UNITS.append(UnitsHelper.UGRADUSK)
        t = Termin._new124("ГРАДУС КЕЛЬВИНА", UnitsHelper.UGRADUSK)
        t.add_variant("ГРАДУС ПО КЕЛЬВИНУ", False)
        t.add_variant("KELVIN DEGREE", False)
        UnitsHelper.TERMINS.add(t)
        UnitsHelper.UPERCENT = Unit._new1815("%", "%", "процент", "percent", MeasureKind.PERCENT)
        UnitsHelper.UNITS.append(UnitsHelper.UPERCENT)
        t = Termin._new124("ПРОЦЕНТ", UnitsHelper.UPERCENT)
        ...
        t = Termin._new124("ОБЪЕМНЫЙ ПРОЦЕНТ", UnitsHelper.UALCO)
        ...
        uu = Unit._new1874("л.с.", "hp", "лошадиная сила", "horsepower", u, 735.49875)
        UnitsHelper.UNITS.append(uu)
        t = Termin._new124("ЛОШАДИНАЯ СИЛА", uu)
        t.add_abridge("Л.С.")
        t.add_abridge("ЛОШ.С.")
        ...
        u = Unit("эрг", "erg", "эрг", "erg")
        u.keywords.extend(["РАБОТА", "ЭНЕРГИЯ"])
        UnitsHelper.UNITS.append(u)
        t = Termin._new124("ЭРГ", u)
        t.add_variant("ERG", False)
        UnitsHelper.TERMINS.add(t)
        for f in [UnitsFactors.KILO, UnitsFactors.MEGA, UnitsFactors.GIGA, UnitsFactors.TERA, UnitsFactors.MILLI]: 
            UnitsHelper.__add_factor(f, u, "эрг", "erg", "ЭРГ", "ЭРГ", "ERG")
        ...
        UnitsHelper.UNITS.append(uu)
        t = Termin._new124("БРИТАНСКАЯ ТЕРМИНАЛЬНАЯ ЕДИНИЦА", uu)
        ...
        UnitsHelper.UNITS.append(u)
        t = Termin._new124("ОМ", UnitsHelper.UOM)
        ...
        UnitsHelper.UNITS.append(uu)
        t = Termin._new124("ВОЛЬТ-АМПЕР", uu)
        t.add_abridge("BA")
        t.add_abridge("BA")
        ...
        uu = Unit._new1874("бар", "bar", "бар", "bar", u, 100000)
        UnitsHelper.UNITS.append(uu)
        t = Termin._new124("БАР", uu)
        t.add_variant("BAR", False)
        UnitsHelper.TERMINS.add(t)
        uu = Unit._new1874("мм.рт.ст.", "mm Hg", "миллиметр ртутного столба", "millimeter of mercury", u, 133.332)
        UnitsHelper.UNITS.append(uu)
        t = Termin._new124("МИЛЛИМЕТР РТУТНОГО СТОЛБА", uu)
        ...
        UnitsHelper.TERMINS.add(t)
        uu = Unit._new1874("кгс", "kgf", "килограмм-сила", "kilogram-force", u, 1000)
        UnitsHelper.UNITS.append(uu)
        t = Termin._new124("КИЛОГРАММ СИЛА", uu)
        t.add_abridge("КГС")
        ...
        UnitsHelper.UNITS.append(u)
        t = Termin._new124("DOTS PER INCH", u)
        t.add_variant("DPI", False)
        UnitsHelper.TERMINS.add(t)
        u = Unit._new1815("IP", "IP", "IP", "IP", MeasureKind.IP)
        u.keywords.extend(["ЗАЩИТА", "КЛАСС ЗАЩИТЫ", "PROTECTION", "PROTACTION RATING"])
        UnitsHelper.UNITS.append(u)
        t = Termin._new124("IP", u)
        UnitsHelper.TERMINS.add(t)
        u = Unit._new1815("кгU", "kgU", "килограмм урана", "kgU", MeasureKind.WEIGHT)
        UnitsHelper.UNITS.append(u)
        t = Termin._new124("КГU", u)
        t.add_variant("КГ U", False)
        t.add_variant("КИЛОГРАММ УРАНА", False)
        t.add_variant("KGU", False)
        t.add_variant("KG U", False)
        UnitsHelper.TERMINS.add(t)
        u = Unit._new1815("кг", "kgZr", "килограмм циркония", "kgZr", MeasureKind.WEIGHT)
        UnitsHelper.UNITS.append(u)
        t = Termin._new124("КГZR", u)
    ...
    def __add_factor(f : 'UnitsFactors', u0 : 'Unit', abbr_cyr : str, abbr_lat : str, names_ru : str, names_ua : str, names_en : str) -> 'Unit':
        ...
        if (swichVal == UnitsFactors.CENTI): 
            ...
            pref_en = "TERA"
            mult = (1000000000000)
        u = Unit._new1923(pref_cyr.lower() + u0.name_cyr, pref_lat.lower() + u0.name_lat, pref_ru.lower() + u0.fullname_cyr, pref_en.lower() + u0.fullname_lat, f, mult, u0, u0.kind, u0.keywords)
        ...
        t = Termin._new124(pref_ru + nams[0], u)
        i = 1
        while i < len(nams): 

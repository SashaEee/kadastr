...
class MeasureAnalyzer(Analyzer):
    ...
    def images(self) -> typing.List[tuple]:
        res = dict()
        res[MeasureMeta.IMAGE_ID] = ResourceHelper2.get_bytes("measure.png")
        res[UnitMeta.IMAGE_ID] = ResourceHelper2.get_bytes("munit.png")
        return res
            ...
            for r in kit.ontology.items: 
                ...
                for s in uu.slots: 
                    if (s.type_name == UnitReferent.ATTR_NAME or s.type_name == UnitReferent.ATTR_FULLNAME): 
                        addunits.add(Termin._new124(Utils.asObjectOrNull(s.value, str), uu))
        t = kit.first_token
        first_pass3544 = True
        while True:

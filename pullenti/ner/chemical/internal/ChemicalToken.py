...
class ChemicalToken(MetaToken):
    ...
    def __try_parse(t : 'Token', prev : 'ChemicalToken', lev : int) -> 'ChemicalToken':
        ...
        if (tt.is_char_of("[(")): 
            ...
            if (ee is None or not ee.is_char_of("])")): 
                return None
            ct = ChemicalToken._new808(t, ee, li)
            if (tt.is_char('(')): 
                ct.bracket = '('
            else: 
                ct.bracket = '['
        ...
        if (tt.chars.is_letter and not tt.chars.is_all_lower): 
            ...
            while i < len(str0_): 
                ...
                if (((i + 1) < len(str0_)) and str.islower(str0_[i + 1])): 
                    ...
                    i += 1
                cu = None
                wrapcu809 = RefOutArgWrapper(None)
                inoutres810 = Utils.tryGetValue(ChemicalToken.__m_units_by_mnem, mnem, wrapcu809)
                cu = wrapcu809.value
                if (not inoutres810): 
                    break
                if (cus is None): 
            ...
            if (i >= len(str0_) and cus is not None): 
                ct = ChemicalToken._new811(t, t, cus)
                ct.__add_num()
    ...
    def initialize() -> None:
        ...
        for p in Utils.splitString(all0_.upper(), ';', False): 
            ...
            ChemicalToken.__m_termins.add(Termin._new124(ci.name_cyr, ci))
    ...
    @staticmethod
    def _new808(_arg1 : 'Token', _arg2 : 'Token', _arg3 : typing.List['ChemicalToken']) -> 'ChemicalToken':
        res = ChemicalToken(_arg1, _arg2)
        res.subtokens = _arg3
        return res
    
    @staticmethod
    def _new811(_arg1 : 'Token', _arg2 : 'Token', _arg3 : typing.List['ChemicalUnit']) -> 'ChemicalToken':
        res = ChemicalToken(_arg1, _arg2)
        res.items = _arg3
        return res
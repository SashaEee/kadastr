...
class ServerService:
    ...
    def create_result(dat : bytearray) -> 'AnalysisResult':
        ...
        with MemoryStream(dat) as mem2: 
            ...
            res.base_language = MorphLang._new330(i)
            i = SerializerHelper.deserialize_int(mem2)
            while i > 0: 
    ...
    @staticmethod
    def internal_proc(stream : Stream, error_path : str=None) -> bytearray:
        ...
        lang = None
        if (parts[0] != "0"): 
            lang = MorphLang._new330(int(parts[0]))
        with ProcessorService.create_empty_processor() as proc: 
            ...
            temp_file = None
            try: 
                if (error_path is not None and not Utils.isNullOrEmpty(txt)): 
                    temp_file = pathlib.PurePath(error_path).joinpath(str(uuid.uuid4()) + ".txt")
                    pathlib.Path(temp_file).write_bytes(txt.encode("UTF-8", 'ignore'))
                ar = proc.process(SourceOfAnalysis(txt), None, lang)
            except Exception as ex: 
                ServerService.ERROR_TEXT = txt

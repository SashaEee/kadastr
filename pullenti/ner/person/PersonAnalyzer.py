...
class PersonAnalyzer(Analyzer):
                            ...
                            if (rt.referent is None): 
                                    ...
                                    if (isinstance(s.value, PersonAttrToken)): 
                                        ...
                                        for ss in pat.prop_ref.slots: 
                                            if (ss.type_name == PersonPropertyReferent.ATTR_REF and (isinstance(ss.value, ReferentToken))): 
                                                ...
                                                rr = ReferentToken._new1225(rt1.referent, rt1.begin_token, rt1.end_token, rt1.morph)
                                                kit.embed_token(rr)
                                        ...
                                        if (rt0.referent is not None): 
                                            for s1 in rt0.referent.slots: 
                                                if (isinstance(s1.value, PersonAttrToken)): 
                                                    ...
                                                    rt2 = ReferentToken._new1225(pat.prop_ref, pat.begin_token, pat.end_token, pat.morph)
                                                    kit.embed_token(rt2)
                                ...
                                for p in pats: 
                                    if (p.prop_ref is not None): 
                                        rr = ReferentToken._new1225(p.prop_ref, p.begin_token, p.end_token, p.morph)
                                        kit.embed_token(rr)
                                        if (rr.begin_token == rt.begin_token): 
                                            rt.begin_token = rr
                                        if (rr.end_token == rt.end_token): 
        ...
        for r in ad.referents: 
            ...
            for s in p.slots: 
                if (s.type_name == PersonReferent.ATTR_ATTR and (isinstance(s.value, PersonPropertyReferent))): 
                    ...
                    li = [ ]
                    wrapli2906 = RefOutArgWrapper(None)
                    inoutres2907 = Utils.tryGetValue(props, pr, wrapli2906)
                    li = wrapli2906.value
                    if (not inoutres2907): 
                        li = list()
                        props[pr] = li
        ...
        while True:
            ...
            if (len(pers) == 1): 
                ...
                if (tt is not None and ((tt.is_char('_') or tt.is_newline_before or tt.is_table_control_char))): 
                    ...
                    rt1 = ReferentToken._new1225(ppr, pat.begin_token, pat.end_token, pat.morph)
                    kit.embed_token(rt1)
                    rt2 = ReferentToken._new1225(pers[0], rt1, rt1, pat.morph)
                    kit.embed_token(rt2)
                    t = (rt2)
                    continue
            if (pat.prop_ref is not None): 
                if (pat.can_be_independent_property or len(pers) > 0): 
                    rt = ReferentToken._new1225(ad.register_referent(pat.prop_ref), pat.begin_token, pat.end_token, pat.morph)
                    kit.embed_token(rt)
                    t = (rt)
                    continue
            t = pat.end_token
    ...
    def process_referent_stat(begin : 'Token', param : str=None) -> 'ReferentToken':
        ...
        if (pat is None or pat.prop_ref is None): 
            return None
        rt = ReferentToken._new1225(pat.prop_ref, pat.begin_token, pat.end_token, pat.morph)
        rt.data = (ad)
        return rt
    ...
    def __try_attach_persons(self, t : 'Token', step : int) -> typing.List['ReferentToken']:
        ...
        if (names is not None): 
            for n in names: 
                pers = PersonReferent()
                bi = MorphBaseInfo._new2912(MorphNumber.SINGULAR, t.kit.base_language)
                bi.class0_ = MorphClass._new2871(True)
    ...
    def _try_attach_person(t : 'Token', for_ext_ontos : bool, step : int, for_attribute : bool=False) -> 'ReferentToken':
        ...
        if ((res is None and t.is_newline_before and (t.begin_char < 1000)) and step == 0): 
            res = PersonAnalyzer.__try_attach_hello(t)
        ad.overflow_level -= 1
        return res
    
    @staticmethod
    def __try_attach_hello(t : 'Token') -> 'ReferentToken':
        from pullenti.ner.person.internal.PersonItemToken import PersonItemToken
        ...
        while True:
            ...
            if (t.length_char == 1): 
                continue
            pits = PersonItemToken.try_attach_list(t, PersonItemToken.ParseAttr.NOMINATIVECASE, 3)
            if (pits is None or len(pits) != 2): 
                break
            if (pits[0].firstname is None or pits[1].middlename is None): 
                break
            mal = 0
            fem = 0
            for v in pits[0].firstname.vars0_: 
                if (v.gender == MorphGender.FEMINIE): 
                    fem += 1
                elif (v.gender == MorphGender.MASCULINE): 
                    mal += 1
            for v in pits[1].middlename.vars0_: 
                if (v.gender == MorphGender.FEMINIE): 
                    fem += 1
                elif (v.gender == MorphGender.MASCULINE): 
                    mal += 1
            t = tok.begin_token.previous
            first_pass3629 = True
            while True:
                if first_pass3629: first_pass3629 = False
                else: t = t.previous
                if (not (t is not None)): break
                pers0 = Utils.asObjectOrNull(t.get_referent(), PersonReferent)
                if (pers0 is None): 
                    continue
                nam = pers0.get_string_value(PersonReferent.ATTR_FIRSTNAME)
                if (nam is None): 
                    continue
                if (not pits[0].firstname.term.startswith(nam)): 
                    continue
                sec = pers0.get_string_value(PersonReferent.ATTR_MIDDLENAME)
                if (sec is None): 
                    continue
                if (not pits[1].middlename.term.startswith(sec)): 
                    continue
                pers = PersonReferent()
                pers.add_slot(PersonReferent.ATTR_FIRSTNAME, pits[0].firstname.term, True, 0)
                pers.add_slot(PersonReferent.ATTR_MIDDLENAME, pits[1].middlename.term, True, 0)
                for fam in pers0.get_string_values(PersonReferent.ATTR_LASTNAME): 
                    pers.add_slot(PersonReferent.ATTR_LASTNAME, fam, False, 0)
                if (mal > fem): 
                    pers.is_male = True
                elif (mal < fem): 
                    pers.is_female = True
                return ReferentToken(pers, pits[0].begin_token, pits[1].end_token)
            cou = 100
            t = tok.begin_token.previous
            first_pass3630 = True
            while True:
                ...
                if (not (t is not None and cou > 0)): break
                if (t.chars.is_all_lower): 
                    continue
                if (not t.is_newline_before): 
                    continue
                pits2 = PersonItemToken.try_attach_list(t, PersonItemToken.ParseAttr.NO, 4)
                if (pits2 is None or len(pits2) != 3): 
                    continue
                if (not pits2[2].is_newline_after): 
                    continue
                if (pits2[0].typ != PersonItemToken.ItemType.VALUE or pits2[1].typ != PersonItemToken.ItemType.INITIAL or pits2[2].typ != PersonItemToken.ItemType.INITIAL): 
                    continue
                if (not pits[0].value.startswith(pits2[1].value) or not pits[1].value.startswith(pits2[2].value)): 
                    continue
                pers = PersonReferent()
                pers.add_slot(PersonReferent.ATTR_FIRSTNAME, pits[0].firstname.term, True, 0)
                pers.add_slot(PersonReferent.ATTR_MIDDLENAME, pits[1].middlename.term, True, 0)
                if (mal > fem): 
                    ...
                    pers.is_female = True
                if (pits2[0].lastname is not None): 
                    for v in pits2[0].lastname.vars0_: 
                        if (v.case_.is_instrumental): 
                            pers.add_slot(PersonReferent.ATTR_LASTNAME, v.value, True, 0)
                            break
                    if (pers.find_slot(PersonReferent.ATTR_LASTNAME, None, True) is None): 
                        for v in pits2[0].lastname.vars0_: 
                            if (((mal > fem and v.gender == MorphGender.MASCULINE)) or (((mal < fem) and v.gender == MorphGender.FEMINIE))): 
                                pers.add_slot(PersonReferent.ATTR_LASTNAME, v.value, True, 0)
                                break
                if (pers.find_slot(PersonReferent.ATTR_LASTNAME, None, True) is None): 
                    pers.add_slot(PersonReferent.ATTR_LASTNAME, pits2[0].value, True, 0)
                return ReferentToken(pers, pits[0].begin_token, pits[1].end_token)
            break
        return None
    ...
    def __try_attach_person_int(t : 'Token', for_ext_ontos : bool, step : int, for_attribute : bool=False) -> 'ReferentToken':
        ...
        while True:
            ...
            if ((a is None and t.is_value("НА", None) and t.next0_ is not None) and t.next0_.is_value("ИМЯ", None)): 
                a = PersonAttrToken._new2685(t, t.next0_, MorphCollection._new2689(MorphCase.GENITIVE))
                is_genitive = True
            if (a is None): 
                break
        ...
        if (attrs is not None): 
            ...
            while t is not None:
                if (t.is_table_control_char): 
                    t = t.next0_
                elif (t.is_char('[') and t.next0_ is not None and ((t.next0_.is_value3("МЕСТО", "ДЛЯ", "ПЕЧАТИ") or t.next0_.is_value3("МЕСТО", "ДЛЯ", "ШТАМПА")))): 
        ...
        for k in range(2):
            ...
            if (pits is not None and len(pits) == 1 and pits[0].firstname is not None): 
                if (pits[0].end_token.next0_ is not None and pits[0].end_token.next0_.is_and and (isinstance(pits[0].end_token.next0_.next0_, ReferentToken))): 
                    ...
                    if (pr is not None): 
                        ...
                        pers = PersonReferent()
                        bi = MorphBaseInfo._new2918(v.gender, MorphNumber.SINGULAR, pits[0].kit.base_language)
                        bi.class0_ = MorphClass._new2871(True)
                ...
                if ((attr is not None and attr.prop_ref is not None and attr.prop_ref.kind == PersonPropertyKind.KIN) and attr.gender != MorphGender.UNDEFINED): 
                    ...
                    if (pr is not None): 
                        pers = PersonReferent()
                        bi = MorphBaseInfo._new2921(MorphNumber.SINGULAR, attr.gender, attr.kit.base_language)
                        bi.class0_ = MorphClass._new2871(True)
                        for s in pr.slots: 
                            if (s.type_name == PersonReferent.ATTR_LASTNAME): 
                                sur = Utils.asObjectOrNull(s.value, str)
                    ...
                    if (pits[1].lastname is not None and pits[1].middlename is None): 
                        ...
                        if (pits1 is not None and pits1[0].lastname is not None): 
                            ...
                            if ((pli11 is not None and len(pli11) > 0 and pli11[0].coef > 1) and pli11[0].end_token == pli0[0].end_token): 
                                if (len(pits1) == 3 and pits1[1].begin_token.is_value("РОМАН", None) and pits1[2].begin_token.is_value("АБРАМОВИЧ", None)): 
                                    pass
                                else: 
                                    pli0 = pli11
            ...
            if (not for_ext_ontos and ((attrs is None or attrs[len(attrs) - 1].typ == PersonAttrTerminType.POSITION))): 
                if ((len(pits) == 4 and pits[0].firstname is not None and pits[1].firstname is None) and pits[2].firstname is not None and ((pits[3].firstname is None or pits[3].lastname is not None))): 
                    pass
            ...
            if (pli is not None and len(pli) > 0): 
                ...
                best = pli[0]
                min_coef = 2
                if (best.coef >= 1 and (best.coef < min_coef) and attrs is None): 
                    tt = t.previous
                    while tt is not None: 
                        if (tt.length_char == 1): 
                            pass
                        else: 
                            if (tt.is_value("ИСПОЛНИТЕЛЬ", None) or tt.is_value("ИСП", None)): 
                                best.coef += (1)
                            break
                        tt = tt.previous
        ...
        if (attrs is not None): 
            attr = attrs[len(attrs) - 1]
            if (attr.can_be_single_person and attr.prop_ref is not None): 
                return ReferentToken._new1225(attr.prop_ref, attr.begin_token, attr.end_token, attr.morph)
        return None

...
class PersonItemToken(MetaToken):
    ...
    class MorphPersonItemVariant(MorphBaseInfo):
        ...
        @staticmethod
        def _new2877(_arg1 : str, _arg2 : 'MorphBaseInfo', _arg3 : bool, _arg4 : str) -> 'MorphPersonItemVariant':
    ...
    class MorphPersonItem:
        ...
        @staticmethod
        def _new2819(_arg1 : bool) -> 'MorphPersonItem':
            res = PersonItemToken.MorphPersonItem()
            res.is_has_std_postfix = _arg1
            return res
        
        @staticmethod
        def _new2827(_arg1 : str) -> 'MorphPersonItem':
            res = PersonItemToken.MorphPersonItem()
            res.term = _arg1
            return res
        
        @staticmethod
        def _new2830(_arg1 : str, _arg2 : bool) -> 'MorphPersonItem':
            res = PersonItemToken.MorphPersonItem()
            res.term = _arg1
        ...
        @staticmethod
        def _new2865(_arg1 : bool) -> 'MorphPersonItem':
            res = PersonItemToken.MorphPersonItem()
    ...
    def __str__(self) -> str:
        res = io.StringIO()
        if (self.fio_pref > 0): 
            print("{0}: ".format(("ФАМИЛИЯ" if self.fio_pref == 1 else ("ИМЯ" if self.fio_pref == 2 else "ОТЧЕСТВО"))), end="", file=res, flush=True)
    ...
    def __add_postfix_info(self, postfix : str, gen : 'MorphGender') -> None:
        ...
        if (postfix.startswith("СТАРШ") or postfix.startswith("МЛАДШ")): 
            return
        if (self.firstname is not None): 
            ...
            self.firstname = PersonItemToken.MorphPersonItem._new2819(True)
            self.firstname.vars0_.append(PersonItemToken.MorphPersonItemVariant(self.value, MorphBaseInfo._new2820(gen), False))
    ...
    def try_attach_latin(t : 'Token') -> 'PersonItemToken':
        ...
        if (tt.term == "JR" or tt.term == "JNR" or tt.term == "JUNIOR"): 
            ...
            if (tt.next0_ is not None and tt.next0_.is_char('.')): 
                t1 = tt.next0_
            return PersonItemToken._new2821(tt, t1, PersonItemToken.ItemType.SUFFIX, "JUNIOR")
        if ((tt.term == "SR" or tt.term == "SNR" or tt.term == "SENIOR") or tt.term == "FITZ" or tt.term == "FILS"): 
            t1 = tt
            if (tt.next0_ is not None and tt.next0_.is_char('.')): 
                t1 = tt.next0_
            return PersonItemToken._new2821(tt, t1, PersonItemToken.ItemType.SUFFIX, "SENIOR")
        ...
        if (not initials and len(tt.term) == 2 and tt.chars.is_capital_upper): 
            if (not LanguageHelper.is_latin_vowel(tt.term[0]) and not LanguageHelper.is_latin_vowel(tt.term[1])): 
                initials = True
        if (initials): 
            rii = PersonItemToken._new2823(tt, tt, PersonItemToken.ItemType.INITIAL, tt.term, tt.chars)
            if (tt.next0_ is not None and tt.next0_.is_char('.')): 
        ...
        if (tt.length_char == 1): 
            ...
            if (tt.next0_.is_char('.')): 
                return PersonItemToken._new2823(tt, tt.next0_, PersonItemToken.ItemType.INITIAL, tt.term, tt.chars)
            ...
            if (not LanguageHelper.is_latin_vowel(tt.term[0]) or tt.whitespaces_after_count != 1): 
                ...
                if (nex is not None and nex.typ == PersonItemToken.ItemType.VALUE): 
                    return PersonItemToken._new2823(tt, tt, PersonItemToken.ItemType.INITIAL, tt.term, tt.chars)
                return None
            if (tt.term == "I"): 
                return None
            return PersonItemToken._new2823(tt, tt, PersonItemToken.ItemType.VALUE, tt.term, tt.chars)
        ...
        if (cla.is_proper_name or ((cla.is_proper and ((tt.morph.gender == MorphGender.MASCULINE or tt.morph.gender == MorphGender.FEMINIE))))): 
            res.firstname = PersonItemToken.MorphPersonItem._new2827(res.value)
        ...
        if (cla.is_proper_surname): 
            res.lastname = PersonItemToken.MorphPersonItem._new2827(res.value)
            for wf in tt.morph.items: 
                if (wf.is_in_dictionary): 
        ...
        if (ots is not None): 
            if (ots[0].termin.ignore_terms_order): 
                return PersonItemToken._new2829(ots[0].begin_token, ots[0].end_token, PersonItemToken.ItemType.REFERENT, Utils.asObjectOrNull(ots[0].item.tag, PersonReferent), ots[0].morph)
            res.lastname = PersonItemToken.MorphPersonItem._new2830(ots[0].termin.canonic_text, True)
            for ot in ots: 
                if (ot.termin is not None): 
                    mi = ot.morph
                    if (ot.termin.gender == MorphGender.MASCULINE or ot.termin.gender == MorphGender.FEMINIE): 
                        mi = MorphBaseInfo._new2820(ot.termin.gender)
        ...
        if (res.value.startswith("MAC")): 
            ...
            res.lastname = PersonItemToken.MorphPersonItem._new2819(True)
            res.lastname.vars0_.append(PersonItemToken.MorphPersonItemVariant(res.value, MorphBaseInfo(), True))
        return res
    ...
    def try_attach(t : 'Token', attrs : 'ParseAttr'=ParseAttr.NO, prev_list : typing.List['PersonItemToken']=None) -> 'PersonItemToken':
        ...
        if (((((isinstance(t, TextToken)) and t.length_char == 2 and ((t.term == "JI" or t.term == "IO"))) and t.chars.is_all_upper and not t.is_whitespace_after) and t.next0_ is not None and t.next0_.is_char('.')) and t.kit.base_language.is_ru): 
            re1 = PersonItemToken._new2821(t, t.next0_, PersonItemToken.ItemType.INITIAL, ("Л" if t.term == "JI" else "Ю"))
        ...
        if ((((((isinstance(t, TextToken)) and t.length_char == 1 and t.term == "J") and t.chars.is_all_upper and not t.is_whitespace_after) and (isinstance(t.next0_, NumberToken)) and t.next0_.value == "1") and t.next0_.typ == NumberSpellingType.DIGIT and t.next0_.next0_ is not None) and t.next0_.next0_.is_char('.')): 
            re1 = PersonItemToken._new2821(t, t.next0_.next0_, PersonItemToken.ItemType.INITIAL, "Л")
            re1.chars.is_cyrillic_letter = True
            re1.chars.is_all_upper = True
            return re1
        if ((((((isinstance(t, TextToken)) and t.length_char == 1 and t.term == "I") and t.chars.is_all_upper and not t.is_whitespace_after) and (isinstance(t.next0_, NumberToken)) and t.next0_.value == "1") and t.next0_.typ == NumberSpellingType.DIGIT and t.next0_.next0_ is not None) and t.next0_.next0_.is_char('.')): 
            if (prev_list is not None and prev_list[0].chars.is_cyrillic_letter): 
                re1 = PersonItemToken._new2821(t, t.next0_.next0_, PersonItemToken.ItemType.INITIAL, "П")
                re1.chars.is_cyrillic_letter = True
                re1.chars.is_all_upper = True
                return re1
        if (t.is_value("ФАМИЛИЯ", None) or t.is_value("ИМЯ", None) or t.is_value("ОТЧЕСТВО", None)): 
            tt1 = t.next0_
            if (tt1 is not None and ((tt1.is_char(':') or tt1.is_hiphen or tt1.is_table_control_char))): 
                tt1 = tt1.next0_
            res1 = PersonItemToken.__try_attach(tt1, Utils.valToEnum((attrs) | (PersonItemToken.ParseAttr.NOMINATIVECASE), PersonItemToken.ParseAttr), prev_list)
            if (res1 is not None): 
                res1.begin_token = t
                if (t.is_value("ФАМИЛИЯ", None)): 
                    res1.middlename = (None)
                    res1.firstname = (None)
                    res1.fio_pref = 1
                    if (res1.lastname is None): 
                        res1.lastname = PersonItemToken.MorphPersonItem()
                        res1.lastname.vars0_.append(PersonItemToken.MorphPersonItemVariant(res1.value, None, True))
                    res1.lastname.is_lastname_has_std_tail = True
                elif (t.is_value("ИМЯ", None)): 
                    if (t.previous is not None and t.previous.is_value("ОТ", None)): 
                        return None
                    res1.fio_pref = 2
                    if (res1.firstname is None): 
                        res1.firstname = res1.lastname
                    res1.lastname = None
                    res1.middlename = res1.lastname
                elif (t.is_value("ОТЧЕСТВО", None)): 
                    res1.fio_pref = 3
                    if (res1.middlename is None): 
                        res1.middlename = res1.lastname
                    res1.lastname = None
                    res1.firstname = res1.lastname
                return res1
        ...
        if (res is not None): 
            if (res.typ == PersonItemToken.ItemType.INITIAL and t.previous is not None and (t.whitespaces_before_count < 3)): 
                if (t.previous.is_value("ГЛАВА", None) or t.previous.is_value("РАЗДЕЛ", None) or t.previous.is_value("СТАТЬЯ", None)): 
                    return None
            return res
        if (t.chars.is_latin_letter and (((attrs) & (PersonItemToken.ParseAttr.CANBELATIN))) != (PersonItemToken.ParseAttr.NO)): 
            ...
            if (ots is not None and (isinstance(t, TextToken))): 
                if (ots[0].termin.ignore_terms_order): 
                    return PersonItemToken._new2829(ots[0].begin_token, ots[0].end_token, PersonItemToken.ItemType.REFERENT, Utils.asObjectOrNull(ots[0].item.tag, PersonReferent), ots[0].morph)
                res = PersonItemToken._new2837(ots[0].begin_token, ots[0].end_token, t.term, ots[0].chars)
                res.lastname = PersonItemToken.MorphPersonItem._new2830(ots[0].termin.canonic_text, True)
                for ot in ots: 
                    if (ot.termin is not None): 
                        mi = ot.morph
        ...
        if (((isinstance(t, NumberToken)) and t.length_char == 1 and (((attrs) & (PersonItemToken.ParseAttr.CANINITIALBEDIGIT))) != (PersonItemToken.ParseAttr.NO)) and t.next0_ is not None and t.next0_.is_char_of(".„")): 
            if (t.value == "1"): 
                return PersonItemToken._new2823(t, t.next0_, PersonItemToken.ItemType.INITIAL, "І", CharsInfo._new2840(True))
            if (t.value == "0"): 
                return PersonItemToken._new2823(t, t.next0_, PersonItemToken.ItemType.INITIAL, "О", CharsInfo._new2840(True))
            if (t.value == "3"): 
                return PersonItemToken._new2823(t, t.next0_, PersonItemToken.ItemType.INITIAL, "З", CharsInfo._new2840(True))
        if ((((isinstance(t, NumberToken)) and t.length_char == 1 and (((attrs) & (PersonItemToken.ParseAttr.CANINITIALBEDIGIT))) != (PersonItemToken.ParseAttr.NO)) and t.next0_ is not None and t.next0_.chars.is_all_lower) and not t.is_whitespace_after and t.next0_.length_char > 2): 
            ...
            if (num == "3" and t.next0_.chars.is_cyrillic_letter): 
                return PersonItemToken._new2823(t, t.next0_, PersonItemToken.ItemType.VALUE, "З" + t.next0_.term, CharsInfo._new2846(True, True))
            if (num == "0" and t.next0_.chars.is_cyrillic_letter): 
                return PersonItemToken._new2823(t, t.next0_, PersonItemToken.ItemType.VALUE, "О" + t.next0_.term, CharsInfo._new2846(True, True))
        if (((((isinstance(t, TextToken)) and t.length_char == 1 and t.chars.is_letter) and t.chars.is_all_upper and (t.whitespaces_after_count < 2)) and (isinstance(t.next0_, TextToken)) and t.next0_.length_char == 1) and t.next0_.chars.is_all_lower): 
            ...
            while tt is not None and tt.end_char <= t1.end_char: 
                ...
                tt = tt.next0_
            res = PersonItemToken._new2821(t, t1, PersonItemToken.ItemType.VALUE, Utils.toStringStringIO(val))
            res.chars = CharsInfo._new2851(True, is_cyr, not is_cyr, True)
            return res
        if ((((attrs) & (PersonItemToken.ParseAttr.MUSTBEITEMALWAYS))) != (PersonItemToken.ParseAttr.NO) and (isinstance(t, TextToken)) and not t.chars.is_all_lower): 
            res = PersonItemToken._new2852(t, t, t.term)
            return res
        if (((t.chars.is_all_upper and (isinstance(t, TextToken)) and t.length_char == 1) and prev_list is not None and len(prev_list) > 0) and (t.whitespaces_before_count < 2) and prev_list[0].chars.is_capital_upper): 
            ...
            if ((last.typ == PersonItemToken.ItemType.VALUE and last.lastname is not None and last.lastname.is_in_dictionary) and len(prev_list) == 1): 
                ...
                ok = True
            if (ok): 
                return PersonItemToken._new2853(t, t, t.term, PersonItemToken.ItemType.INITIAL)
        return None
    ...
    def __try_attach(t : 'Token', attrs : 'ParseAttr', prev_list : typing.List['PersonItemToken']=None) -> 'PersonItemToken':
        ...
        if (tt is None): 
            if (t.chars.is_letter and t.chars.is_capital_upper and (isinstance(t, ReferentToken))): 
                ...
                if (rt.begin_token == rt.end_token and not (isinstance(rt.referent, PersonReferent))): 
                    ...
                    if (res0 is None): 
                        res0 = PersonItemToken._new2854(rt, rt, rt.referent.to_string_ex(True, t.kit.base_language, 0).upper(), rt.chars, rt.morph)
                        res0.lastname = PersonItemToken.MorphPersonItem._new2827(res0.value)
                    else: 
        ...
        if (prev_list is not None and len(prev_list) > 0): 
            prev = prev_list[len(prev_list) - 1]
            if (prev is not None and prev.typ == PersonItemToken.ItemType.VALUE and ((prev.lastname is not None or ((prev.firstname is None and prev.middlename is None))))): 
                if (tt.is_value("СТАРШИЙ", None) or tt.is_value("МЛАДШИЙ", None)): 
                    npt = NounPhraseHelper.try_parse(tt, NounPhraseParseAttr.NO, 0, None)
                    if (npt is not None and npt.end_token != tt): 
                        pass
                    else: 
                        return PersonItemToken._new2821(tt, tt, PersonItemToken.ItemType.SUFFIX, tt.get_normal_case_text(MorphClass.ADJECTIVE, MorphNumber.UNDEFINED, MorphGender.UNDEFINED, False))
                if (tt.term == "СТ" or tt.term == "СТАРШ"): 
                    return PersonItemToken._new2821(tt, tt, PersonItemToken.ItemType.SUFFIX, "СТАРШИЙ")
                if (tt.term == "МЛ" or tt.term == "МЛАД"): 
                    return PersonItemToken._new2821(tt, tt, PersonItemToken.ItemType.SUFFIX, "МЛАДШИЙ")
        ...
        if (tt.length_char == 1 or tt.term == "ДЖ"): 
            if (tt.next0_ is None): 
                return None
            ini = tt.term
            ci = CharsInfo._new2859(tt.chars.value)
            ...
            if (tt.next0_.is_char('.')): 
                return PersonItemToken._new2823(tt, tt.next0_, PersonItemToken.ItemType.INITIAL, ini, ci)
            if ((tt.next0_.is_char_of(",;„") and prev_list is not None and len(prev_list) > 0) and prev_list[len(prev_list) - 1].typ == PersonItemToken.ItemType.INITIAL): 
                return PersonItemToken._new2823(tt, tt, PersonItemToken.ItemType.INITIAL, ini, ci)
        ...
        ots = None
        ad = PersonAnalyzer._get_data(t)
        if ((len(ad.local_ontology.items) < 1000) and len(ad.local_ontology.items) > 0): 
            ots = ad.local_ontology.try_attach(t, PersonReferent.OBJ_TYPENAME, False)
        ...
        if (ots is not None): 
            ...
            if (ots[0].begin_token == ots[0].end_token and mc.is_proper_name and not mc.is_proper_surname): 
                ots = (None)
        if (ots is not None): 
            res = PersonItemToken._new2837(ots[0].begin_token, ots[0].end_token, tt.term, ots[0].chars)
            res.lastname = PersonItemToken.MorphPersonItem._new2865(True)
            ...
            res = PersonItemToken._new2854(t, t, tt.term, tt.chars, tt.morph)
            if (tt.term in PersonItemToken.M_SUR_PREFIXES): 
                if (((tt.is_value("БЕН", None) or tt.is_value("ВАН", None))) and (((attrs) & (PersonItemToken.ParseAttr.ALTVAR))) != (PersonItemToken.ParseAttr.NO) and ((tt.next0_ is None or not tt.next0_.is_hiphen))): 
                    ...
                    if (tt.next0_ is not None): 
                        t1 = tt.next0_
                        if ((isinstance(t1, TextToken)) and ((((t1.is_value("ДЕР", None) or t1.term == "Л" or t1.term == "ЛЬ") or t1.term in PersonItemToken.M_SUR_PREFIXES)))): 
                            res.end_token = t1
                            res.value += t1.term
                            t1 = t1.next0_
        ...
        if ((((attrs) & (PersonItemToken.ParseAttr.MUSTBEITEMALWAYS))) != (PersonItemToken.ParseAttr.NO)): 
            ...
            if (tt.length_char > 6 and tt.term.startswith("ДО")): 
                if (tt.is_value("ДОРОГОЙ", None)): 
                    return None
            if (tt.is_value("ГЛУБОКОУВАЖАЕМЫЙ", None)): 
                return None
        ...
        for wff in tt.morph.items: 
            ...
            if (wf.class0_.is_proper_surname or sur_prefix is not None): 
                if (res.lastname is None): 
                    res.lastname = PersonItemToken.MorphPersonItem._new2827(tt.term)
                if (adj is not None): 
                ...
                if (wf.is_in_dictionary and v.gender == MorphGender.UNDEFINED and wf.gender == MorphGender.UNDEFINED): 
                    ...
                    vv = PersonItemToken.MorphPersonItemVariant(t.term, MorphBaseInfo._new2775(MorphCase.NOMINATIVE, MorphGender.MASCULINE), True)
                    vv.value = t.term
                ...
                if (tt.term.endswith("ИХ") or tt.term.endswith("ЫХ")): 
                    if (res.lastname.vars0_[0].value != tt.term): 
                        res.lastname.vars0_.insert(0, PersonItemToken.MorphPersonItemVariant(tt.term, MorphBaseInfo._new2872(MorphCase.ALL_CASES, Utils.valToEnum((MorphGender.MASCULINE) | (MorphGender.FEMINIE), MorphGender), MorphClass._new2871(True)), True))
            ...
            if (wf.class0_.is_proper_name and wf.number != MorphNumber.PLURAL): 
                ...
                if (ok): 
                    if (res.firstname is None): 
                        res.firstname = PersonItemToken.MorphPersonItem._new2827(tt.term)
            ...
            if (not PersonItemToken.MorphPersonItem.ends_with_std_surname(tt.term)): 
                if (wf.class0_.is_proper_secname): 
                    if (res.middlename is None): 
                        res.middlename = PersonItemToken.MorphPersonItem._new2827(tt.term)
        ...
        if (res.lastname is not None): 
            ...
            if (tt.morph.class0_.is_adjective and not res.lastname.is_in_ontology): 
                ...
                if (not std_end and (tt.whitespaces_after_count < 2)): 
                    if (res.lastname.is_in_dictionary and not res.begin_token.get_morph_class_in_dictionary().is_adjective): 
                        pass
                    else: 
                        npt = NounPhraseHelper.try_parse(tt, NounPhraseParseAttr.NO, 0, None)
                        if (npt is not None and npt.end_token != npt.begin_token): 
                            if ((prev_list is not None and len(prev_list) == 1 and prev_list[0].firstname is not None) and prev_list[0].firstname.is_in_dictionary and tt.whitespaces_before_count == 1): 
                                ...
                                if (nex is not None and ((nex.firstname is not None or nex.middlename is not None))): 
                                    pass
                                else: 
                                    res.lastname = (None)
            ...
            for wf in tt.morph.items: 
                if (not wf.class0_.is_verb): 
                    ...
                    if (wf.case_.is_genitive and wf.number == MorphNumber.PLURAL and tt.term.endswith("ОВ")): 
                        ...
                        res.lastname.vars0_.append(PersonItemToken.MorphPersonItemVariant(tt.term, MorphBaseInfo._new2794(MorphGender.MASCULINE, MorphCase.NOMINATIVE), True))
                        res.lastname.is_lastname_has_std_tail = True
                        break
                ...
                if (g != MorphGender.MASCULINE and g != MorphGender.FEMINIE): 
                    ...
                    for kp in di: 
                        if (fi): 
                            ...
                            mi = MorphBaseInfo._new2820(kp.gender)
                            res.firstname.vars0_.append(PersonItemToken.MorphPersonItemVariant._new2877(kp.name, mi, False, val))
                else: 
                    cou = 0
                    for kp in di: 
                        if (kp.gender == g): 
                            ...
                            if (cou < 2): 
                                ...
                                res.firstname.vars0_.insert(i + 1, PersonItemToken.MorphPersonItemVariant._new2877(kp.name, res.firstname.vars0_[i], False, val))
        ...
        if ((t is not None and t.is_char('.') and pit.typ == PersonItemToken.ItemType.VALUE) and pit.length_char > 3): 
            ...
            if (str.isupper(str0_[0]) and str.isupper(str0_[len(str0_) - 1])): 
                ...
                if (ok): 
                    ...
                    pit2 = PersonItemToken._new2821(t, t, PersonItemToken.ItemType.INITIAL, str0_[len(str0_) - 1:])
                    res.append(pit2)
                    t = t.next0_
        zap = False
        while t is not None: 
                ...
                if (pit1.typ == PersonItemToken.ItemType.INITIAL): 
                    if (pit1.chars.is_cyrillic_letter): 
                        ...
                        if (v != (chr(0))): 
                            pit1.value = "{0}".format(v)
                            ok = True
                            pit1.chars = CharsInfo._new2840(True)
                        elif (pit.typ == PersonItemToken.ItemType.INITIAL): 
                            v = LanguageHelper.get_lat_for_cyr(pit.value[0])
                            if (v != (chr(0))): 
                                pit.value = "{0}".format(v)
                                ok = True
                                pit.chars = CharsInfo._new2880(True)
                                pit = pit1
                elif (pit.typ == PersonItemToken.ItemType.INITIAL): 
        ...
        while True:
            ...
            if (res[i].firstname is not None and res[i].begin_token.is_value("СВЕТА", None)): 
                ...
                if (res[i].lastname is None): 
                    res[i].lastname = PersonItemToken.MorphPersonItem._new2819(True)
    ...
    def try_parse_person(t : 'Token', prev_pers_template : 'FioTemplateType'=FioTemplateType.UNDEFINED, from_input : bool=False) -> 'ReferentToken':
                ...
                if (tt1 is not None and (tt1.whitespaces_before_count < 2)): 
                    ...
                    if (pits0 is not None and pits0[0].typ == PersonItemToken.ItemType.INITIAL): 
                        ...
                        if (str0_ is not None and str0_.startswith(pits0[0].value)): 
                            res = ReferentToken._new2885(rt.referent, t, pits0[0].end_token, FioTemplateType.SURNAMEI)
                    ...
                    if (((((isinstance(tt1, TextToken)) and tt1.length_char == 1 and tt1.chars.is_all_upper) and tt1.chars.is_cyrillic_letter and (isinstance(tt1.next0_, TextToken))) and (tt1.whitespaces_after_count < 2) and tt1.next0_.length_char == 1) and tt1.next0_.chars.is_all_upper and tt1.next0_.chars.is_cyrillic_letter): 
                        ...
                        if (str0_ is not None and str0_.startswith(tt1.term)): 
                            ...
                            if (str2 is None or str2.startswith(tt1.next0_.term)): 
                                res = ReferentToken._new2885(rt.referent, t, tt1.next0_, FioTemplateType.NAMEISURNAME)
                                if (str2 is None): 
        ...
        if (t.get_referent() is not None and t.get_referent().type_name == "ORGANIZATION"): 
            rt = Utils.asObjectOrNull(t, ReferentToken)
            ppp = PersonItemToken.try_parse_person(rt.begin_token, FioTemplateType.UNDEFINED, False)
        ...
        if (pits is not None and prev_pers_template != FioTemplateType.UNDEFINED and pits[0].typ == PersonItemToken.ItemType.VALUE): 
            ...
            if (((isinstance(tt1, TextToken)) and tt1.chars.is_letter and tt1.chars.is_all_upper) and tt1.length_char == 1 and (tt1.whitespaces_before_count < 2)): 
                ii = PersonItemToken._new2823(tt1, tt1, PersonItemToken.ItemType.INITIAL, tt1.term, tt1.chars)
                pits.append(ii)
        ...
        if (pits is not None and len(pits) > 1): 
            ...
            if (pits[0].typ == PersonItemToken.ItemType.VALUE and pits[1].typ == PersonItemToken.ItemType.INITIAL): 
                ...
                elif (from_input and pits[0].firstname is not None): 
                    return None
        ...
        if (pits is not None and len(pits) == 1 and pits[0].typ == PersonItemToken.ItemType.VALUE): 
            ...
            if (((isinstance(tt, TextToken)) and tt.length_char == 2 and tt.chars.is_all_upper) and tt.chars.is_cyrillic_letter): 
                ...
                res = ReferentToken._new2885(pers, t, tt, FioTemplateType.SURNAMEII)
                if (tt.next0_ is not None and tt.next0_.is_char('.')): 
                    tt = tt.next0_
                    res.end_token = tt
                res.data = t.kit.get_analyzer_data_by_analyzer_name(PersonAnalyzer.ANALYZER_NAME)
            ...
            if ((((((isinstance(tt, TextToken)) and (tt.whitespaces_before_count < 2) and tt.length_char == 1) and tt.chars.is_all_upper and tt.chars.is_cyrillic_letter) and (isinstance(tt.next0_, TextToken)) and (tt.whitespaces_after_count < 2)) and tt.next0_.length_char == 1 and tt.next0_.chars.is_all_upper) and tt.next0_.chars.is_cyrillic_letter): 
                ...
                res = ReferentToken._new2885(pers, t, tt.next0_, FioTemplateType.SURNAMEII)
            ...
            if (comma and tt is not None and (tt.whitespaces_before_count < 2)): 
                ...
                if (pits1 is not None and len(pits1) > 0 and pits1[0].typ == PersonItemToken.ItemType.INITIAL): 
                    ...
                    res = ReferentToken._new2885(pers, t, pits1[0].end_token, FioTemplateType.SURNAMEI)
    ...
    @staticmethod
    def _new2821(_arg1 : 'Token', _arg2 : 'Token', _arg3 : 'ItemType', _arg4 : str) -> 'PersonItemToken':
    ...
    @staticmethod
    def _new2823(_arg1 : 'Token', _arg2 : 'Token', _arg3 : 'ItemType', _arg4 : str, _arg5 : 'CharsInfo') -> 'PersonItemToken':
        res = PersonItemToken(_arg1, _arg2)
        res.typ = _arg3
        res.value = _arg4
        res.chars = _arg5
    ...
    @staticmethod
    def _new2829(_arg1 : 'Token', _arg2 : 'Token', _arg3 : 'ItemType', _arg4 : 'PersonReferent', _arg5 : 'MorphCollection') -> 'PersonItemToken':
        res = PersonItemToken(_arg1, _arg2)
        res.typ = _arg3
        res.referent = _arg4
        res.morph = _arg5
    ...
    @staticmethod
    def _new2837(_arg1 : 'Token', _arg2 : 'Token', _arg3 : str, _arg4 : 'CharsInfo') -> 'PersonItemToken':
        res = PersonItemToken(_arg1, _arg2)
        res.value = _arg3
        res.chars = _arg4
        return res
    ...
    @staticmethod
    def _new2852(_arg1 : 'Token', _arg2 : 'Token', _arg3 : str) -> 'PersonItemToken':
        res = PersonItemToken(_arg1, _arg2)
        res.value = _arg3
        return res
    
    @staticmethod
    def _new2853(_arg1 : 'Token', _arg2 : 'Token', _arg3 : str, _arg4 : 'ItemType') -> 'PersonItemToken':
        res = PersonItemToken(_arg1, _arg2)
        res.value = _arg3
        res.typ = _arg4
        return res
    ...
    @staticmethod
    def _new2854(_arg1 : 'Token', _arg2 : 'Token', _arg3 : str, _arg4 : 'CharsInfo', _arg5 : 'MorphCollection') -> 'PersonItemToken':
        res = PersonItemToken(_arg1, _arg2)
        res.value = _arg3
        res.chars = _arg4
        res.morph = _arg5

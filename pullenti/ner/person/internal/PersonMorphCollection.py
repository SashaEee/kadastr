...
class PersonMorphCollection:
    ...
    class PersonMorphVariant:
        ...
        @staticmethod
        def _new2780(_arg1 : str) -> 'PersonMorphVariant':
            res = PersonMorphCollection.PersonMorphVariant()
            res.value = _arg1
            return res
        
        @staticmethod
        def _new2891(_arg1 : str, _arg2 : 'MorphGender', _arg3 : str) -> 'PersonMorphVariant':
            res = PersonMorphCollection.PersonMorphVariant()
            res.value = _arg1
            res.gender = _arg2
    ...
    class SortComparer(object):
        ...
        def compare(self, x : 'PersonMorphVariant', y : 'PersonMorphVariant') -> int:
            if (x.value.find('-') > 0): 
                ...
                return 1
            if (len(x.value) > 4 and len(y.value) > 4 and x.value[0] != y.value[0]): 
                if (x.value[0:0+4] != y.value[0:0+4]): 
                    return 0
    ...
    def correct(self) -> None:
        for it in self.items: 
            ...
            if (it.gender == MorphGender.FEMINIE): 
                if (it.value.endswith("СТАРШИЙ") or it.value.endswith("МЛАДШИЙ")): 
                    it.value = (it.value[0:0+len(it.value) - 2] + "АЯ")
        i = 0
        ...
        if (gen == MorphGender.MASCULINE or gen == MorphGender.FEMINIE): 
            for it in self.items: 
                if (it.value == val and it.gender == gen): 
                    return
            self.items.append(PersonMorphCollection.PersonMorphVariant._new2891(val, gen, shortval))
            if (add_other_gender_var): 
                g0 = (MorphGender.MASCULINE if gen == MorphGender.FEMINIE else MorphGender.FEMINIE)
                v = None
                try: 
                    v = MorphologyService.get_wordform(val, MorphBaseInfo._new549(MorphClass._new2871(True), g0))
                except Exception as ex2894: 
                    pass
                if (v is not None): 
                    self.items.append(PersonMorphCollection.PersonMorphVariant._new2891(v, g0, shortval))
        else: 

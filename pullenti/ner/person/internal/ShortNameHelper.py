...
class ShortNameHelper:
    ...
    class ShortnameVar:
        ...
        @staticmethod
        def _new2899(_arg1 : str, _arg2 : 'MorphGender') -> 'ShortnameVar':
    ...
    def get_names_for_shortname(shortname : str) -> typing.List['ShortnameVar']:
        res = [ ]
        wrapres2897 = RefOutArgWrapper(None)
        inoutres2898 = Utils.tryGetValue(ShortNameHelper.M_SHORTS_NAMES, shortname, wrapres2897)
        res = wrapres2897.value
        if (not inoutres2898): 
            return None
        else: 
            return res
        ...
        if (obj is not None): 
            ...
            while t is not None: 
                if (t.is_newline_before): 
                    ...
                    for s in shos: 
                        li = None
                        wrapli2900 = RefOutArgWrapper(None)
                        inoutres2901 = Utils.tryGetValue(ShortNameHelper.M_SHORTS_NAMES, s, wrapli2900)
                        li = wrapli2900.value
                        if (not inoutres2901): 
                            li = list()
                            ShortNameHelper.M_SHORTS_NAMES[s] = li
                        li.append(ShortNameHelper.ShortnameVar._new2899(nam, g))
                    if (t is None): 
                        break
                    t = t.previous
                t = t.next0_

...
class PersonReferent(Referent):
        ...
        if (g != MorphGender.UNDEFINED): 
            if (not self.is_female and not self.is_male): 
                if (g == MorphGender.MASCULINE): 
                    ...
                    self.is_female = True
            elif (g == MorphGender.MASCULINE): 
                self.is_male = True
            elif (g == MorphGender.FEMINIE): 
                self.is_female = True
    ...
    def create_ontology_item(self) -> 'IntOntologyItem':
        ...
        for a in self.slots: 
            if (a.type_name == PersonReferent.ATTR_IDENTITY): 
                oi.termins.append(Termin._new1344(str(a.value), True))
    ...
    def split_male_and_female(self, male : 'PersonReferent', female : 'PersonReferent') -> bool:
        """ Если получилась и мужчина, и женщина, то расщепить на 2 отдельные сущности
        
        Args:
            male(PersonReferent): 
            female(PersonReferent): 
        
        """
        if (not self.is_male or not self.is_female): 
            return False
        surs = self.get_string_values(PersonReferent.ATTR_LASTNAME)
        if (len(surs) == 0): 
            return False
        male.is_male = True
        female.is_female = True
        if (len(surs) == 1): 
            male.add_slot(PersonReferent.ATTR_LASTNAME, surs[0], False, 0)
            female.add_slot(PersonReferent.ATTR_LASTNAME, surs[0], False, 0)
        elif (len(surs) > 1): 
            if (len(surs[0]) <= len(surs[1])): 
                male.add_slot(PersonReferent.ATTR_LASTNAME, surs[0], False, 0)
                female.add_slot(PersonReferent.ATTR_LASTNAME, surs[1], False, 0)
            else: 
                male.add_slot(PersonReferent.ATTR_LASTNAME, surs[1], False, 0)
                female.add_slot(PersonReferent.ATTR_LASTNAME, surs[0], False, 0)
        nams = self.get_string_values(PersonReferent.ATTR_FIRSTNAME)
        if (len(nams) == 1): 
            male.add_slot(PersonReferent.ATTR_FIRSTNAME, nams[0], False, 0)
            female.add_slot(PersonReferent.ATTR_FIRSTNAME, nams[0], False, 0)
        elif (len(surs) > 1): 
            if (len(nams[0]) <= len(nams[1])): 
                male.add_slot(PersonReferent.ATTR_FIRSTNAME, nams[0], False, 0)
                female.add_slot(PersonReferent.ATTR_FIRSTNAME, nams[1], False, 0)
            else: 
                male.add_slot(PersonReferent.ATTR_FIRSTNAME, nams[1], False, 0)
                female.add_slot(PersonReferent.ATTR_FIRSTNAME, nams[0], False, 0)
        for s in self.slots: 
            if (s.type_name != PersonReferent.ATTR_FIRSTNAME and s.type_name != PersonReferent.ATTR_LASTNAME and s.type_name != PersonReferent.ATTR_SEX): 
                male.add_slot(s.type_name, s.value, False, 0)
                female.add_slot(s.type_name, s.value, False, 0)
        return True
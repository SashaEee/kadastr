...
class PhoneHelper:
    ...
    def initialize() -> None:
        ...
        for line0 in Utils.splitString(str0_, '\n', False): 
            ...
            while i < len(cod): 
                dig = cod[i]
                nn = None
                wrapnn2926 = RefOutArgWrapper(None)
                inoutres2927 = Utils.tryGetValue(tn.children, dig, wrapnn2926)
                nn = wrapnn2926.value
                if (not inoutres2927): 
    ...
    def get_country_prefix(full_number : str) -> str:
        ...
        while i < len(full_number): 
            dig = full_number[i]
            nn = None
            wrapnn2928 = RefOutArgWrapper(None)
            inoutres2929 = Utils.tryGetValue(nod.children, dig, wrapnn2928)
            nn = wrapnn2928.value
            if (not inoutres2929): 
                break

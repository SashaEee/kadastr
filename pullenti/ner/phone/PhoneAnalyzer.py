...
class PhoneAnalyzer(Analyzer):
    ...
    class PhoneAnalizerData(AnalyzerData):
        ...
        def register_referent(self, referent : 'Referent') -> 'Referent':
            ...
            if (len(key) >= 10): 
                key = key[3:]
            ph_li = [ ]
            wrapph_li2957 = RefOutArgWrapper(None)
            inoutres2958 = Utils.tryGetValue(self.__m_phones_hash, key, wrapph_li2957)
            ph_li = wrapph_li2957.value
            if (not inoutres2958): 
                ph_li = list()
    ...
    def images(self) -> typing.List[tuple]:
        res = dict()
        res[MetaPhone.PHONE_IMAGE_ID] = ResourceHelper2.get_bytes("phone.png")
        return res
            ...
            while tt is not None: 
                if (isinstance(tt.get_referent(), PhoneReferent)): 
                    ...
                    if (ttt is None or not ttt.is_char('(')): 
                        break
                    tt = ttt
                elif (tt.is_value("ГОСТ", None) or tt.is_value("ОСТ", None)): 
                    not_phone = True
                    break
        ...
        if (prev_phone is not None and prev_phone._m_template is not None and pli[j].item_type == PhoneItemToken.PhoneItemType.NUMBER): 
            ...
            while True:
                ...
                if (templ0 == prev_phone._m_template): 
                    ...
                    break
            vv = Utils.toStringStringIO(tmp)
            if (vv == "4-2-2" or vv == "2-2-4"): 
                return None

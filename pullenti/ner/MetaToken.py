...
class MetaToken(Token):
    ...
    def __init__(self, begin : 'Token', end : 'Token', kit_ : 'AnalysisKit'=None) -> None:
        ...
        if (begin is None or end is None): 
            return
        if (kit_ is not None and kit_.ignore_begin_end_char_in_metatoken): 
            pass
        elif (self.kit is not None and self.kit.ignore_begin_end_char_in_metatoken): 
            pass
        elif (begin.begin_char > end.begin_char): 
            raise Utils.newException("Begin > End in MetaToken", None)
        self._m_begin_token = begin
        self._m_end_token = end
        if (begin.chars is not None): 
            self.chars = CharsInfo._new2859(begin.chars.value)
        if (begin != end): 
            t = begin.next0_
    ...
    def __str__(self) -> str:
        res = io.StringIO()
        t = self._m_begin_token
        while t is not None and t.end_char <= self._m_end_token.end_char: 
    ...
    @staticmethod
    def _new933(_arg1 : 'Token', _arg2 : 'Token', _arg3 : object) -> 'MetaToken':
        res = MetaToken(_arg1, _arg2)
        res.tag = _arg3
        return res
    
    @staticmethod
    def _new950(_arg1 : 'Token', _arg2 : 'Token', _arg3 : 'MorphCollection') -> 'MetaToken':
        res = MetaToken(_arg1, _arg2)
        res.morph = _arg3
        return res
    
    @staticmethod
    def _new2639(_arg1 : 'Token', _arg2 : 'Token', _arg3 : object, _arg4 : 'MorphCollection') -> 'MetaToken':
        res = MetaToken(_arg1, _arg2)
        res.tag = _arg3
        res.morph = _arg4
        return res
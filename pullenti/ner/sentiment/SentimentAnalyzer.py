...
class SentimentAnalyzer(Analyzer):
    ...
    def images(self) -> typing.List[tuple]:
        res = dict()
        res[MetaSentiment.IMAGE_ID] = ResourceHelper3.get_bytes("neutral.png")
        res[MetaSentiment.IMAGE_ID_GOOD] = ResourceHelper3.get_bytes("good.png")
        res[MetaSentiment.IMAGE_ID_BAD] = ResourceHelper3.get_bytes("bad.png")
        return res
    ...
    def initialize() -> None:
        ...
        try: 
            for i in range(2):
                str0_ = ResourceHelper3.get_string(("Positives.txt" if i == 0 else "Negatives.txt"))
                if (str0_ is None): 
                    continue
                for line0 in Utils.splitString(str0_, '\n', False): 
                    ...
                    SentimentAnalyzer.__m_termins.add(Termin._new124(line, coef))
        except Exception as ex: 
            pass
        for s in ["ОЧЕНЬ", "СИЛЬНО"]: 
            SentimentAnalyzer.__m_termins.add(Termin._new124(s, 0))

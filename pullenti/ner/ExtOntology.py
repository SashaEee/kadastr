...
class ExtOntology:
    ...
    def add(self, ext_id : object, type_name : str, definition_ : str) -> 'ExtOntologyItem':
        ...
        if (rs is None): 
            return None
        self.__m_hash = (None)
        res = ExtOntologyItem._new3115(ext_id, rs[0], type_name)
        if (len(rs) > 1): 
            del rs[0]
            res._refs = rs
    ...
    def add_referent(self, ext_id : object, referent : 'Referent') -> 'ExtOntologyItem':
        ...
        if (referent is None): 
            return None
        self.__m_hash = (None)
        res = ExtOntologyItem._new3115(ext_id, referent, referent.type_name)
        self.items.append(res)
        return res
    
    def __create_referent(self, type_name : str, definition_ : str) -> typing.List['Referent']:
        analyzer = None
        wrapanalyzer3117 = RefOutArgWrapper(None)
        inoutres3118 = Utils.tryGetValue(self.__m_anal_by_type, type_name, wrapanalyzer3117)
        analyzer = wrapanalyzer3117.value
        if (not inoutres3118): 
            return None
    ...
    def refresh(self, item : 'ExtOntologyItem', definition_ : object) -> bool:
        ...
        analyzer = None
        wrapanalyzer3119 = RefOutArgWrapper(None)
        inoutres3120 = Utils.tryGetValue(self.__m_anal_by_type, item.type_name, wrapanalyzer3119)
        analyzer = wrapanalyzer3119.value
        if (not inoutres3120): 
            return False
    ...
    def _get_analyzer_data(self, type_name : str) -> 'AnalyzerData':
        a = None
        wrapa3121 = RefOutArgWrapper(None)
        inoutres3122 = Utils.tryGetValue(self.__m_anal_by_type, type_name, wrapa3121)
        a = wrapa3121.value
        if (not inoutres3122): 
            return None
    ...
    def __init_hash(self) -> None:
        with self.__m_lock: 
            ...
            for it in self.items: 
                if (it.referent is not None): 
                    ont = None
                    wrapont3124 = RefOutArgWrapper(None)
                    inoutres3125 = Utils.tryGetValue(hash0_, it.referent.type_name, wrapont3124)
                    ont = wrapont3124.value
                    if (not inoutres3125): 
                        ont = IntOntologyCollection._new3123(True)
    ...
    def attach_referent(self, r : 'Referent') -> typing.List['ExtOntologyItem']:
        ...
        if (self.__m_hash is None): 
            self.__init_hash()
        onto = None
        wraponto3126 = RefOutArgWrapper(None)
        inoutres3127 = Utils.tryGetValue(self.__m_hash, r.type_name, wraponto3126)
        onto = wraponto3126.value
        if (not inoutres3127): 
            return None
        li = [ ]
    ...
    def attach_token(self, type_name : str, t : 'Token') -> typing.List['IntOntologyToken']:
        ...
        if (self.__m_hash is None): 
            self.__init_hash()
        onto = None
        wraponto3128 = RefOutArgWrapper(None)
        inoutres3129 = Utils.tryGetValue(self.__m_hash, type_name, wraponto3128)
        onto = wraponto3128.value
        if (not inoutres3129): 
            return None

...
class GeoReferent(Referent):
    ...
    ATTR_PROBABLE = "PROBABLE"
    """ Имя атрибута - сомнительный результат """
    ...
    def __is_city(v : str) -> bool:
        if ((((((((((((((("поселок" in v or "селение" in v or "городок" in v) or "село" in v or "деревня" in v) or "станица" in v or "пункт" in v) or "станция" in v or "полустанок" in v) or "аул" in v or "улус" in v) or "кишлак" in v or "наслег" in v) or "хутор" in v or "слобода" in v) or "местечко" in v or "усадьба" in v) or "починок" in v or "заимка" in v) or "аал" in v or "выселки" in v) or "арбан" in v or v == "участок") or "місто" in v or "селище" in v) or "сіло" in v or "станиця" in v) or "станція" in v or "city" in v) or "municipality" in v or "town" in v): 
            return True
        if ("порт" in v): 
            return True
        if ("агрогородок" in v): 
    ...
    def is_probable(self) -> bool:
        """ Результат сомнительный (в плане типа) """
        return self.get_string_value(GeoReferent.ATTR_PROBABLE) == "true"
    @is_probable.setter
    def is_probable(self, value) -> bool:
        if (value): 
            self.add_slot(GeoReferent.ATTR_PROBABLE, "true", True, 0)
        else: 
            self.add_slot(GeoReferent.ATTR_PROBABLE, None, True, 0)
        return value

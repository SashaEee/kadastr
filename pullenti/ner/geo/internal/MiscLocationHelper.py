...
class MiscLocationHelper:
    ...
    @staticmethod
    def is_user_param_no_flats(t : 'Token') -> bool:
        if (t is None): 
            return False
        if (t.kit.sofa.user_params is not None): 
            if ("NOFLATS" in t.kit.sofa.user_params): 
                return True
        return False
    ...
    def check_territory(t : 'Token') -> 'Token':
        ...
        if (tok.termin.canonic_text == "УЧАСТОК"): 
            ...
            if ((ait is not None and not Utils.isNullOrEmpty(ait.value) and (len(ait.value) < 4)) and str.isdigit(ait.value[0])): 
                return None
            if ((tok.begin_token == tok.end_token and tok.length_char == 2 and tok.end_token.next0_ is not None) and tok.end_token.next0_.is_hiphen): 
                return None
        tt2 = tok.end_token
    ...
    def get_geo_referent_by_name(name : str) -> 'GeoReferent':
        from pullenti.ner.geo.internal.TerrItemToken import TerrItemToken
        res = None
        wrapres1378 = RefOutArgWrapper(None)
        inoutres1379 = Utils.tryGetValue(MiscLocationHelper.__m_geo_ref_by_name, name, wrapres1378)
        res = wrapres1378.value
        if (inoutres1379): 
            return res
    ...
    def try_attach_nord_west(t : 'Token') -> 'MetaToken':
        ...
        if (tok is None): 
            return None
        res = MetaToken._new950(t, t, t.morph)
        t1 = None

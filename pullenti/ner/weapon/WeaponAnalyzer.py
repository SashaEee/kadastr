...
class WeaponAnalyzer(Analyzer):
    ...
    def images(self) -> typing.List[tuple]:
        res = dict()
        res[MetaWeapon.IMAGE_ID] = ResourceHelper3.get_bytes("weapon.jpg")
        return res
                        ...
                        if (s.type_name == WeaponReferent.ATTR_MODEL): 
                            ...
                            for k in range(2):
                                if (not str.isdigit(mod[0])): 
                                    li = [ ]
                                    wrapli3112 = RefOutArgWrapper(None)
                                    inoutres3113 = Utils.tryGetValue(objs_by_model, mod, wrapli3112)
                                    li = wrapli3112.value
                                    if (not inoutres3113): 
                                        li = list()

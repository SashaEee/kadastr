...
class OrgOwnershipHelper:
    ...
    def can_be_higher(higher : 'OrganizationReferent', lower : 'OrganizationReferent', robust : bool=False) -> bool:
        ...
        if (hk != OrganizationKind.DEPARTMENT and lk == OrganizationKind.DEPARTMENT): 
            ...
            if (OrgOwnershipHelper.__contains(ltyps, "созыв", None)): 
                return hk == OrganizationKind.GOVENMENT or higher.contains_profile(OrgProfile.STATE)
        ...
        if (lk == OrganizationKind.DEPARTMENT): 
            ...
            if ("дежурная часть" in ltyps): 
                return True
            if (OrgOwnershipHelper.__contains(htyps, "филиал", "філіал")): 
                if (OrgOwnershipHelper.__contains(htyps, "отделение", "відділення") or OrgOwnershipHelper.__contains(ltyps, "отдел", "відділ")): 
                    return True
        if (OrgOwnershipHelper.__contains(ltyps, "управление", "управління") or OrgOwnershipHelper.__contains(ltyps, "отдел", "відділ")): 

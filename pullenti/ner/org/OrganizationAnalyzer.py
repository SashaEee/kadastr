...
class OrganizationAnalyzer(Analyzer):
    ...
    def __init_politic() -> None:
        ...
        for s in ["объединенный", "всероссийский", "общероссийский", "христианский", "независимый", "альтернативный"]: 
            OrganizationAnalyzer.M_POLITIC_PREFS.add(Termin._new2625(s.upper(), s))
        for s in ["политический", "правящий", "оппозиционный", "запрешенный", "террористический", "запрещенный", "экстремистский"]: 
            OrganizationAnalyzer.M_POLITIC_PREFS.add(Termin._new124(s.upper(), s))
        for s in ["активист", "член", "руководство", "лидер", "глава", "демонстрация", "фракция", "съезд", "пленум", "террорист", "парламент", "депутат", "парламентарий", "оппозиция", "дума", "рада"]: 
            OrganizationAnalyzer.M_POLITIC_PREFS.add(Termin._new360(s.upper(), s, s))
    ...
    def __try_attach_org_int(t : 'Token', attach_typ : 'AttachType', mult_typ : 'OrgItemTypeToken', is_additional_attach : bool, step : int) -> 'ReferentToken':
                    ...
                    if (org0_.number is None): 
                        ...
                        if (nnn is None and not ot.is_whitespace_after and ttt is not None): 
                            ...
                            if (isinstance(ttt, NumberToken)): 
                                nnn = OrgItemNumberToken._new2019(ot.end_token.next0_, ttt, str(ttt.value))
                        if (nnn is not None): 
                            org0_.number = nnn.number
                            te = nnn.end_token
                    norm = (ot.end_token.end_char - ot.begin_token.begin_char) > 5
                    tt0 = ot.begin_token
                    if (types is not None and types[0].root is not None and types[0].root.typ == OrgItemTypeTyp.PREFIX): 
                        tt0 = types[0].end_token.next0_
                    s = MiscHelper.get_text_value(tt0, ot.end_token, Utils.valToEnum((((GetTextAttr.FIRSTNOUNGROUPTONOMINATIVE if norm else GetTextAttr.NO))) | (GetTextAttr.IGNOREARTICLES), GetTextAttr))
        ...
        if ((rt0 is None and types is not None and len(types) == 1) and types[0].name is None): 
            ...
            cou = 100
            if (tt0 is not None): 
                accort_type_ref = None
                tt00 = tt0.previous
                while tt00 is not None and cou > 0: 
                    if (isinstance(tt00.get_referent(), OrganizationReferent)): 
                        oo = Utils.asObjectOrNull(tt00.get_referent(), OrganizationReferent)
                        if (types[0].typ in oo.types): 
                            rt0 = ReferentToken(oo, tt0, types[0].end_token)
                            break
                        if (OrgItemTypeToken.is_type_accords(oo, types[0])): 
                            if ((types[0].whitespaces_after_count < 3) and OrgItemTypeToken.try_attach(types[0].end_token.next0_, True) is not None): 
                                pass
                            elif (accort_type_ref is None): 
                                accort_type_ref = oo
                    tt00 = tt00.previous; cou -= 1
                if (rt0 is None and accort_type_ref is not None): 
                    rt0 = ReferentToken(accort_type_ref, tt0, types[0].end_token)
        if (rt0 is not None): 
    ...
    def __try_attach_org_(t0 : 'Token', t : 'Token', types : typing.List['OrgItemTypeToken'], spec_word_before : bool, attach_typ : 'AttachType', mult_typ : 'OrgItemTypeToken', is_additional_attach : bool) -> 'ReferentToken':
        ...
        if (typ is not None and typ.root is not None and ((typ.root.can_be_single_geo and not typ.root.can_has_single_name))): 
            ...
            if (len(org0_.geo_objects) > 0 and te is not None): 
                ...
                if (npt11 is not None and (te.whitespaces_after_count < 2) and npt11.noun.is_value("ДЕПУТАТ", None)): 
                    ...
                    if (t0.begin_char > te.begin_char): 
                        return None
                ...
                if (ty2 is not None): 
                    ...
                    if (rt2 is not None): 
                        ...
                        if (not OrgItemTypeToken.is_types_antagonisticoo(org0_, org0)): 
                            ...
                            return rt2
        if ((typ is not None and br is None and BracketHelper.can_be_start_of_sequence(t, False, False)) and BracketHelper.is_bracket(t, True)): 
            br = BracketHelper.try_parse(t, BracketParseAttr.NEARCLOSEBRACKET, 100)
        ...
        if (br is not None and br.is_quote_type and ((br.open_char != '<' or ((typ is not None and typ.root is not None and typ.root.is_pure_prefix)) or (((types is not None and len(types) > 0 and types[0].root is not None) and types[0].root.is_pure_prefix))))): 
            ...
            if (ok2 and (isinstance(rt0.referent, OrganizationReferent))): 
                ...
                if (hi): 
                    ...
                    if (typ is not None and typ.root is not None and typ.root.typ == OrgItemTypeTyp.PREFIX): 
                        if (org0.contains_profile(OrgProfile.UNIT)): 
                            org0.add_slot(OrganizationReferent.ATTR_PROFILE, None, True, 0)
                            org0.add_slot(OrganizationReferent.ATTR_TYPE, None, True, 0)
            ...
            for step in range(2):
                        ...
                        if ((org0) is not None): 
                            ...
                            if (typ is not None and ((pref or not typ.is_dep))): 
                                ...
                                if (((attach_typ == OrganizationAnalyzer.AttachType.EXTONTOLOGY or attach_typ == OrganizationAnalyzer.AttachType.HIGH)) and ((typ.root is None or not typ.root.is_pure_prefix))): 
                                    ...
                                    if (typ.begin_token == br.begin_token.next0_ and typ.end_token.next0_ == br.end_token and BracketHelper.is_bracket(br.end_token, True)): 
                                        org0_.add_name(MiscHelper.get_text_value_of_meta_token(typ, GetTextAttr.NO), True, None)
                                        t1 = typ.end_token
                                        break
                ...
                if (ok0): 
                    ...
                    return ReferentToken._new1227(org0_, t0, t1, org0_)
        num = None
        _num = None
        epon = None
        _epon = None
        ...
        while True:
            ...
            if ((rt) is not None): 
                ...
                if (rt is not None): 
                    return rt
            if ((isinstance(t.get_referent(), GeoReferent)) and t.chars.is_cyrillic_letter): 
                if (org0_.find_slot(None, t.get_referent(), True) is not None and (t.whitespaces_before_count < 3)): 
                    t1 = t
                    continue
                ...
                if (br.is_quote_type): 
                    ...
                    if (names is not None and len(names) > 0 and not spec_word_before): 
                        ...
                        if (tt2 is not None and tt2.is_comma): 
                            tt2 = tt2.next0_
                        if (tt2 is not None and tt2.is_value("ИМЕНУЕМЫЙ", None)): 
                            pass
                        else: 
                            break
                    if (not org0_.can_be_equals(org0, ReferentsEqualType.FORMERGING)): 
            ...
            if (own_org is not None): 
                if (names is None and t.is_value("ПО", None) and not t.is_value2("ПО", "ИТОГ")): 
                    pass
            ...
            if (na is None and t is not None): 
                if (org0_.kind == OrganizationKind.CHURCH or ((typ is not None and typ.typ is not None and "фермер" in typ.typ))): 
                    ...
                    if (prt is not None): 
                        na = OrgItemNameToken._new2632(t, prt.end_token, True)
                        na.value = MiscHelper.get_text_value_of_meta_token(na, GetTextAttr.NO)
                        na.chars = CharsInfo._new2633(True)
                        na.morph = prt.morph
            ...
            if (na is None): 
                ...
                if ((((names is None and t.is_char('/') and (isinstance(t.next0_, TextToken))) and not t.is_whitespace_after and t.next0_.chars.is_all_upper) and t.next0_.length_char >= 3 and (isinstance(t.next0_.next0_, TextToken))) and not t.next0_.is_whitespace_after and t.next0_.next0_.is_char('/')): 
                    na = OrgItemNameToken._new2634(t, t.next0_.next0_, t.next0_.get_source_text().upper(), t.next0_.chars)
                    ...
                    if (na.preposition is not None): 
                        ...
                        if (prt is not None): 
                            if (org0_.kind == OrganizationKind.CHURCH): 
                                ...
                                na.end_token = prt.end_token
                            elif (prt.referent.type_name == "PERSONPROPERTY"): 
                                pass
                            else: 
                                break
                if (na.is_empty_word): 
        ...
        if (typ is not None): 
            ...
            if (LanguageHelper.ends_with_ex(typ.typ, "комитет", "комиссия", "комітет", "комісія")): 
                ...
                if (isinstance(typ.begin_token.previous, NumberToken)): 
                    ...
                    num = OrgItemNumberToken._new2019(t0, t0, typ.begin_token.previous.value)
        ...
        if (((num is not None or org0_.number is not None or epon is not None) or attach_typ == OrganizationAnalyzer.AttachType.HIGH or attach_typ == OrganizationAnalyzer.AttachType.EXTONTOLOGY) or own_org is not None): 
            ...
            elif (org0_.find_slot(OrganizationReferent.ATTR_NAME, None, True) is not None): 
                pass
            elif (typ.root is not None and typ.root.is_pure_prefix): 
    ...
    def __try_attach_dep_after_org(typ : 'OrgItemTypeToken') -> 'ReferentToken':
        ...
        if (dep.find_slot(OrganizationReferent.ATTR_GEO, None, True) is None): 
            if (typ.root.can_has_number and dep.number is not None and dep.higher is not None): 
                pass
            else: 
                return None
        return res
    ...
    def __try_attach_dep(typ : 'OrgItemTypeToken', attach_typ : 'AttachType', spec_word_before : bool) -> 'ReferentToken':
        ...
        if (typ.end_token.next0_ is not None and (typ.end_token.whitespaces_after_count < 2)): 
            if (isinstance(typ.end_token.next0_.get_referent(), OrganizationReferent)): 
                ...
                in_br = False
                if (na0 is not None): 
                    if (typ.typ == "созыв" or typ.typ == "курс"): 
                        return None
                if (na0 is not None and ((na0.std_org_name_nouns > 0 or na0.is_std_name))): 
                    if (typ.typ == "управление" and na0.morph is not None and na0.morph.case_.is_instrumental): 
                        pass
                    else: 
                        spec_word_before = True
                else: 
                    ...
                    if (rt00 is not None): 
                        ...
                        rrr = OrganizationAnalyzer._try_attach_orgs(typ.end_token.next0_, 0)
        ...
        if (typ.typ == "курс" or typ.typ == "группа" or typ.typ == "група"): 
            if (dep.number is None): 
                ...
                n = 0
                wrapn2636 = RefOutArgWrapper(0)
                inoutres2637 = Utils.tryParseInt(dep.number, wrapn2636)
                n = wrapn2636.value
                if (inoutres2637): 
                    if (n > 0 and (n < 9)): 
                        coef += (2)
    ...
    def __correct_dep_attrs(res : 'ReferentToken', typ : 'OrgItemTypeToken', after_temp_org : bool=False) -> None:
        ...
        is_unit = False
        for ty in dep.types: 
            if (((((("офис" in ty or "офіс" in ty or "отдел" in ty) or "отделение" in ty or "инспекция" in ty) or "лаборатория" in ty or "управление" in ty) or "управління" in ty or "відділ" in ty) or "відділення" in ty or "інспекція" in ty) or "лабораторія" in ty): 
                is_unit = True
        if (((typ is not None and typ.root is not None and typ.root.can_has_number)) or is_unit): 
            if ((isinstance(t0.previous, NumberToken)) and (t0.whitespaces_before_count < 3) and t0.previous.is_whitespace_before): 
                if (t0.previous.morph.class0_.is_noun and not t0.previous.morph.class0_.is_adjective): 
                    pass
                else: 
                    ok = True
                    cou = 3
                    tt1 = t0.previous.previous
                    while tt1 is not None and cou >= 0: 
                        typ00 = OrgItemTypeToken.try_attach(tt1, False)
                        if (typ00 is not None and typ00.root is not None and typ00.root.typ == OrgItemTypeTyp.DEPADD): 
                            ok = False
                            break
                        tt1 = tt1.previous; cou -= 1
                    if (ok): 
    ...
    def __try_attach_org_med_typ(t : 'Token') -> 'MetaToken':
        ...
        if (((t is not None and s == "Г" and t.next0_ is not None) and t.next0_.is_char_of("\\/.") and t.next0_.next0_ is not None) and t.next0_.next0_.is_value("Б", None)): 
            ...
            if (t.next0_.is_char('.') and t1.next0_ is not None and t1.next0_.is_char('.')): 
                t1 = t1.next0_
            return MetaToken._new2639(t, t1, "городская больница", MorphCollection._new2638(MorphGender.FEMINIE))
        if ((s == "ИН" and t.next0_ is not None and t.next0_.is_hiphen) and t.next0_.next0_ is not None and t.next0_.next0_.is_value("Т", None)): 
            return MetaToken._new2639(t, t.next0_.next0_, "институт", MorphCollection._new2638(MorphGender.MASCULINE))
        if ((s == "Б" and t.next0_ is not None and t.next0_.is_hiphen) and (isinstance(t.next0_.next0_, TextToken)) and ((t.next0_.next0_.is_value("ЦА", None) or t.next0_.next0_.is_value("ЦУ", None)))): 
            return MetaToken._new2639(t, t.next0_.next0_, "больница", MorphCollection._new2638(MorphGender.FEMINIE))
        if (s == "ГКБ"): 
            return MetaToken._new2639(t, t, "городская клиническая больница", MorphCollection._new2638(MorphGender.FEMINIE))
        if (t.is_value("ПОЛИКЛИНИКА", None)): 
            return MetaToken._new2639(t, t, "поликлиника", MorphCollection._new2638(MorphGender.FEMINIE))
        if (t.is_value("БОЛЬНИЦА", None)): 
            return MetaToken._new2639(t, t, "больница", MorphCollection._new2638(MorphGender.FEMINIE))
    ...
    def __try_attach_org_names(t : 'Token') -> 'ReferentToken':
        ...
        if (br is not None): 
            ...
            while tt is not None and tt.end_char <= br.end_char: 
                ...
                if (tt.is_char_of("?:")): 
                    return None
                if (((tt == br.begin_token.next0_ or tt == br.end_token.previous)) and not br.begin_token.next0_.chars.is_all_lower): 
        ...
        if ((tok is None and not ok and tname1 is None) and prof == OrgProfile.UNDEFINED): 
            ...
            if (tok1 is not None): 
                ...
                if (prof != OrgProfile.UNDEFINED and prof != pr): 
                    return None
                if ((isinstance(t00, TextToken)) and t00.is_pure_verb): 
                    mc0 = br.begin_token.next0_.get_morph_class_in_dictionary()
                    if (mc0.is_noun): 
                        return None
            else: 
    ...
    def __init_sport() -> None:
        ...
        for s in ["байдарка", "каноэ", "лук", "трава", "коньки", "трамплин", "двоеборье", "батут", "вода", "шпага", "сабля", "лыжи", "скелетон"]: 
            OrganizationAnalyzer.M_SPORTS.add(Termin._new2625(s.upper(), s))
        ...
        for s in ["СПАРТАК", "ЦСКА", "ЗЕНИТ!", "ТЕРЕК", "КРЫЛЬЯ СОВЕТОВ", "ДИНАМО", "АНЖИ", "КУБАНЬ", "АЛАНИЯ", "ТОРПЕДО", "АРСЕНАЛ!", "ЛОКОМОТИВ", "МЕТАЛЛУРГ!", "РОТОР", "СКА", "СОКОЛ!", "ХИМИК!", "ШИННИК", "РУБИН", "ШАХТЕР", "САЛАВАТ ЮЛАЕВ", "ТРАКТОР!", "АВАНГАРД!", "АВТОМОБИЛИСТ!", "АТЛАНТ!", "ВИТЯЗЬ!", "НАЦИОНАЛЬНАЯ ХОККЕЙНАЯ ЛИГА;НХЛ", "КОНТИНЕНТАЛЬНАЯ ХОККЕЙНАЯ ЛИГА;КХЛ", "СОЮЗ ЕВРОПЕЙСКИХ ФУТБОЛЬНЫХ АССОЦИАЦИЙ;УЕФА;UEFA", "Женская теннисная ассоциация;WTA", "Международная федерация бокса;IBF", "Всемирная боксерская организация;WBO", "РЕАЛ", "МАНЧЕСТЕР ЮНАЙТЕД", "манчестер сити", "БАРСЕЛОНА!", "БАВАРИЯ!", "ЧЕЛСИ", "ЛИВЕРПУЛЬ!", "ЮВЕНТУС", "НАПОЛИ", "БОЛОНЬЯ", "ФУЛХЭМ", "ЭВЕРТОН", "ФИЛАДЕЛЬФИЯ", "ПИТТСБУРГ", "ИНТЕР!", "Аякс", "ФЕРРАРИ;FERRARI", "РЕД БУЛЛ;RED BULL", "МАКЛАРЕН;MCLAREN", "МАКЛАРЕН-МЕРСЕДЕС;MCLAREN-MERCEDES"]: 
            ...
            t = Termin._new124(pp[0], OrgProfile.SPORT)
            if (not is_bad): 
                t.tag2 = (ss)
            if (len(pp) > 1): 
                if (len(pp[1]) < 4): 
        ...
        for s in ["ИТАР ТАСС;ТАСС;Телеграфное агентство советского союза", "Интерфакс;Interfax", "REGNUM", "ЛЕНТА.РУ;Lenta.ru", "Частный корреспондент;ЧасКор", "РИА Новости;Новости!;АПН", "Росбалт;RosBalt", "УНИАН", "ИНФОРОС;inforos", "Эхо Москвы", "Сноб!", "Серебряный дождь", "Вечерняя Москва;Вечерка", "Московский Комсомолец;Комсомолка", "Коммерсантъ;Коммерсант", "Афиша", "Аргументы и факты;АИФ", "Викиновости", "РосБизнесКонсалтинг;РБК", "Газета.ру", "Русский Репортер!", "Ведомости", "Вести!", "Рамблер Новости", "Живой Журнал;ЖЖ;livejournal;livejournal.ru", "Новый Мир", "Новая газета", "Правда!", "Известия!", "Бизнес!", "Русская жизнь!", "НТВ Плюс", "НТВ", "ВГТРК", "ТНТ", "Муз ТВ;МузТВ", "АСТ", "Эксмо", "Астрель", "Терра!", "Финанс!", "Собеседник!", "Newsru.com", "Nature!", "Россия сегодня;Russia Today;RT!", "БЕЛТА", "Ассошиэйтед Пресс;Associated Press", "France Press;France Presse;Франс пресс;Agence France Presse;AFP", "СИНЬХУА", "Gallup", "Cable News Network;CNN", "CBS News", "ABC News", "GoogleNews;Google News", "FoxNews;Fox News", "Reuters;Рейтер", "British Broadcasting Corporation;BBC;БиБиСи;BBC News", "MSNBC", "Голос Америки", "Аль Джазира;Al Jazeera", "Радио Свобода", "Радио Свободная Европа", "Guardian;Гардиан", "Daily Telegraph", "The Telegraph", "Times;Таймс!", "Independent!", "Financial Times", "Die Welt", "Bild!", "La Pepublica;Република!", "Le Monde", "People Daily", "BusinessWeek", "Economist!", "Forbes;Форбс", "Los Angeles Times", "New York Times", "Wall Street Journal;WSJ", "Washington Post", "Le Figaro;Фигаро", "Bloomberg", "DELFI!", "Sky News;SkyNews", "The Wall Street Journal;Wall Street Journal", "The Moscow Times"]: 
            ...
            if (ss.endswith("!")): 
                ...
                ss = ss[0:0+len(ss) - 1]
            pp = Utils.splitString(ss, ';', False)
            t = Termin._new124(pp[0], OrgProfile.MEDIA)
            if (not is_bad): 
                t.tag2 = (ss)
            ii = 1
            while ii < len(pp): 
        ...
        for s in ["Машина времени!", "ДДТ", "Биттлз;Bittles", "ABBA;АББА", "Океан Эльзы;Океан Эльзи", "Аквариум!", "Крематорий!", "Наутилус;Наутилус Помпилиус!", "Пусси Райот;Пусси Риот;Pussy Riot", "Кино!", "Алиса!", "Агата Кристи!", "Чайф", "Ария!", "Земфира!", "Браво!", "Черный кофе!", "Воскресение!", "Урфин Джюс", "Сплин!", "Пикник!", "Мумий Троль", "Коррозия металла", "Арсенал!", "Ночные снайперы!", "Любэ", "Ласковый май!", "Noize MC", "Linkin Park", "ac dc", "green day!", "Pink Floyd;Пинк Флойд", "Depeche Mode", "Bon Jovi", "Nirvana;Нирвана!", "Queen;Квин!", "Nine Inch Nails", "Radioheads", "Pet Shop Boys", "Buggles"]: 
            ...
            if (ss.endswith("!")): 
                ...
                ss = ss[0:0+len(ss) - 1]
            pp = Utils.splitString(ss, ';', False)
            t = Termin._new124(pp[0], OrgProfile.MUSIC)
            if (not is_bad): 
                t.tag2 = (ss)
            ii = 1
            while ii < len(pp): 
        ...
        for s in ["ФАНАТ", "БОЛЕЛЬЩИК", "гонщик", "вратарь", "нападающий", "голкипер", "полузащитник", "полу-защитник", "центрфорвард", "центр-форвард", "форвард", "игрок", "легионер", "спортсмен"]: 
            OrganizationAnalyzer.M_PROP_PREF.add(Termin._new124(s.upper(), OrgProfile.SPORT))
        for s in ["защитник", "капитан", "пилот", "игра", "поле", "стадион", "гонка", "чемпионат", "турнир", "заезд", "матч", "кубок", "олипмиада", "финал", "полуфинал", "победа", "поражение", "разгром", "дивизион", "олипмиада", "финал", "полуфинал", "играть", "выигрывать", "выиграть", "проигрывать", "проиграть", "съиграть"]: 
            OrganizationAnalyzer.M_PROP_PREF.add(Termin._new360(s.upper(), OrgProfile.SPORT, s))
        for s in ["корреспондент", "фотокорреспондент", "репортер", "журналист", "тележурналист", "телеоператор", "главный редактор", "главред", "телеведущий", "редколлегия", "обозреватель", "сообщать", "сообщить", "передавать", "передать", "писать", "написать", "издавать", "пояснить", "пояснять", "разъяснить", "разъяснять", "сказать", "говорить", "спрашивать", "спросить", "отвечать", "ответить", "выяснять", "выяснить", "цитировать", "процитировать", "рассказать", "рассказывать", "информировать", "проинформировать", "поведать", "напечатать", "напоминать", "напомнить", "узнать", "узнавать", "репортаж", "интервью", "информации", "сведение", "ИА", "информагенство", "информагентство", "информационный", "газета", "журнал"]: 
            OrganizationAnalyzer.M_PROP_PREF.add(Termin._new124(s.upper(), OrgProfile.MEDIA))
        for s in ["сообщение", "статья", "номер", "журнал", "издание", "издательство", "агентство", "цитата", "редактор", "комментатор", "по данным", "оператор", "вышедший", "отчет", "вопрос", "читатель", "слушатель", "телезритель", "источник", "собеедник"]: 
            OrganizationAnalyzer.M_PROP_PREF.add(Termin._new360(s.upper(), OrgProfile.MEDIA, s))
        for s in ["музыкант", "певец", "певица", "ударник", "гитарист", "клавишник", "солист", "солистка", "исполнять", "исполнить", "концерт", "гастроль", "выступление", "известный", "известнейший", "популярный", "популярнейший", "рокгруппа", "панкгруппа", "артгруппа", "группа", "пластинка", "грампластинка", "концертный", "музыка", "запись", "студия"]: 
            OrganizationAnalyzer.M_PROP_PREF.add(Termin._new124(s.upper(), OrgProfile.MUSIC))
        ...
        for ot in gt: 
            ...
            if (ot.begin_token == ot.end_token): 
                if (len(gt) == 1): 
                    ...
                    if (not ok): 
                        return None
            if (((t.chars.is_all_lower and attach_typ != OrganizationAnalyzer.AttachType.EXTONTOLOGY and ext_geo is None) and not t.is_value("МИД", None) and not org0._types_contains("факультет")) and not org0._types_contains("министерство") and org0.kind != OrganizationKind.JUSTICE): 
                if (ot.begin_token == ot.end_token): 
                    continue
                ...
                if (t.next0_ is not None and (isinstance(t.next0_.get_referent(), GeoReferent))): 
                    ...
                    pass
                elif (tyty is not None and ot.length_char > 20): 
                    pass
                elif (not okk): 
                    continue
    ...
    def process(self, kit : 'AnalysisKit') -> None:
                ...
                if (step > 0 and (isinstance(t, ReferentToken)) and (isinstance(t.get_referent(), OrganizationReferent))): 
                    ...
                    if (mt is not None): 
                        if (ad is not None): 
                            ...
                            ad.aliases.add(term)
                        rt = ReferentToken._new1225(t.get_referent(), t, mt.end_token, t.morph)
                        kit.embed_token(rt)
                        t = (rt)
                for kk in range(5):
                    rts = OrganizationAnalyzer._try_attach_orgs(t, step)
                    ...
                    if ((len(rts) == 1 and t == rts[0] and (isinstance(t.next0_, ReferentToken))) and (isinstance(t.next0_.get_referent(), OrganizationReferent))): 
                        ...
                        if (org1.higher is None and OrgOwnershipHelper.can_be_higher(org0, org1, False) and not OrgOwnershipHelper.can_be_higher(org1, org0, False)): 
                            ...
                            kit.debed_token(rtt)
                            org1.higher = org0
                            rt1 = ReferentToken._new1225(ad.register_referent(org1), t, rtt.end_token, t.next0_.morph)
                            kit.embed_token(rt1)
                            t = (rt1)
        ...
        while True:
            ...
            if (((t1 is not None and t1.is_char('(') and t1.next0_ is not None) and (isinstance(t1.next0_.get_referent(), OrganizationReferent)) and t1.next0_.next0_ is not None) and t1.next0_.next0_.is_char(')')): 
                ...
                if (org0 == org0_ or org0_.higher == org0): 
                    rt1 = ReferentToken._new1225(org0_, t, t1.next0_.next0_, t.morph)
                    kit.embed_token(rt1)
                    t = (rt1)
                    t1 = t.next0_
        ...
        while True:
            ...
            for ty in org0_.types: 
                li = [ ]
                wrapli2664 = RefOutArgWrapper(None)
                inoutres2665 = Utils.tryGetValue(owners, ty, wrapli2664)
                li = wrapli2664.value
                if (not inoutres2665): 
                    li = list()
                    owners[ty] = li
        ...
        while True:
            ...
            for ty in org0_.types: 
                li = [ ]
                wrapli2666 = RefOutArgWrapper(None)
                inoutres2667 = Utils.tryGetValue(owners, ty, wrapli2666)
                li = wrapli2666.value
                if (not inoutres2667): 
                    continue
                for h in li: 
    ...
    def process_referent_stat(begin : 'Token', param : str=None) -> 'ReferentToken':
        ...
        if (param == "TYPE"): 
            ...
            if (ty is not None and ty.begin_token == begin): 
                org0_ = OrganizationReferent()
                org0_.add_type(ty, False)
                return ReferentToken._new1225(org0_, begin, ty.end_token, ty.morph)
        if (param == "MINTYPE"): 
            ad.level += 1
    ...
    def _try_attach_orgs(t : 'Token', step : int) -> typing.List['ReferentToken']:
        ...
        if (t is None): 
            return None
        if (t.chars.is_latin_letter and MiscHelper.is_eng_article(t)): 
            res11 = OrganizationAnalyzer._try_attach_orgs(t.next0_, step)
                    ...
                    if (rt0 is not None): 
                        ...
                        if (len(rt0.referent.slots) > 3): 
                            if (tt.previous is not None and ((tt.previous.morph.class0_.is_adjective and not tt.previous.morph.class0_.is_verb)) and tt.whitespaces_before_count == 1): 
                                adj = None
                                try: 
                                    adj = MorphologyService.get_wordform(tt.previous.get_source_text().upper(), MorphBaseInfo._new2670(MorphClass.ADJECTIVE, gen, tt.previous.morph.language))
                                except Exception as ex2671: 
                                    pass
                                    ...
                                    if (rt0.begin_token.previous is not None and rt0.begin_token.previous.is_hiphen and rt0.begin_token.previous.previous is not None): 
                                        ...
                                        if (tt0.chars.equals(rt0.begin_token.chars) and (isinstance(tt0, TextToken))): 
                                            ...
                                            if (tt0.morph.class0_.is_adjective and not tt0.morph.contains_attr("неизм.", None)): 
                                                try: 
                                                    adj = MorphologyService.get_wordform(adj, MorphBaseInfo._new2670(MorphClass.ADJECTIVE, gen, tt0.morph.language))
                                                except Exception as ex2673: 
                                                    pass
        ...
        while True:
            ...
            for n in org00.names: 
                ...
                if (t.is_value(str0_, None)): 
                    if (ad is not None): 
                        ad.aliases.add(Termin._new124(str0_, org00))
                    if (br): 
                        t = t.next0_
                ...
                if (org2 is not None): 
                    ...
                    if (ok and ((org0_.higher is None or org0_.higher.can_be_equals(org2, ReferentsEqualType.WITHINONETEXT)))): 
                        ...
                        if (org0_.higher == org2): 
                            if (res is None): 
                                res = ReferentToken._new1225(org0_, t, tt, tt0.morph)
                            else: 
                                res.end_token = tt
                            t = tt
                            if (len(org0_.geo_objects) == 0): 

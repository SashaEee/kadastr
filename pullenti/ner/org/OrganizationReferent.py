...
class OrganizationReferent(Referent):
    ...
    def to_string_ex(self, short_variant : bool, lang : 'MorphLang'=None, lev : int=0) -> str:
        ...
        if (fams is not None): 
            ...
            while i < len(fams): 
                ...
                i += 1
        if (altname is not None and not is_dep and not short_variant): 
    ...
    def __correct_name(self, name : str, num : int) -> str:
        ...
        if (str.isdigit(name[0]) and name.find(' ') > 0): 
            i = 0
            wrapi2676 = RefOutArgWrapper(0)
            inoutres2677 = Utils.tryParseInt(name[0:0+name.find(' ')], wrapi2676)
            i = wrapi2676.value
            if (inoutres2677): 
                if (i > 1): 
                    num.value = i
            ...
            if (i >= 0 and name[i] == '.'): 
                ...
                inoutres2678 = Utils.tryParseInt(name[i + 1:], num)
                if (i > 0 and inoutres2678 and num.value > 0): 
                    if (i < 1): 
                        return None
    ...
    def add_name(self, name : str, remove_long_gov_names : bool=True, t : 'Token'=None) -> None:
        num = 0
        wrapnum2679 = RefOutArgWrapper(0)
        s = self.__correct_name(name, wrapnum2679)
        num = wrapnum2679.value
        if (s == "ЗАКАЗЧИК"): 
            pass
        if (s is None): 
    ...
    def add_name_str(self, name : str, typ : 'OrgItemTypeToken', cou : int=1) -> None:
        ...
        if (name is None): 
            ...
            if (typ.name is not None and Utils.compareStrings(typ.name, typ.typ, True) != 0 and ((len(typ.name) > len(typ.typ) or self.find_slot(OrganizationReferent.ATTR_NAME, None, True) is None))): 
                num = 0
                wrapnum2680 = RefOutArgWrapper(0)
                s = self.__correct_name(typ.name, wrapnum2680)
                num = wrapnum2680.value
            ...
            if (typ is None or typ.is_not_typ): 
                ...
                if (typ.name is not None): 
                    num = 0
                    wrapnum2681 = RefOutArgWrapper(0)
                    ss = self.__correct_name(typ.name, wrapnum2681)
                    num = wrapnum2681.value
                    if (ss is not None): 
            ...
            if (((eq_number or eq_eponyms)) and ((self.find_slot(OrganizationReferent.ATTR_NAME, None, True) is None or org0_.find_slot(OrganizationReferent.ATTR_NAME, None, True) is None))): 
                ...
                for v in self._name_vars.items(): 
                    if (typ == ReferentsEqualType.DIFFERENTTEXTS and v[1]): 
                        continue
                    b = False
                    wrapb2683 = RefOutArgWrapper(False)
                    inoutres2684 = Utils.tryGetValue(org0_._name_vars, v[0], wrapb2683)
                    b = wrapb2683.value
                    if (not inoutres2684): 
                        continue

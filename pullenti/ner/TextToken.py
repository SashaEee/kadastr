...
class TextToken(Token):
    ...
    def __set_source(self, source : 'MorphToken') -> None:
    ...
    def correct_word(self, word : str) -> None:
        toks = MorphologyService.process(word, None, None)
        if (toks is not None and len(toks) == 1): 
            self.__set_source(toks[0])
    ...
    def check_value(self, dict0_ : typing.List[tuple]) -> object:
        ...
        if (dict0_ is None): 
            return None
        res = None
        wrapres3170 = RefOutArgWrapper(None)
        inoutres3171 = Utils.tryGetValue(dict0_, self.term, wrapres3170)
        res = wrapres3170.value
        if (inoutres3171): 
            return res
            ...
            for it in self.morph.items: 
                ...
                if (mf is not None): 
                    if (mf.normal_case is not None): 
                        wrapres3166 = RefOutArgWrapper(None)
                        inoutres3167 = Utils.tryGetValue(dict0_, mf.normal_case, wrapres3166)
                        res = wrapres3166.value
                        if (inoutres3167): 
                            return res
                    if (mf.normal_full is not None and mf.normal_case != mf.normal_full): 
                        wrapres3168 = RefOutArgWrapper(None)
                        inoutres3169 = Utils.tryGetValue(dict0_, mf.normal_full, wrapres3168)
                        res = wrapres3168.value
                        if (inoutres3169): 
                            return res
        return None
    ...
    def get_normal_case_text(self, mc : 'MorphClass'=None, num : 'MorphNumber'=MorphNumber.UNDEFINED, gender : 'MorphGender'=MorphGender.UNDEFINED, keep_chars : bool=False) -> str:
        ...
        for it in self.morph.items: 
            if (mc is not None and not mc.is_undefined): 
                ...
                if (cc.is_undefined): 
                    continue
                if (mc.is_proper_geo != it.class0_.is_proper_geo): 
                    continue
            ...
            if (gender != MorphGender.UNDEFINED): 
                if (((it.gender) & (gender)) == (MorphGender.UNDEFINED)): 
                    if (num == MorphNumber.PLURAL and it.number == MorphNumber.SINGULAR): 
                        continue
        ...
        if (num == MorphNumber.SINGULAR and mc is not None): 
            bi = MorphBaseInfo._new922(MorphClass._new327(mc.value), gender, MorphNumber.SINGULAR, self.morph.language)
            vars0_ = None
            try: 
                vars0_ = MorphologyService.get_wordform(self.term, bi)
            except Exception as ex3174: 
                pass
    ...
    def is_pure_verb(self) -> bool:
        ...
        ret = False
        if (((self.is_value("МОЖНО", None) or self.is_value("МОЖЕТ", None) or self.is_value("ДОЛЖНЫЙ", None)) or self.is_value("НУЖНО", None) or self.term == "ВПРАВЕ") or self.term == "ДОПУСТИМО" or self.term == "НЕДОПУСТИМО"): 
            return True
        short_form = False
        for it in self.morph.items: 
            if ((isinstance(it, MorphWordForm)) and it.is_in_dictionary): 
                if (it.class0_.is_verb and it.case_.is_undefined): 
                    ...
                    if (it.class0_.is_adjective and it.contains_attr("к.ф.", None)): 
                        ...
                        return False
        if (short_form): 
            npt = NounPhraseHelper.try_parse(self, NounPhraseParseAttr.NO, 0, None)
            if (npt is not None and npt.end_token != self): 
                return False
            return True
        return ret
    ...
    @staticmethod
    def _new900(_arg1 : 'MorphToken', _arg2 : 'AnalysisKit', _arg3 : int, _arg4 : int, _arg5 : str) -> 'TextToken':
        res = TextToken(_arg1, _arg2, _arg3, _arg4)
        res.term0 = _arg5
        return res
    
    @staticmethod
    def _new903(_arg1 : 'MorphToken', _arg2 : 'AnalysisKit', _arg3 : int, _arg4 : int, _arg5 : 'CharsInfo', _arg6 : str) -> 'TextToken':
        res = TextToken(_arg1, _arg2, _arg3, _arg4)
        res.chars = _arg5
        res.term0 = _arg6
        return res
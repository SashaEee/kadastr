...
class TransportAnalyzer(Analyzer):
    ...
    def images(self) -> typing.List[tuple]:
        res = dict()
        res[Utils.enumToString(TransportKind.FLY)] = ResourceHelper3.get_bytes("fly.png")
        res[Utils.enumToString(TransportKind.SHIP)] = ResourceHelper3.get_bytes("ship.png")
        res[Utils.enumToString(TransportKind.SPACE)] = ResourceHelper3.get_bytes("space.png")
        res[Utils.enumToString(TransportKind.TRAIN)] = ResourceHelper3.get_bytes("train.png")
        res[Utils.enumToString(TransportKind.AUTO)] = ResourceHelper3.get_bytes("auto.png")
        res[MetaTransport.IMAGE_ID] = ResourceHelper3.get_bytes("transport.png")
        return res
                        ...
                        if (s.type_name == TransportReferent.ATTR_MODEL): 
                            ...
                            for k in range(2):
                                if (not str.isdigit(mod[0])): 
                                    li = [ ]
                                    wrapli2995 = RefOutArgWrapper(None)
                                    inoutres2996 = Utils.tryGetValue(objs_by_model, mod, wrapli2995)
                                    li = wrapli2995.value
                                    if (not inoutres2996): 
                                        li = list()
                            ...
                            obj_by_names.add(Termin._new124(str(s.value), rt.referent))

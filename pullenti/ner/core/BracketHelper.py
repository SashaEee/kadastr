...
class BracketHelper:
    ...
    def can_be_start_of_sequence(t : 'Token', quotes_only : bool=False, ignore_whitespaces : bool=False) -> bool:
        ...
        if (BracketHelper.M_OPEN_CHARS.find(ch) < 0): 
            return False
        if ((quotes_only and (isinstance(t.next0_, TextToken)) and t.next0_.term == "S") and not t.is_whitespace_before and not t.is_whitespace_after): 
            return False
    ...
    def try_parse(t : 'Token', attrs : 'BracketParseAttr'=BracketParseAttr.NO, max_tokens : int=100) -> 'BracketSequenceToken':
                ...
                if (t.is_newline_before and (((attrs) & (BracketParseAttr.CANBEMANYLINES))) == (BracketParseAttr.NO)): 
                    if (t.whitespaces_before_count > 10 or BracketHelper.can_be_start_of_sequence(t, False, False)): 
                        if (t.is_char('(') and not t0.is_char('(')): 
                            ...
                            break
                if (t.is_newline_before): 
                    if (BracketHelper.can_be_start_of_sequence(t, True, False) or t.previous.is_char_of(".;")): 
                        last = t.previous
                        break
            ...
            if ((((attrs) & (BracketParseAttr.CANBEMANYLINES))) != (BracketParseAttr.NO)): 
                if (t.is_newline_before): 
                    if (t.newlines_before_count > 1): 
                        break
                    if (t.previous is not None and t.previous.is_char_of(";.") and not t0.is_char('(')): 
                        break
                    if ((isinstance(t.previous, MetaToken)) and BracketHelper.can_be_end_of_sequence(t.previous.end_token, False, t0, False)): 
                        break
                    if (BracketHelper.can_be_end_of_sequence(t.previous, False, t0, False)): 
                        break
                    if ((isinstance(t, TextToken)) and t.is_pure_verb): 
                        break
                    crlf += 1
                continue
            if (t.is_newline_before): 
        ...
        if ((len(br_list) >= 3 and BracketHelper.__can_be_close_char(br_list[1].char0_, br_list[0].char0_, False) and br_list[1].source.next0_.is_char_of(".;,")) and br_list[1].source.next0_.next0_ == br_list[2].source): 
            del br_list[2:2+len(br_list) - 2]
        if ((res is None and len(br_list) >= 3 and br_list[2].can_be_close) and not br_list[2].can_be_open): 
            ...
            if (BracketHelper.__can_be_close_char(br_list[2].char0_, br_list[0].char0_, False) and BracketHelper.__can_be_close_char(br_list[1].char0_, br_list[0].char0_, False) and br_list[1].can_be_close): 
                if (br_list[2].char0_ == br_list[1].char0_ and br_list[2].source == br_list[1].source.next0_): 
                    return BracketSequenceToken(br_list[0].source, br_list[1].source)
                t = br_list[1].source
                ...
                if (ok): 
                    ...
                    while t != br_list[1].source and t is not None: 
                        if (t.is_newline_before and br_list[0].source.begin_char <= t.previous.begin_char): 
        ...
        if (res is None and (((attrs) & (BracketParseAttr.NEARCLOSEBRACKET))) != (BracketParseAttr.NO) and len(br_list) > 2): 
            if (br_list[0].can_be_open and br_list[1].can_be_close): 
                return BracketSequenceToken(t0, br_list[1].source)
        return res
    
    @staticmethod
    def find_open_quote(close_quot : 'Token') -> 'Token':
        """ Найти назад открывающую скобку
        
        Args:
            close_quot(Token): закрывающая скобка
        ...
        while True:
            ...
            if (not BracketHelper.can_be_start_of_sequence(t, False, False)): 
                continue
            br = BracketHelper.try_parse(t, Utils.valToEnum((BracketParseAttr.CANBEMANYLINES) | (BracketParseAttr.CANCONTAINSVERBS), BracketParseAttr), 100)
            if (br is not None and br.end_token == close_quot): 
                return t
        return None

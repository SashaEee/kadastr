...
class TableHelper:
    ...
    # Начало таблицы - символ 1Eh, конец - 1Fh, ячейки оканчиваются 07h ('\a'),
    # комбинация 0D 0A 07 ("\r\n\a") или 0A 07 ("\r\a") - конец строки.
    # Если ячейка имеет RowSpan > 1 (объединение по вертикали), то перед '\a' добавляются RowSpan-1 символов 0C ('\f').
    # Если ячейка имеет ColSpan > 1 (объединнение по горизонтали), то перед '\a' добавляются ColSpan - 1 символов 09 ('\t').
    ...
    @staticmethod
    def try_parse_rows(t : 'Token', max_char : int, must_be_start_of_table : bool, always : bool=False) -> typing.List['TableRowToken']:
        if (t is None): 
            return None
        is_tab = False
        if (must_be_start_of_table): 
            if (not t.is_char(chr(0x1E))): 
                return None
            is_tab = True
        wrapis_tab1008 = RefOutArgWrapper(is_tab)
        rw = TableHelper.__parse(t, max_char, None, wrapis_tab1008)
        is_tab = wrapis_tab1008.value
        if (rw is None): 
            return None
        res = list()
        res.append(rw)
        ...
        while True:
            ...
            if (t.is_char(chr(0x1E))): 
                max0_ = t.begin_char + 10000
                if (max0_ < max_char): 
                    max0_ = max_char
                rows0 = TableHelper.try_parse_rows(t, max_char, True, True)
                if (rows0 is not None and rows0[len(rows0) - 1].end_token.is_char(chr(0x1F))): 
                    t = rows0[len(rows0) - 1].end_token
                    continue
            wrapis_tab1007 = RefOutArgWrapper(is_tab)
            rw0 = TableHelper.__parse(t, max_char, rw, wrapis_tab1007)
            is_tab = wrapis_tab1007.value
            if (rw0 is None): 
                if (t.is_table_control_char): 
                    cell_info = TableHelper.TableInfo(t)
    ...
    def __parse(t : 'Token', max_char : int, prev : 'TableRowToken', is_tab : bool) -> 'TableRowToken':
        ...
        res.cells.append(TableCellToken._new1009(t, tt, cell_info.row_span, cell_info.col_span))
        tt = tt.next0_
        while tt is not None and ((tt.end_char <= max_char or max_char == 0)): 
            t0 = tt
            cell_info = (None)
            ...
            if (cell_info.typ == TableHelper.TableTypes.ROWEND): 
                if (tt != t0): 
                    res.cells.append(TableCellToken._new1009(t0, tt, cell_info.row_span, cell_info.col_span))
                res.end_token = tt
                res._eor = True
                break
            if (cell_info.typ != TableHelper.TableTypes.CELLEND): 

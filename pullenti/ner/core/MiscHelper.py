...
class MiscHelper:
            ...
            while t1 is not None or t2 is not None:
                ...
                if (not t1.chars.is_letter): 
                    ...
                    if (t1.is_hiphen and t2.is_hiphen): 
                        ...
                        if (t1.is_value("N", None) and t2.is_value("№", None)): 
                            pass
                        elif (t2.is_value("N", None) and t1.is_value("№", None)): 
                            pass
                        else: 
                            return False
                    t1 = t1.next0_
                ...
                if (ok): 
                    ...
                    t2 = t2.next0_
                    continue
                if ((((attrs) & (CanBeEqualsAttr.IGNORENONLETTERS))) != (CanBeEqualsAttr.NO)): 
                    mc1 = t1.get_morph_class_in_dictionary()
                    if (((mc1.is_conjunction or mc1.is_preposition)) and t1.next0_ is not None): 
                        if (t1.next0_.length_char == t2.length_char): 
                            if (str(t1.next0_) == str(t2)): 
                                t1 = t1.next0_
                                continue
                    mc2 = t2.get_morph_class_in_dictionary()
                    if (((mc2.is_conjunction or mc2.is_preposition)) and t2.next0_ is not None): 
                        if (t2.next0_.length_char == t1.length_char): 
                            if (str(t2.next0_) == str(t1)): 
                                t2 = t2.next0_
                                continue
                break
    ...
    def can_be_start_of_sentence(t : 'Token') -> bool:
        ...
        if (t.chars.is_letter and t.chars.is_all_lower): 
            ...
            if (((t.previous.is_hiphen or t.previous.is_comma)) and not t.previous.is_whitespace_before and t.previous.previous is not None): 
                if (t.previous.previous.chars.is_letter and t.previous.previous.chars.is_all_lower): 
                    return False
            if (t.previous.is_hiphen and t.previous.is_newline_before): 
                return False
        ...
        if (t.previous.is_char(':')): 
            return False
        if (t.previous.is_char_of(";") and t.is_newline_before): 
            return True
        ...
        if (t.previous.is_char('.') or (((isinstance(t.previous, ReferentToken)) and t.previous.end_token.is_char('.')))): 
            ...
            if ((isinstance(t.previous.previous, NumberToken)) and t.previous.is_whitespace_before): 
                if (t.previous.previous.typ != NumberSpellingType.WORDS): 
                    return False
            if (t.previous.is_char('.') and (isinstance(t.previous.previous, TextToken)) and (t.whitespaces_before_count < 2)): 
                if (t.previous.previous.is_value("Г", None)): 
                    return False
            return True
    ...
    def convert_first_char_upper_and_other_lower(str0_ : str) -> str:
        ...
        i = 1
        while i < (tmp.tell() - 5): 
            if (Utils.getCharAtStringIO(tmp, i) == '-' and Utils.getCharAtStringIO(tmp, i + 3) == '-'): 
                Utils.setCharAtStringIO(tmp, i + 1, str.lower(Utils.getCharAtStringIO(tmp, i + 1)))
                Utils.setCharAtStringIO(tmp, i + 2, str.lower(Utils.getCharAtStringIO(tmp, i + 2)))
                break
            i += 1
    ...
    @staticmethod
    def check_abbreviation(name : str, abbr : str) -> bool:
        """ Проверить аббревиатуру
        
        Args:
            name(str): исходный термин
            abbr(str): возможная аббревиатура
        
        Returns:
            bool: может ли быть
        """
        if (Utils.isNullOrEmpty(name) or Utils.isNullOrEmpty(abbr)): 
            return False
        ...
        while name.find("  ") > 0:
            name = name.replace("  ", " ")
        name = name.upper()
        words = Utils.splitString(name, ' ', False)
        i = 0
        if (len(words) == len(abbr)): 
            i = 0
            while i < len(words): 
                if (Utils.isNullOrEmpty(words[i])): 
                    break
                elif (words[i][0] != abbr[i]): 
                    break
                i += 1
            if (i == len(words)): 
                return True
            if (len(words) == 2): 
                if (words[0][0] == abbr[1] and words[1][0] == abbr[0]): 
                    return True
            return False
        if (len(words) == (len(abbr) + 1)): 
            i = 0
            while i < (len(words) - 1): 
                if (Utils.isNullOrEmpty(words[i])): 
                    break
        ...
        if ((len(words) + 1) == len(abbr)): 
            if (words[0][0] != abbr[0]): 
                return False
            i = words[0].find(abbr[1])
            if ((i < 3) or (i + 3) >= len(words[0])): 
                return False
            i = 2
            while i < len(abbr): 
                if (Utils.isNullOrEmpty(words[i - 1])): 
                    break
                elif (words[i - 1][0] != abbr[i]): 
                    break
                i += 1
            return i == len(abbr)
        return False
    ...
    @staticmethod
    def check_image(t : 'Token') -> 'Token':
        """ Проверка картинки - конструкции вида [КартинкаXXX]
        
        Args:
            t(Token): 
        
        Returns:
            Token: при успехе вернёт указатель на последний токен ']'
        """
        if (t is None or not t.is_char('[') or t.next0_ is None): 
            return None
        if (not t.next0_.is_value("КАРТИНКА", None)): 
            return None
        cou = 4
        t = t.next0_
        while t is not None and cou > 0: 
            if (t.is_char(']')): 
                return t
            cou -= 1; t = t.next0_
        return None
    ...
    def convert_latin_word_to_russian_variants(str0_ : str) -> typing.List[str]:
        ...
        """
        res = list()
        MiscHelper.__convert_cyr_lat_word(str0_, True, res, None)
        return res
    ...
    def convert_russian_word_to_latin_variants(str0_ : str) -> typing.List[str]:
        ...
        """
        res = list()
        MiscHelper.__convert_cyr_lat_word(str0_, False, res, None)
        return res
    
    @staticmethod
    def __convert_cyr_lat_word(str0_ : str, latin_to_rus : bool, res : typing.List[str], comp : typing.List[str]) -> bool:
        if (Utils.isNullOrEmpty(str0_)): 
            return False
        str0_ = str0_.upper()
        vars0_ = list()
        ...
        while True:
            ...
            while i < len(vars0_): 
                ...
                i += 1
            www = Utils.toStringStringIO(str_tmp)
            if (res is not None): 
                res.append(www)
            elif (www in comp): 
                return True
    ...
    def can_be_equal_cyr_and_latts(t : 'Token', str0_ : str) -> bool:
        ...
        Args:
            t(Token): токен на одном языке
            str0_(str): слово на другом языке
    ...
    def __get_text_value_(begin : 'Token', end : 'Token', attrs : 'GetTextAttr', r : 'Referent') -> str:
        ...
        if ((((attrs) & (((GetTextAttr.FIRSTNOUNGROUPTONOMINATIVE) | (GetTextAttr.FIRSTNOUNGROUPTONOMINATIVESINGLE))))) != (GetTextAttr.NO)): 
            ...
            if (npt is not None and npt.end_char <= end.end_char): 
                            ...
                            if (it.gender == npt.morph.gender or ((it.gender) & (npt.morph.gender)) != (MorphGender.UNDEFINED)): 
                                if (not ((it.case_) & npt.morph.case_).is_undefined): 
                                    if (it.number == npt.morph.number or ((it.number) & (npt.morph.number)) != (MorphNumber.UNDEFINED)): 
                                        ...
                                        if (isinstance(it, MorphWordForm)): 
                                            var = it.normal_case
                                        bi = MorphBaseInfo._new922(MorphClass.ADJECTIVE, npt.morph.gender, npt.morph.number, npt.morph.language)
                                        try: 
                                            var = MorphologyService.get_wordform(var, bi)
                                        except Exception as ex923: 
                                            pass
                                        if (var is not None): 
                ...
                if (begin.is_value("ДОЛЖНЫЙ", None)): 
                    print(MiscHelper.__corr_chars("ДОЛЖЕН", begin.chars, keep_chars, Utils.asObjectOrNull(begin, TextToken)), end="", file=res)
                    begin = begin.next0_
            if ((((attrs) & (GetTextAttr.FIRSTNOUNGROUPTONOMINATIVE))) == (GetTextAttr.FIRSTNOUNGROUPTONOMINATIVE)): 
                attrs = (Utils.valToEnum((attrs) ^ (GetTextAttr.FIRSTNOUNGROUPTONOMINATIVE), GetTextAttr))
            if ((((attrs) & (GetTextAttr.FIRSTNOUNGROUPTONOMINATIVESINGLE))) == (GetTextAttr.FIRSTNOUNGROUPTONOMINATIVESINGLE)): 
        ...
        while True:
            ...
            if (t.is_whitespace_before and res.tell() > 0): 
                if (t.is_hiphen and t.is_whitespace_after and last != ' '): 
                    print(" - ", end="", file=res)
                    continue
                if (((last != ' ' and not t.is_hiphen and last != '-') and not BracketHelper.can_be_start_of_sequence(t.previous, False, False) and not t.is_char_of(",.;!?")) and not BracketHelper.can_be_end_of_sequence(t, False, None, False)): 
                    print(' ', end="", file=res)
                ...
                if (isinstance(t, NumberToken)): 
                    ...
                    if (not t.chars.is_letter): 
                        ...
                        if ((t.next0_ is not None and t.next0_.is_hiphen and t.is_value("ГЕНЕРАЛ", None)) or t.is_value("КАПИТАН", None)): 
                            ...
                            mbi = MorphBaseInfo._new925(npt.morph.gender, cas, MorphNumber.SINGULAR)
                            if (num != MorphNumber.UNDEFINED): 
                                mbi.number = num
                            wcas = None
                            try: 
                                wcas = MorphologyService.get_wordform(word, mbi)
                            except Exception as ex926: 
                                pass
                            if (wcas is not None): 
                                word = wcas
    ...
    def get_text_morph_var_by_case_and_number_ex(str0_ : str, cas : 'MorphCase'=None, num : 'MorphNumber'=MorphNumber.SINGULAR, num_val : str=None) -> str:
        ...
        ar = None
        try: 
            ar = ProcessorService.get_empty_processor().process(SourceOfAnalysis._new927(str0_, False), None, None)
        except Exception as ex: 
            pass
        if (ar is None or ar.first_token is None): 
            return str0_
        ...
        adj_bi = MorphBaseInfo._new928((MorphClass.ADJECTIVE) | MorphClass.NOUN, cas, num, npt.morph.gender)
        noun_bi = MorphBaseInfo._new929(npt.noun.morph.class0_, cas, num)
        ...
        if (not Utils.isNullOrEmpty(num_val) and num == MorphNumber.UNDEFINED): 
            ...
            n = 0
            wrapn930 = RefOutArgWrapper(0)
            Utils.tryParseInt(num_val, wrapn930)
            n = wrapn930.value
    ...
    def get_pure_referent_token(rt : 'ReferentToken') -> 'MetaToken':
        ...
        if (t0 is None): 
            t0 = rt.begin_token
            t1 = rt.end_token
        return MetaToken._new933(t0, t1, MiscHelper.get_text_value(t0, t1, Utils.valToEnum((GetTextAttr.KEEPQUOTES) | (GetTextAttr.KEEPREGISTER), GetTextAttr)))
    
    @staticmethod
    def check_pure_verb(begin_token : 'Token', end_token : 'Token') -> bool:
        """ Проверка, что в данном диапазоне есть глагол такой, что данный фрагмент не является наименованием,
        а является законченным предложением.
        
        Args:
            begin_token(Token): начальный токен
            end_token(Token): конечный токен (если null, то до конца текста)
        ...
        while True:
            ...
            if (end_token is not None and t.begin_char > end_token.end_char): 
                break
            tt = Utils.asObjectOrNull(t, TextToken)
            if (tt is None): 
                continue

...
class NumberHelper:
    ...
    def __try_parse(token : 'Token', prev_val : 'NumberToken'=None) -> 'NumberToken':
        ...
        if (val is not None): 
            ...
            if (et.next0_ is not None and et.next0_.is_char('(')): 
                ...
                if ((num2 is not None and num2.value == val and num2.end_token.next0_ is not None) and num2.end_token.next0_.is_char(')') and num2.typ != NumberSpellingType.DIGIT): 
                ...
                if (str.isdigit(sss[0]) and len(sss) == 3): 
                    ...
                    while True:
                        ...
                        if (ttt.whitespaces_before_count == 1 and ttt.length_char == 3 and str.isdigit(ss[0])): 
                            ii = 0
                            wrapii940 = RefOutArgWrapper(0)
                            inoutres941 = Utils.tryParseInt(ss, wrapii940)
                            ii = wrapii940.value
                            if (not inoutres941): 
                                break
                            val2 += ss
        ...
        while True:
            ...
            else: t = (Utils.asObjectOrNull(t.next0_, TextToken))
            ...
            if (num is None): 
                break
            if (isinstance(num.termin.tag, str)): 
                large_val = (Utils.asObjectOrNull(num.termin.tag, str))
                if (loc_value > 0): 
                    large_val = "{0}{1}".format(loc_value, large_val[1:])
                    loc_value = 0
                    jprev = -1
                    et = t
                    continue
            wrapj943 = RefOutArgWrapper(0)
            inoutres944 = Utils.tryParseInt(str(num.termin.tag), wrapj943)
            j = wrapj943.value
            if (not inoutres944): 
                break
        ...
        if (et is None): 
            return None
        if (int_val == 0 and large_val is None): 
            return None
        if (large_val is None): 
            large_val = str(int_val)
        elif (int_val > 0): 
            ss = str(int_val)
            if (len(ss) < len(large_val)): 
                large_val = (large_val[0:0+len(large_val) - len(ss)] + ss)
        nt = NumberToken(tt, et, large_val, NumberSpellingType.WORDS)
    ...
    def try_parse_age(t : 'Token') -> 'NumberToken':
        ...
        if (nt is not None): 
            ...
            if (isinstance(t, TextToken)): 
                if (t.term == "БО" and t.next0_ is not None and t.next0_.is_value("ЛЕТ", None)): 
                    return NumberToken(t, t.next0_, "60", NumberSpellingType.AGE)
        if (nt is not None): 
            if (nt_next is not None): 
                ...
                if (isinstance(t1, TextToken)): 
                    v = t1.term
                    if (((v == "ЛЕТ" or v == "ЛЕТИЯ" or v == "ЛЕТИЕ") or v == "РІЧЧЯ" or v == "ЖЫЛЖЫГЫ") or t1.is_value("ГОДОВЩИНА", None)): 
                        return NumberToken._new942(t, t1, nt.value, NumberSpellingType.AGE, t1.morph)
        ...
        if (LanguageHelper.ends_with_ex(s, "ЛЕТИЕ", "ЛЕТИЯ", "РІЧЧЯ", None)): 
            ...
            if (term is not None): 
                return NumberToken._new942(tt, tt, str(term.tag), NumberSpellingType.AGE, tt.morph)
        s = tt.lemma
        i = s.find("ЛЕТН")
        if (i < 0): 
            i = s.find("РІЧН")
        if (i > 3): 
            ...
            for i in range(len(ss) - 2, 2, -1):
                ...
                if (len(num0) > 1 and num0[len(num0) - 1] == '0' and len(num1) == 1): 
                    return NumberToken._new942(tt, tt, num0[0:0+len(num0) - 1] + num1, NumberSpellingType.AGE, tt.morph)
            else: i = 2
        return None
    ...
    def __analize_number_tail(tt : 'TextToken', val : str) -> 'MetaToken':
            ...
            if (((s == "<" or s == "(")) and (isinstance(tt.next0_, TextToken))): 
                ...
                if ((s == "TH" or s == "ST" or s == "RD") or s == "ND"): 
                    if (tt.next0_.next0_ is not None and tt.next0_.next0_.is_char_of(">)")): 
                        ...
                        return MetaToken._new950(tt, tt.next0_.next0_, mc)
            return None
        if ((s == "TH" or s == "ST" or s == "RD") or s == "ND"): 
            mc = MorphCollection()
            mc.class0_ = MorphClass.ADJECTIVE
            mc.language = MorphLang.EN
            return MetaToken._new950(tt, tt, mc)
        ...
        if (tt.morph.language.is_ua and mc is None and s == "Ї"): 
            mc = MorphCollection()
            mc.add_item(MorphBaseInfo._new953(MorphClass.ADJECTIVE))
        if (mc is not None): 
            return MetaToken._new950(tt, tt, mc)
        if ((((len(s) < 3) and not tt.is_whitespace_before and tt.previous is not None) and tt.previous.is_hiphen and not tt.previous.is_whitespace_before) and tt.whitespaces_after_count == 1 and s != "А"): 
            if (len(s) == 1 and s != "Й" and not LanguageHelper.is_cyrillic_vowel(s[0])): 
                ...
                return MetaToken._new950(tt, tt, MorphCollection._new955(MorphClass.ADJECTIVE))
        return None
    
    @staticmethod
    def __try_parse_float(t : 'NumberToken', d : float, is_real : bool, attrs : 'NumberParseAttr') -> 'Token':
            ...
            if ((i < (len(ns) - 1)) or not is_last_drob): 
                ...
                if (merge): 
                    ...
                    while kkk < ns[i].length_char: 
                        ...
                        kkk += 1
                    is_real.value = True
                    if (d.value == 0 and (ns[i].value.find('.') < 0)): 
                        is_real.value = False
                    d.value += f2
            i += 1
    ...
    def try_parse_real_number(t : 'Token', attrs : 'NumberParseAttr'=NumberParseAttr.NO) -> 'NumberToken':
        ...
        if (t.next0_ is not None and t.next0_.is_value("ЦЕЛЫЙ", "ЦІЛИЙ")): 
            ...
            if ((isinstance(tt1, NumberToken)) or (((isinstance(tt1, TextToken)) and tt1.is_value("НОЛЬ", "НУЛЬ")))): 
                ...
                if (res0.real_value == 0): 
                    ...
                    dd = 0
                    wrapdd959 = RefOutArgWrapper(0)
                    inoutres960 = Utils.tryParseFloat(str0_, wrapdd959)
                    dd = wrapdd959.value
                    if (inoutres960): 
                        pass
                    else: 
                        wrapdd957 = RefOutArgWrapper(0)
                        inoutres958 = Utils.tryParseFloat(str0_.replace('.', ','), wrapdd957)
                        dd = wrapdd957.value
                        if (inoutres958): 
                            pass
                        else: 
        ...
        d = 0
        real = False
        wrapd962 = RefOutArgWrapper(0)
        wrapreal963 = RefOutArgWrapper(False)
        tt = NumberHelper.__try_parse_float(Utils.asObjectOrNull(t, NumberToken), wrapd962, wrapreal963, attrs)
        d = wrapd962.value
        real = wrapreal963.value
        if (tt is None): 
        ...
        if (tt.next0_ is not None and tt.next0_.is_value("ДЕСЯТОК", None)): 
            d *= (10)
            tt = tt.next0_
        return NumberExToken._new961(t0, tt, "", NumberSpellingType.DIGIT, NumberExType.UNDEFINED, d, real)
    ...
    def get_number_base_info(num_val : str, cla : 'MorphClass') -> 'MorphBaseInfo':
        ...
        n = 0
        wrapn966 = RefOutArgWrapper(0)
        Utils.tryParseInt(num_val, wrapn966)
        n = wrapn966.value
        res = MorphBaseInfo._new953(cla)
    ...
    def string_to_double(str0_ : str) -> float:
        ...
        if (str0_ == "NaN"): 
            return math.nan
        wrapres971 = RefOutArgWrapper(0)
        inoutres972 = Utils.tryParseFloat(str0_, wrapres971)
        res = wrapres971.value
        if (inoutres972): 
            return res
        wrapres969 = RefOutArgWrapper(0)
        inoutres970 = Utils.tryParseFloat(str0_.replace('.', ','), wrapres969)
        res = wrapres969.value
        if (inoutres970): 
            return res
        wrapres967 = RefOutArgWrapper(0)
        inoutres968 = Utils.tryParseFloat(str0_.replace(',', '.'), wrapres967)

...
class TerminCollection:
    ...
    def __add_to_tree(self, key : str, t : 'Termin') -> None:
        ...
        while i < len(key): 
            ...
            if (nod.children is None): 
                nod.children = dict()
            nn = None
            wrapnn1028 = RefOutArgWrapper(None)
            inoutres1029 = Utils.tryGetValue(nod.children, ch, wrapnn1028)
            nn = wrapnn1028.value
            if (not inoutres1029): 
    ...
    def __remove_from_tree(self, key : str, t : 'Termin') -> None:
        ...
        while i < len(key): 
            ...
            if (nod.children is None): 
                return
            nn = None
            wrapnn1030 = RefOutArgWrapper(None)
            inoutres1031 = Utils.tryGetValue(nod.children, ch, wrapnn1030)
            nn = wrapnn1030.value
            if (not inoutres1031): 
                return
            nod = nn
    ...
    def __find_in_tree(self, key : str, lang : 'MorphLang') -> typing.List['Termin']:
        ...
        while i < len(key): 
            ...
            if (nod.children is not None): 
                wrapnn1032 = RefOutArgWrapper(None)
                Utils.tryGetValue(nod.children, ch, wrapnn1032)
                nn = wrapnn1032.value
            if (nn is None): 
                if (ch == (32)): 
    ...
    def __add_to_hash1(self, key : int, t : 'Termin') -> None:
        li = None
        wrapli1033 = RefOutArgWrapper(None)
        inoutres1034 = Utils.tryGetValue(self.__m_hash1, key, wrapli1033)
        li = wrapli1033.value
        if (not inoutres1034): 
            li = list()
    ...
    def __try_attach_all_(self, token : 'Token', pars : 'TerminParseAttr'=TerminParseAttr.NO, main_root : bool=False) -> typing.List['TerminToken']:
            ...
            if ((((pars) & (TerminParseAttr.TERMONLY))) != (TerminParseAttr.NO)): 
                ...
                while i < tt.invariant_prefix_length_of_morph_vars: 
                    ...
                    if (nod.children is None): 
                        ...
                        break
                    nn = None
                    wrapnn1035 = RefOutArgWrapper(None)
                    inoutres1036 = Utils.tryGetValue(nod.children, ch, wrapnn1035)
                    nn = wrapnn1035.value
                    if (not inoutres1036): 
                        no_vars = True
                        break
                ...
                if ((no_vars and tt.term0 is not None and tt.term != tt.term0) and tt.invariant_prefix_length_of_morph_vars <= len(tt.term0)): 
                    ...
                    while i < tt.invariant_prefix_length_of_morph_vars: 
                        ...
                        if (nod.children is None): 
                            ...
                            break
                        nn = None
                        wrapnn1037 = RefOutArgWrapper(None)
                        inoutres1038 = Utils.tryGetValue(nod.children, ch, wrapnn1037)
                        nn = wrapnn1037.value
                        if (not inoutres1038): 
                            no_vars = True
                            break
                        ...
                        i += 1
            if (not no_vars): 
                wrapres1043 = RefOutArgWrapper(res)
                inoutres1044 = self.__manage_var(token, pars, s, nod, len0, wrapres1043)
                res = wrapres1043.value
                if (inoutres1044): 
                    was_vars = True
                i = 0
                first_pass3443 = True
                while True:
                    ...
                    if (wf.normal_case is None or wf.normal_case == s): 
                        ...
                        if (j < i): 
                            ok = False
                    if (ok): 
                        wrapres1039 = RefOutArgWrapper(res)
                        inoutres1040 = self.__manage_var(token, pars, wf.normal_case, nod, tt.invariant_prefix_length_of_morph_vars, wrapres1039)
                        res = wrapres1039.value
                        if (inoutres1040): 
                            was_vars = True
                    ...
                    if (j < i): 
                        continue
                    wrapres1041 = RefOutArgWrapper(res)
                    inoutres1042 = self.__manage_var(token, pars, wf.normal_full, nod, tt.invariant_prefix_length_of_morph_vars, wrapres1041)
                    res = wrapres1041.value
                    if (inoutres1042): 
                        was_vars = True
        elif (isinstance(token, NumberToken)): 
            wrapres1045 = RefOutArgWrapper(res)
            inoutres1046 = self.__manage_var(token, pars, str(token.value), root, 0, wrapres1045)
            res = wrapres1045.value
            if (inoutres1046): 
                was_vars = True
        else: 
            return None
        if (not was_vars and s is not None and len(s) == 1): 
            vars0_ = [ ]
            wrapvars1047 = RefOutArgWrapper(None)
            inoutres1048 = Utils.tryGetValue(self.__m_hash1, ord(s[0]), wrapvars1047)
            vars0_ = wrapvars1047.value
            if (inoutres1048): 
                for t in vars0_: 
                    if (not t.lang.is_undefined): 
                        if (not token.morph.language.is_undefined): 
    ...
    def __manage_var(self, token : 'Token', pars : 'TerminParseAttr', v : str, nod : 'CharNode', i0 : int, res : typing.List['TerminToken']) -> bool:
        ...
        while i < len(v): 
            ...
            if (nod.children is None): 
                return False
            nn = None
            wrapnn1049 = RefOutArgWrapper(None)
            inoutres1050 = Utils.tryGetValue(nod.children, ch, wrapnn1049)
            nn = wrapnn1049.value
            if (not inoutres1050): 
                return False
            nod = nn
    ...
    def find_termins_by_canonic_text(self, text : str) -> typing.List['Termin']:
        if (self.__m_hash_canonic is None): 
            ...
            for t in self.termins: 
                ct = t.canonic_text
                li = [ ]
                wrapli1051 = RefOutArgWrapper(None)
                inoutres1052 = Utils.tryGetValue(self.__m_hash_canonic, ct, wrapli1051)
                li = wrapli1051.value
                if (not inoutres1052): 
                    li = list()
                ...
                if (not t in li): 
                    li.append(t)
        res = [ ]
        wrapres1053 = RefOutArgWrapper(None)
        inoutres1054 = Utils.tryGetValue(self.__m_hash_canonic, text, wrapres1053)
        res = wrapres1053.value
        if (not inoutres1054): 
            return None
        else: 
            return res
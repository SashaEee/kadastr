...
class BlockTitleToken(MetaToken):
        ...
        while True:
            ...
            if (first): 
                if ((tt.begin_char - t0.begin_char) > 300): 
                    btt0 = BlockTitleToken(t0, (t0 if tt.previous is None else tt.previous))
    ...
    def try_attach(t : 'Token', is_content_item : bool=False, names : 'TerminCollection'=None) -> 'BlockTitleToken':
        ...
        if (li.typ == BlkTyps.INDEX): 
            pass
        if (li.is_exist_name): 
            return BlockTitleToken._new820(t, li.end_token, li.typ)
        if (li.end_token == li.number_end or ((li.end_token.is_char_of(".:") and li.end_token.previous == li.number_end))): 
            res2 = BlockTitleToken._new820(t, li.end_token, li.typ)
        ...
        if (li.number_end is None): 
            return None
        res = BlockTitleToken._new820(t, li.end_token, li.typ)
    ...
    @staticmethod
    def _new820(_arg1 : 'Token', _arg2 : 'Token', _arg3 : 'BlkTyps') -> 'BlockTitleToken':

...
class NounPhraseItem(MetaToken):
    ...
    def get_normal_case_text(self, mc : 'MorphClass'=None, num : 'MorphNumber'=MorphNumber.UNDEFINED, gender : 'MorphGender'=MorphGender.UNDEFINED, keep_chars : bool=False) -> str:
        ...
        for it in self.morph.items: 
            ...
            if (v.undef_coef > 0 and (((v.undef_coef < max_coef) or def_coef >= 0))): 
                continue
            if (num == MorphNumber.SINGULAR and v.single_number_value is not None and v.single_number_value != "СУДНО"): 
                if (mc is not None and ((gender == MorphGender.NEUTER or gender == MorphGender.FEMINIE)) and mc.is_adjective): 
                    bi = MorphBaseInfo._new829(MorphClass._new327(mc.value), gender, MorphNumber.SINGULAR, MorphCase.NOMINATIVE, self.morph.language)
                    str0_ = None
                    try: 
                        str0_ = MorphologyService.get_wordform(v.single_number_value, bi)
                    except Exception as ex830: 
                        pass
                ...
                if (v.undef_coef == 0): 
                    if (res.endswith("НЬЕ")): 
                        for it2 in self.morph.items: 
                            v = (Utils.asObjectOrNull(it2, NounPhraseItemTextVar))
                            if (v is None): 
                                continue
                            if (v.undef_coef == 0 and v.single_number_value == (res[0:0+len(res) - 2] + "ИЕ")): 
                                return v.single_number_value
                    break
            ...
            if (str.isdigit(v.normal_value[0]) and mc is not None and mc.is_adjective): 
                val = 0
                wrapval831 = RefOutArgWrapper(0)
                inoutres832 = Utils.tryParseInt(v.normal_value, wrapval831)
                val = wrapval831.value
                if (inoutres832): 
            ...
            if (num == MorphNumber.PLURAL and ((v.number) & (MorphNumber.PLURAL)) == (MorphNumber.PLURAL)): 
                def_co += 3
            if (num == MorphNumber.SINGULAR and gender != MorphGender.UNDEFINED and ((v.gender) & (gender)) == (MorphGender.UNDEFINED)): 
                def_co -= 1
                    ...
                    if (wf.class0_.is_proper_name and wf.is_in_dictionary): 
                        if (wf.normal_case is None or not wf.normal_case.startswith("ЛЮБ")): 
                            if (mc0.is_adjective and t.morph.contains_attr("неизм.", None)): 
                                ...
                                pass
                            elif (wf.class0_.is_proper_geo and (((attrs) & (NounPhraseParseAttr.REFERENTCANBENOUN))) != (NounPhraseParseAttr.NO)): 
                                pass
                            else: 
                ...
                if (((isinstance(t0, TextToken)) and t0.next0_ is not None and t0.next0_.is_hiphen) and t0.next0_.next0_ is not None and t0.chars.is_cyrillic_letter == t0.next0_.next0_.chars.is_cyrillic_letter): 
                    if (not t0.is_whitespace_after and not t0.morph.class0_.is_pronoun and not (isinstance(t0.next0_.next0_, NumberToken))): 
                        if (not t0.next0_.is_whitespace_after): 
                            ...
                            t = t0.next0_.next0_
            it = NounPhraseItem._new833(t0, t, _can_be_surname)
                ...
                if (v.class0_.is_adjective or ((v.class0_.is_pronoun and not v.class0_.is_personal_pronoun and not v.contains_attr("неизм.", None))) or ((v.class0_.is_noun and (isinstance(t, NumberToken))))): 
                    if (NounPhraseItem.try_accord_variant(items, (0 if items is None else len(items)), v, False)): 
                        ...
                        if (v.contains_attr("неизм.", None)): 
                            continue
                        if (not mc0.is_undefined and wf is not None): 
                            if ((wf.undef_coef < (3)) and wf.undef_coef > (0)): 
                                if (wf.normal_case.endswith("Ы")): 
                                    pass
                                else: 
                                    continue
    ...
    @staticmethod
    def _new833(_arg1 : 'Token', _arg2 : 'Token', _arg3 : bool) -> 'NounPhraseItem':

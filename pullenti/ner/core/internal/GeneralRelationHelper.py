...
class GeneralRelationHelper:
    ...
    class Node:
        ...
        @staticmethod
        def _new823(_arg1 : 'Referent', _arg2 : 'AnalyzerData') -> 'Node':
    ...
    def refresh_generals(proc : 'Processor', kit : 'AnalysisKit') -> None:
        ...
        for a in proc.analyzers: 
            ...
            for r in ad.referents: 
                nod = GeneralRelationHelper.Node._new823(r, ad)
                all_refs.append(nod)
                r.tag = (nod)
                si = None
                wrapsi826 = RefOutArgWrapper(None)
                inoutres827 = Utils.tryGetValue(all0_, a.name, wrapsi826)
                si = wrapsi826.value
                if (not inoutres827): 
                    si = dict()
                    all0_[a.name] = si
                ...
                for s in strs: 
                    if (s is None): 
                        continue
                    li = [ ]
                    wrapli824 = RefOutArgWrapper(None)
                    inoutres825 = Utils.tryGetValue(si, s, wrapli824)
                    li = wrapli824.value
                    if (not inoutres825): 
                        li = list()
                        si[s] = li

...
class NounPhraseToken(MetaToken):
    ...
    def get_multivars(self) -> typing.List['NounPhraseMultivarToken']:
        ...
        while i < len(self.adjectives): 
            v = NounPhraseMultivarToken._new934(self.adjectives[i].begin_token, self.adjectives[i].end_token, self, i, i)
    ...
    def get_normal_case_text(self, mc : 'MorphClass'=None, num : 'MorphNumber'=MorphNumber.UNDEFINED, gender : 'MorphGender'=MorphGender.UNDEFINED, keep_chars : bool=False) -> str:
        ...
        if (self.noun.end_token.is_value("ДАННЫЙ", None)): 
            num = MorphNumber.PLURAL
            gender = MorphGender.NEUTER
    ...
    def get_normal_case_text_with_only_one_adjective(self, adj_index : int) -> str:
        res = io.StringIO()
        i = 0
        while i < len(self.adjectives): 
            if (i == adj_index): 
                s = self.adjectives[i].get_normal_case_text((MorphClass.ADJECTIVE) | MorphClass.PRONOUN, MorphNumber.SINGULAR, self.morph.gender, False)
                print("{0} ".format(Utils.ifNotNull(s, "?")), end="", file=res, flush=True)
            i += 1
        r = self.noun.get_normal_case_text((MorphClass.NOUN) | MorphClass.PRONOUN, MorphNumber.SINGULAR, MorphGender.UNDEFINED, False)
        if (r is None): 
            r = self.noun.get_normal_case_text(None, MorphNumber.UNDEFINED, MorphGender.UNDEFINED, False)
        print(Utils.ifNotNull(r, str(self.noun)), end="", file=res)
        return Utils.toStringStringIO(res)
    ...
    def get_morph_variant(self, cas : 'MorphCase', plural : bool) -> str:
        ...
        """
        mi = MorphBaseInfo._new935(cas, MorphLang.RU)
        if (plural): 
    ...
    @staticmethod
    def _new896(_arg1 : 'Token', _arg2 : 'Token', _arg3 : 'PrepositionToken') -> 'NounPhraseToken':

...
class StatisticCollection:
    ...
    def __add_token(self, tt : 'TextToken') -> 'StatisticWordInfo':
        ...
        res = None
        for v in vars0_: 
            wrapres987 = RefOutArgWrapper(None)
            inoutres988 = Utils.tryGetValue(self.__m_items, v, wrapres987)
            res = wrapres987.value
            if (inoutres988): 
                break
        if (res is None): 
            res = StatisticWordInfo._new989(tt.lemma)
        for v in vars0_: 
    ...
    def __find_item(self, tt : 'TextToken', do_absolute : bool=True) -> 'StatisticWordInfo':
        if (tt is None): 
            return None
        res = None
        wrapres996 = RefOutArgWrapper(None)
        inoutres997 = Utils.tryGetValue(self.__m_items, tt.term, wrapres996)
        res = wrapres996.value
        if (inoutres997): 
            return res
        if (do_absolute): 
            s = MiscHelper.get_absolute_normal_value(tt.term, False)
            if (s is not None): 
                wrapres990 = RefOutArgWrapper(None)
                inoutres991 = Utils.tryGetValue(self.__m_items, s, wrapres990)
                res = wrapres990.value
                if (inoutres991): 
                    return res
        for wff in tt.morph.items: 
            wf = Utils.asObjectOrNull(wff, MorphWordForm)
            if (wf is None): 
                continue
            wrapres994 = RefOutArgWrapper(None)
            inoutres995 = Utils.tryGetValue(self.__m_items, Utils.ifNotNull(wf.normal_case, ""), wrapres994)
            res = wrapres994.value
            if (inoutres995): 
                return res
            wrapres992 = RefOutArgWrapper(None)
            inoutres993 = Utils.tryGetValue(self.__m_items, wf.normal_full, wrapres992)
            res = wrapres992.value
            if (wf.normal_full is not None and inoutres993): 
                return res
        return None
    
    def __add_bigramm(self, b1 : 'StatisticWordInfo', b2 : 'StatisticWordInfo') -> None:
        di = None
        wrapdi1000 = RefOutArgWrapper(None)
        inoutres1001 = Utils.tryGetValue(self.__m_bigramms, b1.normal, wrapdi1000)
        di = wrapdi1000.value
        if (not inoutres1001): 
            di = dict()
        ...
        if (b2.normal in di): 
            ...
            di[b2.normal] = 1
        wrapdi998 = RefOutArgWrapper(None)
        inoutres999 = Utils.tryGetValue(self.__m_bigramms_rev, b2.normal, wrapdi998)
        di = wrapdi998.value
        if (not inoutres999): 
            di = dict()
    ...
    def __get_bigrams_info(self, si1 : 'StatisticWordInfo', si2 : 'StatisticWordInfo') -> 'StatisticBigrammInfo':
        res = StatisticBigrammInfo._new1002(si1.total_count, si2.total_count)
        di12 = None
        wrapdi121004 = RefOutArgWrapper(None)
        Utils.tryGetValue(self.__m_bigramms, si1.normal, wrapdi121004)
        di12 = wrapdi121004.value
        di21 = None
        wrapdi211003 = RefOutArgWrapper(None)
        Utils.tryGetValue(self.__m_bigramms_rev, si2.normal, wrapdi211003)
        di21 = wrapdi211003.value
        if (di12 is not None): 
    ...
    def get_initial_info(self, ini : str, sur : 'Token') -> 'StatisticBigrammInfo':
        ...
        if (si2 is None): 
            return None
        si1 = None
        wrapsi11005 = RefOutArgWrapper(None)
        inoutres1006 = Utils.tryGetValue(self.__m_items, ini[0:0+1], wrapsi11005)
        si1 = wrapsi11005.value
        if (not inoutres1006): 
            return None
        if (si1 is None): 
            return None

...
class AnalysisKit:
    ...
    def __init__(self, sofa_ : 'SourceOfAnalysis'=None, only_tokenizing : bool=False, lang : 'MorphLang'=None, progress : EventHandler=None) -> None:
        ...
        self.msgs = list()
        self.ignore_begin_end_char_in_metatoken = False
        if (sofa_ is None): 
            return
        self.__m_sofa = sofa_
        ...
        if (tokens is not None): 
            ...
            while ii < len(tokens): 
                ...
                if (sofa_.correction_dict is not None): 
                    corw = None
                    wrapcorw901 = RefOutArgWrapper(None)
                    inoutres902 = Utils.tryGetValue(sofa_.correction_dict, mt.term, wrapcorw901)
                    corw = wrapcorw901.value
                    if (inoutres902): 
                        ccc = MorphologyService.process(corw, lang, None)
                        if (ccc is not None and len(ccc) == 1): 
                            tt1 = TextToken._new900(ccc[0], self, tt.begin_char, tt.end_char, tt.term)
                            tt1.chars = tt.chars
                            tt = tt1
    ...
    def __correct_words_by_merging(self, lang : 'MorphLang') -> None:
        ...
        while True:
            ...
            if (t1.morph.contains_attr("прдктв.", None)): 
                continue
            if (t1.next0_ is not None and t1.next0_.is_hiphen): 
                continue
    ...
    def correct_word_by_morph(self, tt : 'Token', lang : 'MorphLang') -> 'Token':
        ...
        if (ccc is None or len(ccc) != 1): 
            return None
        tt1 = TextToken._new903(ccc[0], self, tt.begin_char, tt.end_char, tt.chars, tt.term)
    ...
    def embed_token(self, mt : 'MetaToken') -> None:
        ...
        if (isinstance(mt, ReferentToken)): 
            if (mt.referent is not None): 
                mt.referent.add_occurence(TextAnnotation._new904(self.sofa, mt.begin_char, mt.end_char))
    ...
    def get_analyzer_data(self, analyzer : 'Analyzer') -> 'AnalyzerData':
        ...
        if (analyzer is None or analyzer.name is None): 
            return None
        d = None
        wrapd905 = RefOutArgWrapper(None)
        inoutres906 = Utils.tryGetValue(self.__m_datas, analyzer.name, wrapd905)
        d = wrapd905.value
        if (inoutres906): 
            d.kit = self
            return d
    ...
    def replace_referent(self, old_referent : 'Referent', new_referent : 'Referent') -> None:
        ...
        for d in self.__m_datas.values(): 
            ...
            new_referent.occurrence.extend(old_referent.occurrence)
    ...
    def deserialize(self, stream : Stream) -> bool:
        ...
        self.base_language = MorphLang._new330(SerializerHelper.deserialize_int(stream))
        self.__m_entities = list()
    ...
    @staticmethod
    def _new3147(_arg1 : 'Processor', _arg2 : 'ExtOntology') -> 'AnalysisKit':
        res = AnalysisKit()
        res.processor = _arg1
        res.ontology = _arg2
        return res
    ...
    @staticmethod
    def _new3148(_arg1 : 'SourceOfAnalysis', _arg2 : bool, _arg3 : 'MorphLang', _arg4 : EventHandler, _arg5 : 'ExtOntology', _arg6 : 'Processor', _arg7 : bool) -> 'AnalysisKit':

...
class IntOntologyCollection:
    ...
    class OntologyTermin(Termin):
        ...
        @staticmethod
        def _new919(_arg1 : 'IntOntologyItem', _arg2 : object) -> 'OntologyTermin':
    ...
    def add_item(self, di : 'IntOntologyItem') -> None:
        ...
        while i < len(di.termins): 
            if (isinstance(di.termins[i], IntOntologyCollection.OntologyTermin)): 
                ...
                nt = IntOntologyCollection.OntologyTermin._new919(di, di.termins[i].tag)
    ...
    def add_termin(self, di : 'IntOntologyItem', t : 'Termin') -> None:
        nt = IntOntologyCollection.OntologyTermin._new919(di, t.tag)
        t.copy_to(nt)
        self.m_termins.add(nt)
    ...
    def try_attach(self, t : 'Token', referent_type_name : str=None, can_be_geo_object : bool=False) -> typing.List['IntOntologyToken']:
        attrs = TerminParseAttr.IGNOREBRACKETS
        if (can_be_geo_object): 
            attrs = (Utils.valToEnum((attrs) | (TerminParseAttr.CANBEGEOOBJECT), TerminParseAttr))
        tts = self.m_termins.try_parse_all(t, attrs)
        if (tts is None): 
            return None
        res = list()
        dis = list()
        for tt in tts: 
            ...
            if (di is not None): 
                ...
                if (di in dis): 
                    continue
                dis.append(di)
            res.append(IntOntologyToken._new921(tt.begin_token, tt.end_token, di, tt.termin, tt.morph))
    ...
    @staticmethod
    def _new3123(_arg1 : bool) -> 'IntOntologyCollection':

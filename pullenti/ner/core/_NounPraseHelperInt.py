...
class _NounPraseHelperInt:
    ...
    def __try_parse_ru(first : 'Token', typ : 'NounPhraseParseAttr', max_char_pos : int, def_noun : 'NounPhraseItem'=None) -> 'NounPhraseToken':
        ...
        if ((tt1 is not None and check and ((tt1.morph.class0_.is_preposition or tt1.morph.case_.is_instrumental))) and (tt1.whitespaces_before_count < 2)): 
            ...
            if (inp is not None): 
                ...
                if (npt1 is not None): 
                    ...
                    first_pass3409 = True
                    while True:
                        if first_pass3409: first_pass3409 = False
                        else: ii += 1
        ...
        first_pass3410 = True
        while True:
            if first_pass3410: first_pass3410 = False
            else: i += 1
            ...
            if (items[i].can_be_adj and items[i].begin_token.morph.class0_.is_verb): 
                ...
                if (not it.get_morph_class_in_dictionary().is_verb): 
                    continue
                if (it.is_value("УПОЛНОМОЧЕННЫЙ", None) or it.is_value("ДАННЫЙ", None)): 
                    continue
        ...
        if (noun is None): 
            return None
        res = NounPhraseToken._new896(first, noun.end_token, prep)
        if (adverbs is not None): 
            for a in adverbs: 
        ...
        if ((((typ) & (NounPhraseParseAttr.PARSEPRONOUNS))) == (NounPhraseParseAttr.NO) and ((res.morph.class0_.is_pronoun or res.morph.class0_.is_personal_pronoun))): 
            if (noun.end_token.is_value("ДАННЫЙ", None)): 
                pass
            else: 
                return None
        stat = None
        ...
        if (stat is not None): 
            for adj in items: 
                if (adj.morph.items_count > 1): 
                    ...
                    i1 = 0
                    i2 = 0
                    wrapi1898 = RefOutArgWrapper(0)
                    Utils.tryGetValue(stat, l1, wrapi1898)
                    i1 = wrapi1898.value
                    wrapi2897 = RefOutArgWrapper(0)
                    Utils.tryGetValue(stat, l2, wrapi2897)
                    i2 = wrapi2897.value
                    if (i1 < i2): 
    ...
    def __try_parse_en(first : 'Token', typ : 'NounPhraseParseAttr', max_char_pos : int) -> 'NounPhraseToken':
        ...
        if (res.morph.items_count == 0 and has_article): 
            res.morph.add_item(MorphBaseInfo._new547(MorphClass.NOUN, MorphNumber.SINGULAR))
        i = 0
        while i < (len(items) - 1): 
            res.adjectives.append(items[i])
            i += 1

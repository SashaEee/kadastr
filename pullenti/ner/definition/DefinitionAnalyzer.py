...
class DefinitionAnalyzer(Analyzer):
    ...
    def images(self) -> typing.List[tuple]:
        res = dict()
        res[MetaDefin.IMAGE_DEF_ID] = ResourceHelper2.get_bytes("defin.png")
        res[MetaDefin.IMAGE_ASS_ID] = ResourceHelper2.get_bytes("assert.png")
        return res
        ...
        if (kit.ontology is not None): 
            ...
            for it in kit.ontology.items: 
                if (isinstance(it.referent, DefinitionReferent)): 
                    ...
                    if (not termin in oh): 
                        oh[termin] = True
                        onto.add(Termin._new1308(termin, termin))
    ...
    def try_attach(t : 'Token', glos_regime : bool, onto : 'TerminCollection', max_char : int, this_is_def : bool=False) -> typing.List['ReferentToken']:
        ...
        if (t.is_hiphen or t.is_char_of(":") or ((can_next_sent and t.is_char('.')))): 
            ...
            if (t11 is not None and t11.is_char(':')): 
                t11 = t11.next0_
            if (t11 is not None and t11.is_value("НЕ", None) and t11.next0_ is not None): 
                is_not = True
                t11 = t11.next0_
            npt = NounPhraseHelper.try_parse(t11, NounPhraseParseAttr.NO, 0, None)
        ...
        if (name0 is None): 
            pass
        if (name0 is None): 
            dr.tag = (MetaToken._new933(l0, l1, normal_left))
        ...
        if (coef == DefinitionKind.UNDEFINED): 
            coef = DefinitionKind.ASSERTATION
        if (decree is not None): 
            dr.add_slot(DefinitionReferent.ATTR_DECREE, decree, False, 0)
        dr.kind = coef
        res = list()
    ...
    def __try_attach_end(self, t : 'Token', onto : 'TerminCollection', max_char : int) -> typing.List['ReferentToken']:
        ...
        if ((((l1.end_char - l0.end_char)) * 2) > ((r1.end_char - r0.end_char))): 
            return None
        dr = DefinitionReferent._new1310(DefinitionKind.DEFINITION)
    ...
    def __try_attach_misc_token(t : 'Token') -> 'MetaToken':
        ...
        if (t.is_value("КАК", None)): 
            ...
            while True:
                ...
                break
            if (t1 is not None): 
                res = MetaToken._new933(t, t1, MiscHelper.get_text_value(t, t1, GetTextAttr.KEEPQUOTES))
        ...
        if (npt is not None): 
            if (DefinitionAnalyzer.__m_misc_first_words.try_parse(npt.noun.begin_token, TerminParseAttr.NO) is not None): 
                res = MetaToken._new933(t, npt.end_token, npt.get_normal_case_text(None, MorphNumber.SINGULAR, MorphGender.UNDEFINED, False))
        ...
        if (t.is_value("В", None)): 
            ...
            if (npt is not None): 
                if (npt.noun.is_value("СМЫСЛ", None)): 
                    res = MetaToken._new933(t, npt.end_token, MiscHelper.get_text_value(t, npt.end_token, GetTextAttr.NO))
                    res.morph.class0_ = MorphClass.NOUN
                    return res
        return None

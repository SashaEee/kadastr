...
class DerivateDictionary:
    ...
    def __load_tree_node(self, tn : 'ExplanTreeNode') -> None:
        with self._m_lock: 
            pos = tn.lazy_pos
            if (pos > 0): 
                wrappos3189 = RefOutArgWrapper(pos)
                tn._deserialize(self.__m_buf, self, True, wrappos3189)
                pos = wrappos3189.value
            tn.lazy_pos = 0
    ...
    def _deserialize(self, str0_ : Stream, lazy_load : bool) -> None:
        ...
        with MemoryStream() as tmp: 
            ...
            pos = 0
            wrappos3193 = RefOutArgWrapper(pos)
            cou = wr.deserialize_int(wrappos3193)
            pos = wrappos3193.value
            while cou > 0: 
                wrappos3191 = RefOutArgWrapper(pos)
                p1 = wr.deserialize_int(wrappos3191)
                pos = wrappos3191.value
                ew = DerivateGroup()
                if (lazy_load): 
                    ew._lazy_pos = pos
                    pos = p1
                else: 
                    wrappos3190 = RefOutArgWrapper(pos)
                    ew._deserialize(wr, wrappos3190)
                    pos = wrappos3190.value
                ew.id0_ = (len(self._m_all_groups) + 1)
                self._m_all_groups.append(ew)
                cou -= 1
            self._m_root = ExplanTreeNode()
            wrappos3192 = RefOutArgWrapper(pos)
            self._m_root._deserialize(wr, self, lazy_load, wrappos3192)
            pos = wrappos3192.value
        self.__m_buf = wr

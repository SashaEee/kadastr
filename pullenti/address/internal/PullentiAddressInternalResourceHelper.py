...
class PullentiAddressInternalResourceHelper:
    
    @staticmethod
    def get_bytes(name : str) -> bytearray:
        
        names = Utils.getResourcesNames('pullenti.address.properties', '.png;.txt')
        for n in names: 
            if (Utils.endsWithString(n, name, True)): 
                if (len(name) < len(n)): 
                    if (n[len(n) - len(name) - 1] != '.'): 
                        continue
                try: 
                    inf = Utils.getResourceInfo('pullenti.address.properties', n)
                    if (inf is None): 
                        continue
                    with Utils.getResourceStream('pullenti.address.properties', n) as stream: 
    ...
    def get_string(name : str) -> str:
        arr = PullentiAddressInternalResourceHelper.get_bytes(name)
        if (arr is None): 
            return None

...
class RepObjTree:
    ...
    def open0_(self, dat : bytearray) -> None:
        ...
        while i < cou: 
            ...
            ind += 2
            tn = RepObjTreeNode()
            wrapind216 = RefOutArgWrapper(ind)
            self.__deserialize_node(tn, wrapind216)
            ind = wrapind216.value
    ...
    def __load_node(self, res : 'RepObjTreeNode') -> None:
        if (not res.loaded and res.lazy_pos > 0): 
            ind = res.lazy_pos + 4
            wrapind217 = RefOutArgWrapper(ind)
            self.__deserialize_node(res, wrapind217)
            ind = wrapind217.value
        res.loaded = True
    ...
    def find(self, str0_ : str) -> int:
        ...
        while i < len(str0_): 
            ...
            tn = None
            ch = str0_[i]
            wraptn218 = RefOutArgWrapper(None)
            inoutres219 = Utils.tryGetValue(dic, ch, wraptn218)
            tn = wraptn218.value
            if (not inoutres219): 
                return 0
            if (not tn.loaded): 
        ...
        res = 0
        wrapres220 = RefOutArgWrapper(0)
        inoutres221 = Utils.tryGetValue(gtn.objs, rest, wrapres220)
    ...
    def add(self, str0_ : str, id0_ : int) -> bool:
        ...
        while (i < len(str0_)) and (i < self.max_length): 
            tn = None
            ch = str0_[i]
            wraptn222 = RefOutArgWrapper(None)
            inoutres223 = Utils.tryGetValue(dic, ch, wraptn222)
            tn = wraptn222.value
            if (not inoutres223): 
                tn = RepObjTreeNode()
                tn.loaded = True

...
class RepAddrTree:
    ...
    def open0_(self, dat : bytearray) -> None:
        ...
        while i < cou: 
            ...
            ind += 2
            tn = RepAddrTreeNode()
            wrapind209 = RefOutArgWrapper(ind)
            self.__deserialize_node(tn, wrapind209)
            ind = wrapind209.value
    ...
    def __deserialize_node(self, res : 'RepAddrTreeNode', ind : int) -> None:
        ...
        if (cou > 0): 
            ...
            while cou > 0: 
                ...
                ind.value += 4
                o.lev = (Utils.valToEnum(self.__m_data[ind.value], AddrLevel))
                ind.value += 1
    ...
    def __load_node(self, res : 'RepAddrTreeNode') -> None:
        if (not res.loaded and res.lazy_pos > 0): 
            ind = res.lazy_pos + 4
            wrapind210 = RefOutArgWrapper(ind)
            self.__deserialize_node(res, wrapind210)
            ind = wrapind210.value
        res.loaded = True
    ...
    def find(self, path : str) -> typing.List['RepAddrTreeNodeObj']:
        ...
        while i < len(path): 
            ...
            tn = None
            ch = path[i]
            wraptn211 = RefOutArgWrapper(None)
            inoutres212 = Utils.tryGetValue(dic, ch, wraptn211)
            tn = wraptn211.value
            if (not inoutres212): 
                return None
            if (not tn.loaded): 
    ...
    def add(self, path : str, obj : 'RepAddrTreeNodeObj') -> bool:
        ...
        while i < len(path): 
            tn = None
            ch = path[i]
            wraptn213 = RefOutArgWrapper(None)
            inoutres214 = Utils.tryGetValue(dic, ch, wraptn213)
            tn = wraptn213.value
            if (not inoutres214): 
                tn = RepAddrTreeNode()
                tn.loaded = True

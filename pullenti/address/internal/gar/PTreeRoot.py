...
class PTreeRoot:
    ...
    def add(self, path : str, id0_ : int) -> 'PTreeNode':
        if (Utils.isNullOrEmpty(path)): 
            return None
        res = None
        wrapres67 = RefOutArgWrapper(None)
        inoutres68 = Utils.tryGetValue(self.children, path[0], wrapres67)
        res = wrapres67.value
        if (not inoutres68): 
            res = PTreeNode()
        ...
        while True:
            ...
            if (res.children is None): 
                res.children = dict()
            wraprr65 = RefOutArgWrapper(None)
            inoutres66 = Utils.tryGetValue(res.children, path[i], wraprr65)
            rr = wraprr65.value
            if (not inoutres66): 
                rr = PTreeNode()
    ...
    def find(self, path : str) -> 'PTreeNode':
        if (Utils.isNullOrEmpty(path)): 
            return None
        res = None
        wrapres71 = RefOutArgWrapper(None)
        inoutres72 = Utils.tryGetValue(self.children, path[0], wrapres71)
        res = wrapres71.value
        if (not inoutres72): 
            return None
        j = 1
        ...
        while True:
            ...
            if (res.children is None): 
                return None
            wraprr69 = RefOutArgWrapper(None)
            inoutres70 = Utils.tryGetValue(res.children, path[i], wraprr69)
            rr = wraprr69.value
            if (not inoutres70): 
                return None
            res = rr

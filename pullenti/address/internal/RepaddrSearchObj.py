...
class RepaddrSearchObj:
    ...
    def __init__(self, a : 'AddrObject', typs : 'RepTypTable') -> None:
        self.search_strs = list()
        self.type_ids = list()
        self.lev = AddrLevel.UNDEFINED
        self.src = None;
        self.src = a
    ...
    def calc_coef(self, o : 'RepAddrTreeNodeObj', parent : 'RepaddrObject', parent2 : 'RepaddrObject') -> int:
        ...
        for id0_ in self.type_ids: 
            if (id0_ in o.typ_ids): 
                ...
                break
        if (not eq_typs): 
            if (self.lev == AddrLevel.TERRITORY or self.lev == AddrLevel.LOCALITY): 
        ...
        if (parent is None): 
            ...
            if (i < 0): 
                if (parent2 is not None and AddressHelper.can_be_parent(self.lev, parent2.level)): 
    ...
    @staticmethod
    def get_search_strings(o : 'AddrObject') -> typing.List[str]:
        res = list()
        tmp = io.StringIO()
        h = Utils.asObjectOrNull(o.attrs, HouseAttributes)
        r = Utils.asObjectOrNull(o.attrs, RoomAttributes)
        if (h is not None): 
            if (h.plot_number is not None): 
                print('p', end="", file=tmp)
                print(("0" if Utils.isNullOrEmpty(h.plot_number) else h.number), end="", file=tmp)
            if (h.number is not None or h.typ != HouseType.UNDEFINED): 
                if (h.typ == HouseType.GARAGE): 
                    print('g', end="", file=tmp)
                print(("0" if Utils.isNullOrEmpty(h.number) else h.number), end="", file=tmp)
            if (h.build_number is not None): 
                print("b", end="", file=tmp)
                print(("0" if Utils.isNullOrEmpty(h.build_number) else h.build_number), end="", file=tmp)
            if (h.stroen_number is not None): 
                print("s", end="", file=tmp)
                print(("0" if Utils.isNullOrEmpty(h.stroen_number) else h.stroen_number), end="", file=tmp)
        elif (r is not None): 

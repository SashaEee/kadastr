...
class AddressRepository(object):
    ...
    def open0_(self, path_name : str) -> None:
        ...
        """
        self.__m_base_dir = pathlib.Path(path_name).resolve()
    ...
    def __get_tree(self, id0_ : int) -> 'RepObjTree':
        res = None
        wrapres231 = RefOutArgWrapper(None)
        inoutres232 = Utils.tryGetValue(self.__m_otrees, id0_, wrapres231)
        res = wrapres231.value
        if (inoutres232): 
    ...
    def __search(self, addr : 'TextAddress', add_ : bool) -> int:
        ...
        ret = 0
        for a in addr.items: 
            if (isinstance(a.attrs, RoomAttributes)): 
                room = a
        ...
        if (len(path) == 0): 
            return ret
        if ((path[0].level == AddrLevel.COUNTRY or path[0].level == AddrLevel.REGIONCITY or path[0].level == AddrLevel.REGIONAREA) or path[0].level == AddrLevel.CITY): 
            pass
        else: 
            return -1
        opath = list()
        ...
        while True:
            ...
            if (best is None): 
                ...
                new_obj.level = cur.level
                new_obj.types.extend(cur.attrs.types)
                if (len(opath) > 0): 
                    if (not AddressHelper.can_be_parent(new_obj.level, opath[0].level)): 
                        continue
                ...
                cur.rep_object_id = new_obj.id0_
                aobj = new_obj
                ...
                aobj = self.get_object(best.id0_)
                if (aobj is None): 
                    continue
                cur.rep_object_id = best.id0_
                if (add_): 
                    if (best.correct(aobj, self.__m_typs, (opath[0] if len(opath) > 0 else None))): 
                        self.__m_objs.add(best.id0_, aobj)
                        modif = True
                    for str0_ in so.search_strs: 
        ...
        for kk in range(3):
            pid = opath[0].id0_
            tobj = None
            if (kk == 0 and plot is not None): 
                tobj = plot
            elif (kk == 1 and house is not None): 
                tobj = house
            elif (kk == 2 and room is not None): 
                tobj = room
            if (tobj is None): 
                continue
            ...
            if (id0_ == 0): 
                ...
                new_obj.level = (AddrLevel.PLOT if kk == 0 else (AddrLevel.BUILDING if kk == 1 else AddrLevel.APARTMENT))
                if (not AddressHelper.can_be_parent(new_obj.level, opath[0].level)): 
                    continue
                new_obj.parents = list()
                ...
                if (len(tobj.gars) > 0): 
                    ...
                    for g in tobj.gars: 
                        new_obj.gar_guids.append(g.guid)
                self.__m_objs.add(new_obj.id0_, new_obj)
                tobj.rep_object_id = new_obj.id0_
                aobj = new_obj
                ret += 1
                modif = True
            else: 
                aobj = self.get_object(id0_)
                if (aobj is None): 
                    break
                tobj.rep_object_id = id0_
            if (add_): 
                for s in strs: 
                    if (tree.add(s, id0_)): 
                        modif = True
        ...
        if (modif): 
            ...
            while True:
                ...
                if (not AddressHelper.can_be_parent(opath[i].level, opath[i + 1].level)): 
                    continue
